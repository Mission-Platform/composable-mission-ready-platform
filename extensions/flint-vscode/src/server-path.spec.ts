...
describe('Flint server launcher', () => {
  ...
  it('blocks workspace-relative server overrides in an untrusted workspace', () => {
    expect(() => assertWorkspaceRelativeOverrideAllowed('flint.serverPath', 'tools/server.mjs', false)).toThrow(
      /untrusted workspace/u,
    );
    expect(() => assertWorkspaceRelativeOverrideAllowed('flint.serverPath', '/server/main.mjs', false)).toThrow(
      /untrusted workspace/u,
    );
    expect(() => assertWorkspaceRelativeExecutableAllowed('flint.nodePath', './node', false)).toThrow(
      /untrusted workspace/u,
    );
    expect(() => assertWorkspaceRelativeExecutableAllowed('flint.nodePath', 'node', false)).not.toThrow();
    expect(() => assertWorkspaceRelativeExecutableAllowed('flint.nodePath', '/opt/node24', false)).toThrow(
      /untrusted workspace/u,
    );
  });
  ...
  it('fails promptly when the Node version probe hangs', async () => {
    const root = mkdtempSync(path.join(os.tmpdir(), 'flint-node-probe-'));

...
data class FlintLaunchSettings(
    val nodeExecutable: String = "node",
    val serverCommand: String = "flint-lsp",
...
/**
 * Resolves the configured Node executable and `flint-lsp` command/path into a launchable
 * [GeneralCommandLine]. The default `serverCommand` ("flint-lsp") is a bare command, and
 ...
 */
object FlintCommandLine {
    private val CMD_SHIM_TARGET = Regex("""(?m)^#\s*cmd-shim-target=(.+)$""")
    private val JS_EXTENSIONS = setOf("js", "mjs", "cjs")

    fun from(settings: FlintSettingsState): FlintLaunchSettings = FlintLaunchSettings(
    ...
    fun buildDapAdapter(
        ...
        require(node.isNotEmpty()) {
            "Flint debugging requires a Node.js executable. Configure it in Settings | Tools | Flint."
        }
        val root = Path.of(projectRoot).toAbsolutePath().normalize()
        require(Files.isDirectory(root)) {
            "Flint debugging cannot start because the project root does not exist: $root"
        }
        require(Files.isRegularFile(adapterPath)) {
            "Flint debugging cannot find the packaged DAP adapter: $adapterPath"
        }
    ...
    fun build(settings: FlintLaunchSettings, projectRoot: String, projectTrusted: Boolean = true): GeneralCommandLine {
        val node = settings.nodeExecutable.trim()
        val server = settings.serverCommand.trim()
        require(node.isNotEmpty()) {
            "Flint requires a Node.js executable. Configure it in Settings | Tools | Flint."
        }
        require(server.isNotEmpty()) {
            "Flint requires a flint-lsp command or path. Configure it in Settings | Tools | Flint."
        }
        val root = Path.of(projectRoot).toAbsolutePath().normalize()
        require(Files.isDirectory(root)) {
            "Flint cannot start because the project root does not exist: $root"
        }
        validateExecutablePath(node, "Node.js executable", root, projectTrusted)
    ...
    /**
     * Locates the configured `flint-lsp` command/path and resolves it to something that
    ...
    private fun resolveServerCommand(server: String, projectRoot: Path, projectTrusted: Boolean): ResolvedServerCommand {
        ...
        require(projectTrusted || !hasPathSeparator) {
            "Flint cannot use the configured flint-lsp path in an untrusted project. Trust the project " +
        ...
        val candidate = if (hasPathSeparator) {
            ...
            require(Files.exists(path)) {
                "Flint could not find the configured flint-lsp command/path " +
                    "'$server' (resolved to '$path'). Install flint-lsp or update the plugin settings."
            }
            path
        } else {
            (if (projectTrusted) findInAncestorNodeModulesBin(server, projectRoot) else null)
                ...
                ?: error(
                    "Flint could not find a project-installed '$server'. Run " +
                        "'pnpm add flint-lsp' (or your package manager's equivalent) so that " +
                        "'node_modules/.bin/$server' exists under $projectRoot, install Node.js 24 or " +
                        "newer, or set an explicit server command/path in Settings | Tools | Flint.",
                )
        }
        ...
        if (jsEntryPoint != null) {
            require(projectTrusted || !isWithinProject(jsEntryPoint, projectRoot)) {
                "Flint resolved the server command to a project file in an untrusted project. Trust the project " +
        ...
        require(Files.isExecutable(candidate)) {
            "Flint found '$candidate' but could not resolve a JavaScript entry point and " +
                "the file is not directly executable. Configure an explicit flint-lsp " +
                "script path in Settings | Tools | Flint."
        }
    ...
    private fun validateExecutablePath(value: String, description: String, projectRoot: Path, projectTrusted: Boolean) {
        ...
        require(projectTrusted) {
            "Flint cannot use the configured $description path in an untrusted project. Trust the project " +
        ...
        require(Files.isRegularFile(path) && Files.isExecutable(path)) {
            "Flint could not find an executable $description at '$value'. " +
                "Install Node.js 24 or newer and flint-lsp, or update the plugin settings."
        }
    }
    ...
    internal fun parseArguments(value: String): List<String> {
        ...
        require(!escaped && quote == null) {
            "Flint server arguments contain an unterminated quote or escape."
        }

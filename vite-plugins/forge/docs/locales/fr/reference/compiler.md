...
Les contrats centraux résident dans `forge-plugins/forge-plugin-api/src/framework.ts` :
...
1. **Entrée et diagnostics :** inspectez `vite-plugins/forge/src/compiler/` pour l'analyse, la découverte, l'optimisation neutre,
...
3. **Forme de construction générique :** inspectez `vite-plugins/forge/src/generate.ts`, `generate-hooks.ts` et `tsdown.ts` pour le cache,
   comportement de sortie, de déclaration et de remplacement de l'appelant.
4. **Sortie CMS :** inspectez `forge-plugins/forge-cms-plugin-api/` pour le modèle de contenu, le pilote et la version
   helpers, puis la cible spécifique `forge-plugins/forge-cms-*` pour ses émetteurs et le mappage de la plateforme.

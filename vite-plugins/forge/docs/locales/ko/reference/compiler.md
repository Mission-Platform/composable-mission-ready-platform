...
중앙 계약은 `forge-plugins/forge-plugin-api/src/framework.ts`에 있습니다:
...
1. **입력 및 진단:** 구문 분석, 검색, 중립 최적화를 위해 `vite-plugins/forge/src/compiler/`을 검사합니다.
   의미론적 IR 구성 및 진단 집계.
2. **대상 동작:** 선택한 `forge-plugin-*` 패키지와 해당 `lower`, `optimize`, `generate`를 검사하고 빌드합니다.
   어댑터 구현.
3. **일반 빌드 형태:** 캐시에 대해 `vite-plugins/forge/src/generate.ts`, `generate-hooks.ts` 및 `tsdown.ts`을 검사합니다.
   출력, 선언 및 호출자 재정의 동작.
4. **CMS 출력:** 콘텐츠 모델, 드라이버 및 빌드에 대해 `forge-plugins/forge-cms-plugin-api/`을 검사합니다.
   헬퍼, 이미터 및 플랫폼 매핑을 위한 특정 `forge-plugins/forge-cms-*` 타겟입니다.

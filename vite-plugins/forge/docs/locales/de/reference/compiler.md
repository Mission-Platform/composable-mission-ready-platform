...
Die zentralen Verträge leben in `forge-plugins/forge-plugin-api/src/framework.ts`:
...
1. **Eingabe und Diagnose:** `vite-plugins/forge/src/compiler/` auf Analyse, Erkennung, neutrale Optimierung untersuchen,
...
3. **Generische Build-Form:** Überprüfen Sie `vite-plugins/forge/src/generate.ts`, `generate-hooks.ts` und `tsdown.ts` auf Cache.
   Ausgabe, Deklaration und Aufrufer-Override-Verhalten.
4. **CMS-Ausgabe:** Überprüfen Sie `forge-plugins/forge-cms-plugin-api/` auf das Inhaltsmodell, den Treiber und den Build

 ...
 *
 * The neutral `@mission-platform/forge` import is split into a value import from
...
/** Build the `import … from 'react'` + `import type … from '@mission-platform/forge'` replacements. */
export function buildReactImports(factory: ts.NodeFactory, neutral: NeutralImports): ts.ImportDeclaration[] {
  ...
  // Per-framework components (`Teleport`) are imported from `@mission-platform/forge/react`.

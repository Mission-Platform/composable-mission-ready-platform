...
export function hoistStaticJsx(sourceFile: ts.SourceFile): ts.SourceFile {
  ...
  for (let index = 0; index < rewritten.statements.length; index += 1) {
    ...
    if (ts.isExpressionStatement(statement) && ts.isStringLiteralLike(statement.expression) && index === 0) {
      insertAt = index + 1;
      continue;
    }
    break;
  ...
  const statements = [...rewritten.statements.slice(0, insertAt), ...hoisted, ...rewritten.statements.slice(insertAt)];

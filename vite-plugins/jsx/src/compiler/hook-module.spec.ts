...
const USE_D3 = [
  "import { useEffect, useRef, type MpDependencyList, type MpRef } from '@mission-platform/jsx';",
...
describe('compileHookModule — Vue', () => {
  ...
  it('translates useRef into a Vue ref and returns it as `Ref<…>` (mapping `MpRef`)', () => {
    const { code } = compileHookModule(USE_D3, { framework: 'vue' });
    expect(code).toContain('const reference = ref');

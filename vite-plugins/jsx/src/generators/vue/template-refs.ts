...
export function applyTemplateRefs(
  ...
  const converted = setupLines.map((line) => {
    const name = templateRefNames.find((refName) => line.startsWith(`const ${refName} = ref`));
    if (name === undefined) {
      return line;
    }
  ...
  // Drop the `ref` runtime import when no remaining setup line still calls it
  // (`useState`/other `useRef`s keep it alive when present).

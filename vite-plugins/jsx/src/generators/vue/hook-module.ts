 ...
 * of write-once composables authored against `@mission-platform/jsx`'s
...
const VUE_RUNTIME_IMPORTS: readonly string[] = ['ref', 'computed', 'watch', 'onMounted', 'onUnmounted', 'onUpdated'];
 ...
 * the still-referenced neutral **types** stay imported from `@mission-platform/jsx`,
...
export function emitVueHookModule(sourceFile: ts.SourceFile): string {
  ...
  if (neutralTypes.length > 0) {
    importLines.push(`import type { ${neutralTypes.join(', ')} } from '@mission-platform/jsx';`);
  }
  ...
  if (contextValues.length > 0) {
    importLines.push(`import { ${contextValues.join(', ')} } from '@mission-platform/jsx/vue';`);
  }

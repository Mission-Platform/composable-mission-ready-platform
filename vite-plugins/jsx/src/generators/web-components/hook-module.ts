 ...
 * as a `createContext`/`useContext` pair — carries no JSX and no custom-element
 * class, so it is emitted as a plain, importable `.ts` module rather than a
 * `LitElement` subclass. The neutral React-style hooks (`useState`/`useRef`/
 * `useEffect`/…) and the context primitives (`createContext`/`useContext`) are
 * the framework-neutral runtime baseline — deliberately render-once,
 * side-effect-free implementations (see `@mission-platform/jsx`'s
 ...
 * kept against the neutral package rather than remapped to a bespoke
 * Web-Components mechanism, mirroring how the React target needs no bespoke
 * hook emitter either (a neutral hook module already *is* a React one).
 *
 ...
 *   `@mission-platform/components`, …) is remapped to its `./web-components`
 *   build.
 *
...
/**
 * Build the replacement for the neutral `@mission-platform/jsx` import: the
...
export function emitWebComponentHookModule(rawSourceFile: ts.SourceFile): string {
    ...
    (context) => {
      ...
      const visit = (node: ts.Node): ts.Node | ts.Node[] => {
        if (ts.isImportDeclaration(node) && ts.isStringLiteral(node.moduleSpecifier)) {
          ...
          // Remap a write-once, framework-split workspace import to its `./web-components` build.
          const frameworkModule = frameworkSplitModule(node.moduleSpecifier.text, 'web-components');

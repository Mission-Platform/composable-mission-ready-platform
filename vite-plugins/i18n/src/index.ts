  ...
  return {
    ...
    resolveId(id) {
      ...
      if (id.startsWith(VIRTUAL_LOCALE_PREFIX)) {
        return RESOLVED_VIRTUAL_LOCALE_PREFIX + id.slice(VIRTUAL_LOCALE_PREFIX.length);
      }
      return;
    },
    load(id) {
      if (id === RESOLVED_VIRTUAL_MODULE_ID) {
        ...
        const localesDirectory = options.localesDir ? path.resolve(root, options.localesDir) : path.resolve(root, 'src/locales');
        ...
        if (fs.existsSync(localesDirectory)) {
          ...
          for (const file of files) {
            if (file.endsWith('.yaml') || file.endsWith('.yml')) {
              ...
              try {
                const content = fs.readFileSync(path.join(localesDirectory, file), 'utf8');

...
- 2bd1243: stop adding trailing commas in JSONC files
  ...
  The `**/*.jsonc` override used `trailingComma: 'es5'`, which made Prettier append a trailing comma before every
  closing `}`/`]`. JSONC consumers such as Wrangler's config reject those commas, so the override now uses
  `trailingComma: 'none'` while keeping the `jsonc` parser (so comments are still supported).
...
- 480191b: Add `htmlWhitespaceSensitivity: 'ignore'` to the Prettier config. This ensures multi-line Vue/HTML elements
  with inline content (e.g. `{{ label }}`) are formatted with the content on its own line rather than immediately
  following the closing `>` of the opening tag.

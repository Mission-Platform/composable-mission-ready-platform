...
function checkManifestDependencies(
  ...
  for (const section of depSections) {
    ...
    for (const [depName, spec] of Object.entries(deps)) {
      ...
      if (trimmed === "*" || trimmed === "latest") {
        if (isSeverityAtOrAbove("low", severityThreshold)) {
          findings.push({
            ...
            message: `Dependency "${depName}" in ${relativeFilePath} uses unpinned version "${trimmed}".`,
      ...
      if (trimmed.startsWith("http://")) {
        if (isSeverityAtOrAbove("high", severityThreshold)) {
          findings.push({
            ...
            message: `Dependency "${depName}" in ${relativeFilePath} uses unencrypted http:// URL: ${trimmed}.`,
            filePath: relativeFilePath,
            snippet: `"${depName}": "${spec}"`,
            remediation:
      ...
      // Check insecure Git protocols
      if (trimmed.startsWith("git://") || trimmed.startsWith("git+http://")) {
        if (isSeverityAtOrAbove("high", severityThreshold)) {
          findings.push({
            ...
            message: `Dependency "${depName}" in ${relativeFilePath} uses unencrypted Git protocol: ${trimmed}.`,
            filePath: relativeFilePath,
            snippet: `"${depName}": "${spec}"`,
            remediation:
...
export function auditDependencies(
  ...
  // 1. Scan workspace package.json manifests
    ...
    if (stdoutText.trim().length > 0) {
      try {
        ...
        if (auditData.advisories && typeof auditData.advisories === "object") {
          for (const [advisoryId, advisory] of Object.entries(
            ...
            const vulnRange = advisory.vulnerable_versions
            ...
            findings.push({
              ...
              title,
              message:
                `${title} in package "${pkgName}"${vulnRange}. ${advisory.overview ?? ""}`.trim(),

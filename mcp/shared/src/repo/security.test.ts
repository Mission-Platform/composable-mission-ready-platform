...
test("repository path guard rejects traversal and symlink escapes", () => {
  assert.throws(
    () => paths.resolveRepoPath("../outside.txt", "target"),
  ...
  assert.throws(
    () => paths.resolveRepoPath(linked, "target"),
    /within the repository root|symlink|outside/i,
  );
});
...
test("scanner skips symlinked documents and workspace members", () => {
  ...
  writeFileSync(
    join(outside, "member", "package.json"),
    JSON.stringify({ name: "@outside/member" }),
  );
  symlinkSync(join(outside, "member"), join(root, "packages", "linked-member"));
  mkdirSync(join(root, "packages", "regular-member"));
  writeFileSync(
    join(root, "packages", "regular-member", "package.json"),
    JSON.stringify({ name: "@mission-platform/regular-member" }),
  );
  mkdirSync(join(root, "packages", "ui", "nested-member"), { recursive: true });
  writeFileSync(
    join(root, "packages", "ui", "nested-member", "package.json"),
    JSON.stringify({ name: "@mission-platform/nested-member" }),
  );

  assert.deepEqual(
    scanner.listDocs().map(({ slug }) => slug),
    ["regular"],
  );
  assert.deepEqual(
    scanner.listGroup("packages").map(({ name }) => name),
    ["@mission-platform/nested-member", "@mission-platform/regular-member"],
  );
  assert.equal(
    scanner.findMember("packages", "nested-member")?.relativeDir,
    "packages/ui/nested-member",
  );
});
...
test("locale discovery ignores symlinked locale entries and writes fail closed", () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({ name: "@mission-platform/website" }),
  );
  writeFileSync(
    join(member, "locales", "en", "common.yaml"),
    "title: English\n",
  );
  mkdirSync(join(member, "locales", "es"), { recursive: true });
  symlinkSync(
    join(outside, "outside.md"),
    join(member, "locales", "es", "common.yaml"),
  );
  ...
  assert.throws(
    () =>
      locales.updateTranslation({
        ...
        code: "es",
        entries: { title: "Español" },
        apply: true,
      }),
    /symlink|outside/i,
  );
  assert.equal(
    locales.surveyLocales("apps").some(({ name }) => name === "website"),
    true,
  );
});

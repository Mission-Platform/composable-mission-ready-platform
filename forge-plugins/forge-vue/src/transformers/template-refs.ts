 ...
 * is switched from a plain `shallowRef<Element | null>(null)` to Vue 3.5's
 * purpose-built `useTemplateRef<Element>('name')`. Only the native-`<template>`
 * path emits such string bindings; the render-closure fallback binds an object
 * ref, where the plain `shallowRef(null)` declaration is correct.
 */

/**
 * Re-declare each `useRef` bound to an element via a `ref="name"` string binding
 * in `markup` as `useTemplateRef<Element>('name')`. Returns the (possibly
 * rewritten) setup lines and mutates `vueImports` — adding `useTemplateRef` and
 * dropping a now-unused `ref`/`shallowRef`.
 */
export function applyTemplateRefs(
  setupLines: readonly string[],
  markup: string,
  refElementTypes: ReadonlyMap<string, string | undefined>,
  vueImports: Set<string>,
): string[] {
  const templateRefNames = [...refElementTypes.keys()].filter((name) =>
    markup.includes(` ref="${name}"`),
  );
  ...
  const converted = setupLines.map((line) => {
    ...
    const name = templateRefNames.find(
      (refName) =>
        line.startsWith(`const ${refName} = shallowRef`) ||
        line.startsWith(`const ${refName} = ref`),
    );
    if (name === undefined) {
      return line;
    }

 ...
 */
export function readFrameworkDirective(
 ...
 */
export function moduleTargetsFramework(
  sourceFile: ts.SourceFile,
 ...
 */
export function stripFrameworkDirective(
  sourceFile: ts.SourceFile,
): ts.SourceFile {
  let inPrologue = true;
 ...
 */
export function reactClassNameValue(
  ...
  return factory.createCallExpression(
    factory.createIdentifier("classNames"),
    undefined,
    [...value.elements],
  );
}
 ...
 */
export function usesClassNamesArrayAttribute(
  sourceFile: ts.SourceFile,
): boolean {
  let found = false;
  const visit = (node: ts.Node): void => {
...
export function isSlotElement(
  ...
  if (ts.isJsxElement(node)) {
    return (
      ...
      node.openingElement.tagName.text === SLOT_TAG
    );
  }
  return false;
}
...
export function isFragmentElement(
  ...
  if (ts.isJsxElement(node)) {
    return (
      ts.isIdentifier(node.openingElement.tagName) &&
      node.openingElement.tagName.text === FRAGMENT_TAG
    );
  }
  return false;
}
...
export function slotFallbackChildren(
  node: ts.JsxSelfClosingElement | ts.JsxElement,
): ts.JsxChild[] {
...
export function isHasSlotCall(node: ts.Node): node is ts.CallExpression {
  return (
    ts.isCallExpression(node) &&
    ts.isIdentifier(node.expression) &&
    node.expression.text === HAS_SLOT_CALLEE
  );
}
...
export function isSlotHCall(node: ts.Node): node is ts.CallExpression {
  ...
  return (
    callee !== undefined && ts.isIdentifier(callee) && callee.text === SLOT_TAG
  );
}
...
export function readSlotHCallScope(
  ...
  const visited = rest.map(
    (property) => ts.visitNode(property, visit) as ts.ObjectLiteralElementLike,
  );
 ...
 */
function createSlotMemberAccess(
  factory: ts.NodeFactory,
  object: string,
  key: string,
): ts.Expression {
...
export function createVueHasSlotExpression(
  factory: ts.NodeFactory,
...
export function readSlotScope(
  ...
  for (const attribute of slotOpening(node).attributes.properties) {
    if (ts.isJsxSpreadAttribute(attribute)) {
      properties.push(
        factory.createSpreadAssignment(
          ts.visitNode(attribute.expression, visit) as ts.Expression,
        ),
      );
      continue;
    }
    ...
    properties.push(
      factory.createPropertyAssignment(attribute.name.text, value),
    );
  }
...
export function partitionSlottedChildren(
...
export function stripSlotAttribute<
  T extends ts.JsxElement | ts.JsxSelfClosingElement,
>(factory: ts.NodeFactory, element: T): T {
  ...
  return (context) => {
    ...
    const slotFunction = (children: ts.JsxChild[]): ts.Expression => {
      const stripped = children.map((child) =>
        ts.isJsxElement(child) || ts.isJsxSelfClosingElement(child)
          ? stripSlotAttribute(factory, child)
          : child,
      );
    ...
    const visit = (node: ts.Node): ts.Node => {
      if (
        ...
        const { defaultChildren, namedSlots } = partitionSlottedChildren(
          node.children,
        );
        ...
        if (meaningfulDefault.length > 0) {
          properties.push(
            factory.createPropertyAssignment(
              "default",
              slotFunction(meaningfulDefault),
            ),
          );
        }
        ...
        const replaced = factory.updateJsxElement(
          node,
          node.openingElement,
          [slotsChild],
          node.closingElement,
...
function lowercaseNativeEventName(name: string): string {
  return /^on[A-Z]/.test(name)
    ? `on${name.charAt(2)}${name.slice(3).toLowerCase()}`
    : name;
}
  ...
  return (context) => {
    ...
    const visit = (node: ts.Node): ts.Node => {
      if (
        ...
        !isComponentTagName(node.tagName)
      ) {
      ...
      if (
        ...
        const updated = factory.updateJsxElement(
          node,
          opening,
          node.children,
          node.closingElement,
      ...
      if (
        ...
        const rewrittenProperties = factory.updateObjectLiteralExpression(
          properties,
        ...
        const updated = factory.updateCallExpression(
          ...
          node.expression,
          node.typeArguments,
          [node.arguments[0], rewrittenProperties, ...node.arguments.slice(2)],
        );
        return ts.visitEachChild(updated, visit, context);
      }
      return ts.visitEachChild(node, visit, context);
        ...
        attributes.properties.map((property) => {
          if (ts.isJsxAttribute(property) && ts.isIdentifier(property.name)) {
            ...
            if (local !== undefined) {
              return factory.updateJsxAttribute(
                property,
                factory.createJsxNamespacedName(
                  factory.createIdentifier("onUpdate"),
...
export function isDynamicElement(
  ...
  if (ts.isJsxElement(node)) {
    return (
      ts.isIdentifier(node.openingElement.tagName) &&
      node.openingElement.tagName.text === DYNAMIC_TAG
    );
  }
  return false;
}
...
export function dynamicToHCall(
  ...
  for (const attribute of dynamicOpening(node).attributes.properties) {
    if (ts.isJsxSpreadAttribute(attribute)) {
      properties.push(
        factory.createSpreadAssignment(visitExpression(attribute.expression)),
      );
      continue;
    }
  ...
  return factory.createCallExpression(
    ...
    undefined,
    [tag, propertiesArgument, ...finalChildren],
  );
}
 ...
 */
export function usesComponentSelfReference(
  sourceFile: ts.SourceFile,
...
export function createReactSlotExpression(
  ...
  const access = createSlotMemberAccess(
    factory,
    propertiesParameterName,
    name ?? "children",
  );
  const read: ts.Expression =
    scope === undefined
      ? access
      : createReactScopedSlotRead(factory, access, scope);
  if (fallback.length === 0) {
    return read;
  }
  return factory.createBinaryExpression(
...
function appendExpressionFallback(
  ...
  const value =
    fallback.length === 1
      ? fallback[0]!
      : factory.createArrayLiteralExpression([...fallback], false);
  return factory.createBinaryExpression(
    read,
    factory.createToken(ts.SyntaxKind.QuestionQuestionToken),
    value,
...
export function resolveWorkspaceLocalImport(
  ...
  if (
    sourceRelativeToRoot.startsWith("..") ||
    path.isAbsolute(sourceRelativeToRoot)
  ) {
    return undefined;
  }
  ...
  const relative = path
    .relative(path.dirname(absoluteSource), target)
    .split(path.sep)
    .join("/");
...
export function rewriteWorkspaceLocalImports(
  sourceFile: ts.SourceFile,
  sourceRoot?: string,
): ts.SourceFile {
  ...
  const transformer: ts.TransformerFactory<ts.SourceFile> = (context) => {
    ...
    const visit = (node: ts.Node): ts.Node => {
      if (
        ...
        const resolved = resolveWorkspaceLocalImport(
...
export function readComponentImports(
  sourceFile: ts.SourceFile,
  sourceRoot?: string,
): ComponentImport[] {
...
export function readExternalImports(sourceFile: ts.SourceFile): string[] {
  ...
  for (const statement of sourceFile.statements) {
    ...
    if (
      ...
      specifier === NEUTRAL_MODULE ||
      STYLE_EXTENSIONS.test(specifier)
    ) {
      continue;
    }
 ...
 */
export function ensureI18nHookInComponent(
  factory: ts.NodeFactory,
  sourceFile: ts.SourceFile,
): ts.SourceFile {
  ...
  const transformer: ts.TransformerFactory<ts.SourceFile> = () => {
    const visit = (node: ts.Node): ts.Node => {
      ...
      // The `undefined` transformation context is required by the signature, so
...
export function transformI18nextCalls(
  factory: ts.NodeFactory,
  node: ts.Node,
): ts.Node {
  if (
 ...
 */
export function extractPropertySignatures(
  sourceFile: ts.SourceFile,
  interfaceName: string,
): PropertySignature[] {
 ...
 */
export function extractEventSignatures(
  sourceFile: ts.SourceFile,
  interfaceName: string,
): EventSignature[] {
    ...
    for (const member of statement.members) {
      if (
        ...
        events.push({
          ...
          paramsText: member.type.parameters
 ...
 */
export function extractModelSignatures(
  sourceFile: ts.SourceFile,
  interfaceName: string,
): ModelSignature[] {
  ...
  for (const statement of sourceFile.statements) {
    ...
    for (const member of statement.members) {
      ...
      const eventPropertyName = readModelTag(member);
      if (eventPropertyName === undefined) {
        continue;
      }
      const propertyName = member.name.text;
      models.push({
        propName: propertyName,
        modelName: propertyName === "modelValue" ? undefined : propertyName,
        typeText:
          member.type === undefined
            ? "unknown"
            : member.type.getText(sourceFile),
        optional: member.questionToken !== undefined,
        eventPropName: eventPropertyName,
      });
    }
  }
  return models;
...
function eventFor(
...
): { eventName: string; paramNames: string[] } | undefined {
  if (
  ...
  if (
    ...
    scope.destructuredProps.has(expression.text)
  ) {
...
/**
 * The model ref local a call/reference targets, if it is a model prop's paired
 * change event: either `properties.on<Event>` or a destructured `on<Event>`
 * local.
 */
function modelEventFor(
  scope: RewriteScope,
  expression: ts.Expression,
): string | undefined {
  if (
 ...
 */
export function createReferenceRewriter(
  scope: RewriteScope,
  ...
  return (context) => {
    ...
    const emitCall = (
      ...
      factory.createCallExpression(
        ...
        undefined,
        [factory.createStringLiteral(eventName), ...arguments_],
      );
    ...
    const forwardingArrow = (
      ...
      factory.createArrowFunction(
        ...
        emitCall(
          eventName,
          parameterNames.map((name) => factory.createIdentifier(name)),
        ),
      );
    ...
    const isShadowed = (name: string): boolean =>
      shadowStack.some((names) => names.has(name));
    ...
    const visitCore = (node: ts.Node): ts.Node => {
      ...
      if (
        ...
        return factory.updateArrowFunction(
          ...
          node.equalsGreaterThanToken,
          factory.createBlock(
            [factory.createExpressionStatement(rewrittenBody)],
            true,
          ),
        );
      }
      ...
      if (ts.isCallExpression(node)) {
        const modelLocal = modelEventFor(scope, node.expression);
      ...
      if (ts.isCallExpression(node)) {
        const event = eventFor(scope, node.expression);
        if (event !== undefined) {
          return emitCall(
            event.eventName,
            node.arguments.map(
      ...
      if (isSlotElement(node)) {
        const name = readSlotName(node);
        const fallback = slotFallbackChildren(node).map(
          (child) => ts.visitNode(child, visit) as ts.JsxChild,
        );
      ...
      if (isSlotHCall(node)) {
        ...
        const fallback = slotHCallFallback(node).map(
          (argument) => ts.visitNode(argument, visit) as ts.Expression,
        );
      ...
      if (
        ...
        const event = scope.eventProps.get(node.name.text) as {
          eventName: string;
          paramNames: string[];
        };
      ...
      if (
        ...
        const stateName = scope.setterToState.get(
      ...
      if (ts.isPropertyAssignment(node)) {
        const name = ts.isComputedPropertyName(node.name)
          ? factory.updateComputedPropertyName(
              node.name,
              ts.visitNode(node.name.expression, visit) as ts.Expression,
            )
          : node.name;
        return factory.updatePropertyAssignment(
          node,
        ...
        if (ts.isIdentifier(node.name)) {
          if (node.name.text === CLASS_NAME_ATTRIBUTE) {
            ...
            name = factory.createIdentifier(
              JSX_ATTRIBUTE_RENAMES.get(node.name.text) as string,
            );
          }
        }
        return factory.updateJsxAttribute(
          node,
          name,
          ts.visitNode(node.initializer, visit) as ts.JsxAttributeValue,
        );
      }
      ...
      if (ts.isJsxElement(node)) {
        return factory.updateJsxElement(
          ...
          node.children.map((child) =>
            wrapAsJsxChild(factory, ts.visitNode(child, visit) as ts.Node),
          ),
      ...
      if (ts.isIdentifier(node)) {
        ...
        if (
          scope.stateNames.has(node.text) ||
          scope.memoNames.has(node.text) ||
          scope.modelProps.has(node.text)
        ) {
        ...
        if (
          scope.destructuredProps.has(node.text) &&
          scope.modelEvents.has(node.text)
        ) {
        ...
        if (
          scope.destructuredProps.has(node.text) &&
          scope.eventProps.has(node.text)
        ) {
          const event = scope.eventProps.get(node.text) as {
            eventName: string;
            paramNames: string[];
          };
          return forwardingArrow(event.eventName, event.paramNames);
        ...
        if (scope.destructuredProps.has(node.text)) {
          ...
          const propertyName = scope.propAliases.get(node.text) ?? node.text;
          if (propertyName === "children") {
            return slotsDefaultCall();
          }
          return factory.createPropertyAccessExpression(
            factory.createIdentifier(scope.propsParamName),
            propertyName,
          );
        }
        return node;
...
/**
 * A `<name>.value` access for a candidate state — the shape both reads and write
 * targets take after the reference rewrite.
 */
function valueAccessName(
...
/**
 * The candidate states whose value is **read inside a nested closure** of this
 * function body and is **never written inside a nested closure** (an inner write
 * would make a hoisted snapshot stale). These are exactly the reads whose
 * narrowing the snapshot must restore.
 */
function namesToHoist(
  candidates: ReadonlySet<string>,
  body: ts.Block,
  ...
  const walk = (node: ts.Node, depth: number): void => {
    const name = valueAccessName(candidates, node);
  ...
  // Depth 0 is the function's own statement scope; a read only needs a snapshot
  // once it is used one or more closures deep.
...
/**
 * The `<name>.value` nodes that are assignment / update **targets** (writes), so
 * they are never rewritten to the read-only snapshot alias.
 */
function collectWriteTargets(root: ts.Node): Set<ts.Node> {
 ...
 */
export function createStateSnapshotHoister(
  scope: RewriteScope,
): ts.TransformerFactory<ts.Node> {
  ...
  return (context) => {
    ...
    const visit = (node: ts.Node): ts.Node => {
      if (
        ...
        const hoist = namesToHoist(candidates, node.body, writeTargets);
        if (hoist.length > 0) {
          const hoistSet = new Set(hoist);
          const aliased = aliasReads(
            node.body,
            hoistSet,
            writeTargets,
          ) as ts.Block;
          ...
          const body = factory.updateBlock(aliased, [
            ...snapshots,
            ...aliased.statements,
          ]);

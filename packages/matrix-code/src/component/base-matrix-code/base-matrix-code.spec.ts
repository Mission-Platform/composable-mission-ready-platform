...
import { BaseMatrixCode } from './base-matrix-code';

/**
 * Exercises the **neutral** `BaseMatrixCode` authored in this package, rendering
 ...
 */
const ReactMatrixCode = toReactComponent(BaseMatrixCode, 'MatrixCode');
const VueMatrixCode = toVueComponent(BaseMatrixCode, 'MatrixCode');

describe('BaseMatrixCode authors the same component for React and Vue', () => {
  it('renders an SVG with a module path and a background rect on both frameworks', async () => {
    ...
    for (const html of [react, vue]) {
      ...
      expect(html).toContain('class="base-matrix-code"');
  ...
  it('renders the save/copy toolbar on both frameworks when showActions is set', async () => {
    ...
    for (const html of [react, vue]) {
      ...
      // rendered with the shared `BaseButton` (their visible text is the
      // accessible name).
      expect(html).toContain('<svg');
  ...
  it('overlays a centre logo (plate + image) on both frameworks', async () => {
    ...
    for (const html of [react, vue]) {
      ...
      expect(html).toContain('base-matrix-code__logo-plate');
    }
  });
});

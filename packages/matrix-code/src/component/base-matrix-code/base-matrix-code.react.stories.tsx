import { MatrixCode } from '@mission-platform/matrix-code/react';

import type { Meta, StoryObj } from '@storybook/react-vite';

/**
 * `MatrixCode` is the **React** build of the write-once `BaseMatrixCode` in this
...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `MatrixCode` — a 2D Data Matrix (ECC 200) symbol authored once in the neutral JSX dialect and shipped to both React (this story, via `@mission-platform/matrix-code/react`) and Vue 3 (`@mission-platform/matrix-code/vue`). The payload is encoded entirely on the client by the WebAssembly `@mission-platform/matrix-code` encoder (automatic symbol sizing and Reed-Solomon error correction) and drawn as a crisp SVG. Opt into a save/copy toolbar via `showActions` (or the individual `show*Button` props). Styling comes from the co-located `base-matrix-code.module.scss`.',
      },
    },
  },
  argTypes: {
...
} satisfies Meta<typeof MatrixCode>;

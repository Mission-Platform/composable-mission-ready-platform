...
export default defineVitestConfig({
  ...
  overrides: {
    // The neutral `BaseMatrixCode` is authored in the `@mission-platform/forge`
    ...
    resolve: {
      alias: [
        ...
        { find: /^@mission-platform\/matrix-code$/, replacement: resolve(__dirname, 'src/index.ts') },
    ...
    test: {
      // Instantiate the wasm modules once, before any spec, so the synchronous
      // `encodeMatrix`/`decodeMatrix` work under Vitest (where the binary isn't
      // inlined).
      setupFiles: ['./src/test-setup.ts'],
      // The neutral `@mission-platform/icons` (used by the component's toolbar)
      // ships only per-framework builds, so it must be inlined and transformed by
      // Vite — with the same `h` JSX factory — rather than externalised and
      // loaded as raw `.tsx` by Node.
      server: {
        deps: {

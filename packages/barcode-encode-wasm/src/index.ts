...
// (bundler target) glue for the `barcode-encode` Rust crate.
//
...
import * as bindings from "./wasm/barcode-encode_bg.js";
import wasmBase64 from "./wasm/barcode-encode_bg.wasm";

import type * as WasmApi from "./wasm/barcode-encode.js";
 ...
 * (`1` = bar, `0` = space). Returns `undefined` when the symbology is unknown or
 * the payload is invalid. Initialization occurs synchronously on the first
 * call.
 */
export const encode: typeof WasmApi.encode = (...arguments_) => {
  ensureInitialized();
  return glue.encode(...arguments_);
};

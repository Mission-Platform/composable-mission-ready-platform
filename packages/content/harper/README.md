...
Harper grammar and style checker integration for Monaco Editor on Mission Platform. Powered by WebAssembly
(`harper.js`), running entirely locally in browser Web Workers.
...
- **Vue 3 Composable & Imperative Helpers**: `useHarperMonaco` for Vue 3 SFCs and `attachHarperMonaco` for
  framework-agnostic or vanilla JS setups.
...
- `@mission-platform/harper`: Main package entry exporting `useHarperMonaco`, `attachHarperMonaco`, and types
  (`HarperIssue`).

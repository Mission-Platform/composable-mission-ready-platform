...
function documentForCommand(value: string): Document {
  return { queryCommandValue: vi.fn().mockReturnValue(value) } as unknown as Document;
}
...
describe('queryBlockFormat', () => {
  ...
  it('maps the current block tag onto its command', () => {
    expect(queryBlockFormat(documentForCommand('h1'))).toBe('heading1');
    expect(queryBlockFormat(documentForCommand('H4'))).toBe('heading4');
    expect(queryBlockFormat(documentForCommand('blockquote'))).toBe('blockquote');
    expect(queryBlockFormat(documentForCommand('pre'))).toBe('monospace');
    expect(queryBlockFormat(documentForCommand('p'))).toBe('paragraph');
    expect(queryBlockFormat(documentForCommand('unknown'))).toBe('paragraph');
  });

import { createFlintLanguageService } from '@mission-platform/flint-language-service';
import { describe, expect, it, vi } from 'vitest';

import { attachFlintMonaco } from './flint';
...
class FakeModel {
  readonly uri = {
    path: '/workspace/example.flint',
    toString: () => 'file:///workspace/example.flint',
  };
...
describe('Flint Monaco adapter', () => {
  it('registers lexical tokens with standard Monaco scopes that built-in themes color', async () => {
    ...
    const handle = attachFlintMonaco(fake.editor as never, fake.runtime as never, {
    ...
    expect(tokenResult).toMatchObject({
      tokens: expect.arrayContaining([
        // 'export' and 'fn' are keywords → should emit 'keyword' (not 'flint.keyword')
        expect.objectContaining({ scopes: 'keyword' }),
        // 'i32' is a primitive type → should emit 'type' (not 'flint.type')
    ...
    // Verify no custom 'flint.*' scopes are emitted (they won't be colored by built-in themes)
    const allScopes = (tokenResult as { tokens: Array<{ scopes: string }> }).tokens.map((t) => t.scopes);
    expect(allScopes).not.toContain(expect.stringContaining('flint.'));
  ...
  it('reports unrelated errors while accepting private helpers by default', async () => {
    ...
    expect(invalidMarkers[0]?.code).toMatch(/^FLINT-/);
    expect(invalidMarkers[0]?.source).toContain('flint/');

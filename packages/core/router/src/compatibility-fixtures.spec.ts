...
describe('shared routing compatibility fixtures', () => {
  it('preserves the docs locale route ordering and nested/catch-all slugs', () => {
    ...
    expect(matchRoutes(routes, '/packages/tooling/configs/eslint-config/index')?.flat.name).toBe('doc');
    expect(matchRoutes(routes, '/fr/packages/tooling/configs/eslint-config/index')?.flat.name).toBe('localized-doc');
    expect(matchRoutes(routes, '/fr/search')?.flat.name).toBe('localized-search');
    expect(matchRoutes(routes, '/fr/packages/tooling/configs/eslint-config/index')?.params).toEqual({
      locale: 'fr',

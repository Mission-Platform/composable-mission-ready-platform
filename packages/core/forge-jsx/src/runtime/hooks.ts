/**
 * Framework-neutral, React-style hooks for the `@mission-platform/forge-jsx` dialect.
 *
 ...
 * runtime adapters (`@mission-platform/forge-adapters/react`, `.../vue`) and by SSR: they
...
export function useState<T>(initial?: T | (() => T)): [T, MpSetState<T>] {
  const value = typeof initial === 'function' ? (initial as () => T)() : (initial as T);
  return [value, () => {}];
}

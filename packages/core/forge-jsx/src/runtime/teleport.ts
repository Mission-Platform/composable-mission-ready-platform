 ...
 * `createPortal` (via `@mission-platform/forge-adapters/react`) and Vue's built-in
...
export const Teleport: MpComponent<MpTeleportProperties> = () => {
  throw new Error(
    '@mission-platform/forge-jsx: <Teleport> is a compile-time / adapter marker and must not be rendered directly.',
  );
};

 ...
 * - **Vue** — their import is rewritten to `@mission-platform/forge-adapters/vue`, whose
 ...
 * (`@mission-platform/forge-adapters/react`, `.../vue`) and SSR: a `createContext` carries a
 ...
 */
export const MP_CONTEXT: unique symbol = Symbol.for('@mission-platform/forge-jsx.context');
...
export function createContext<T>(defaultValue: T): MpContext<T> {
  ...
  const Provider = (() => {
    throw new Error(
      '@mission-platform/forge-jsx: a context <Provider> is a compile-time / adapter marker and must not be rendered directly.',
    );

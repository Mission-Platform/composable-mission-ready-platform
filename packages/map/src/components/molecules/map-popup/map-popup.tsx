...
/**
 * `BaseMapPopup` — adds a MapLibre `Popup` to the nearest `<MapLibre>` ancestor's
 * map. Renders no DOM of its own (the popup lives in the map canvas). Authored
 * once in the neutral JSX dialect.
 */
export function BaseMapPopup(properties: Readonly<MapPopupProperties>): MpElement | null {
  const map = useMap();
  usePopup(map, {

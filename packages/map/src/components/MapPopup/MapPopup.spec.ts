...
const { mockSetLngLat, mockSetHTML, mockAddTo, mockRemove, MockPopup } = vi.hoisted(() => {
  ...
  function MockPopupImpl() {
    return { setLngLat: mockSetLngLat, setHTML: mockSetHTML, setText: vi.fn().mockReturnThis(), addTo: mockAddTo, remove: mockRemove, on: vi.fn() }
  }
  return { mockSetLngLat, mockSetHTML, mockAddTo, mockRemove, MockPopup: vi.fn().mockImplementation(MockPopupImpl) }
})

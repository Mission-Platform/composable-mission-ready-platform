...
describe('usePopup', () => {
  ...
  it('creates and adds a popup to the map when the map ref becomes available', async () => {
    ...
    const Wrapper = defineComponent({
      setup() {
        return usePopup(mapRef, { lngLat: [0, 0], content: '<p>Hello</p>' })
      },
      template: '<div />',
    })
  ...
  it('uses setText when isText is true', async () => {
    ...
    const Wrapper = defineComponent({
      setup() {
        return usePopup(mapRef, { lngLat: [0, 0], content: 'plain text', isText: true })
      },
      template: '<div />',
    })
  ...
  it('does not add popup to map when open is false', async () => {
    ...
    const Wrapper = defineComponent({
      setup() {
        return usePopup(mapRef, { lngLat: [0, 0], content: '', open: false })
      },
      template: '<div />',
    })
  ...
  it('updates HTML content reactively', async () => {
    ...
    const Wrapper = defineComponent({
      setup() {
        return usePopup(mapRef, { lngLat: [0, 0], content })
      },
      template: '<div />',
    })

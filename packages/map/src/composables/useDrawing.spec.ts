...
import type { DrawnFeature } from './useDrawing'
...
function mountDrawing(initialFeatures?: DrawnFeature[]) {
  const mapRef = shallowRef<Map | null>(makeFakeMap())
  let drawing: ReturnType<typeof useDrawing>

  const Wrapper = defineComponent({
    setup() {
      drawing = useDrawing(mapRef, { initialFeatures })
    },
    template: '<div />',
  })

  mount(Wrapper)
  return { drawing: drawing!, mapRef }
}
...
describe('useDrawing — initial state', () => {
  it('starts with null mode and empty features', () => {
    ...
    expect(drawing.mode.value).toBeNull()
    expect(drawing.features.value.features).toHaveLength(0)
    expect(drawing.selectedId.value).toBeNull()
  })
...
describe('useDrawing — drawing modes', () => {
  ...
  it('shows a draft feature while drawing', () => {
    ...
    drawing.handleMapClick(makeClickEvent(0, 0))
    drawing.handleMapClick(makeClickEvent(1, 0))
    expect(drawing.draftFeature.value).not.toBeNull()
...
describe('useDrawing — selection', () => {
  it('selects a feature via queryRenderedFeatures in idle mode', async () => {
    ...
    const mapRef = shallowRef<Map | null>(makeFakeMap([featureHit]))
    let drawing: ReturnType<typeof useDrawing>

    const Wrapper = defineComponent({
      setup() {
        drawing = useDrawing(mapRef)
      },
      template: '<div />',
    })
    mount(Wrapper)
    ...
    ;(mapRef.value!.queryRenderedFeatures as ReturnType<typeof vi.fn>).mockReturnValue([])
    drawing!.handleMapClick(makeClickEvent(1, 1))
    expect(drawing!.selectedId.value).toBeNull()
  })
  ...
  it('deselects when selectFeature(null) is called', async () => {
    const { drawing } = mountDrawing()
...
describe('useDrawing — ghost preview', () => {
  it('ghostFeature is null when no mode is active', () => {
    const { drawing } = mountDrawing()
    expect(drawing.ghostFeature.value).toBeNull()
  })
  ...
  it('ghostFeature updates when mousemove is called during line drawing', () => {
    ...
    expect(drawing.ghostFeature.value).not.toBeNull()
...
describe('useDrawing — anchorPoint', () => {
  it('is null when not drawing', () => {
    const { drawing } = mountDrawing()
    expect(drawing.anchorPoint.value).toBeNull()
  })
...
describe('useDrawing — handleMapMoveEnd (reprojection)', () => {
  it('recomputes polygon ring for a square after viewport change', async () => {
    const { drawing, mapRef } = mountDrawing()
    ...
    const ringBefore = JSON.stringify(
      (drawing.features.value.features[0] as DrawnFeature).geometry,
    )
    ...
    ;(mapRef.value!.project as ReturnType<typeof vi.fn>).mockImplementation(
      (lngLat: [number, number]) => ({ x: lngLat[0] * 200, y: lngLat[1] * 50 }),
    )
    ;(mapRef.value!.unproject as ReturnType<typeof vi.fn>).mockImplementation(
      (pt: [number, number]) => ({ lng: pt[0] / 200, lat: pt[1] / 50 }),
    )
    ...
    const props = (drawing.features.value.features[0] as DrawnFeature).properties
    expect(props._anchor).toEqual([1, 2])
    expect(props._edge).toEqual([4, 3])
  })
  ...
  it('does not change line geometry after moveend', async () => {
    ...
    expect(drawing.features.value.features).toHaveLength(1)
    const geomBefore = JSON.stringify(
      (drawing.features.value.features[0] as DrawnFeature).geometry,
    )
    ...
    const geomAfter = JSON.stringify(
      (drawing.features.value.features[0] as DrawnFeature).geometry,
    )
...
describe('useDrawing — removeVertex', () => {
  ...
  it('removes a vertex from a polygon when it has more than 3 unique vertices', async () => {
    ...
    const ring = ((drawing.features.value.features[0] as DrawnFeature).geometry as { coordinates: number[][][] }).coordinates[0]
  ...
  it('does nothing when removing a polygon vertex would leave fewer than 3 unique vertices', async () => {
    ...
    const id = (drawing.features.value.features[0] as DrawnFeature).id
    const ringBefore = JSON.stringify(
      ((drawing.features.value.features[0] as DrawnFeature).geometry as { coordinates: number[][][] }).coordinates[0],
    )
    drawing.removeVertex(id, 0)
    await nextTick()
...
describe('useDrawing — insertVertex', () => {
  it('inserts a vertex into a line on the nearest segment', async () => {
    ...
    const id = (drawing.features.value.features[0] as DrawnFeature).id
    const before = ((drawing.features.value.features[0] as DrawnFeature).geometry as { coordinates: number[][] }).coordinates.length

    drawing.insertVertex(id, [1, 0])
    await nextTick()

    const after = ((drawing.features.value.features[0] as DrawnFeature).geometry as { coordinates: number[][] }).coordinates.length
  ...
  it('inserts a vertex into a polygon on the nearest segment', async () => {
    ...
    const id = (drawing.features.value.features[0] as DrawnFeature).id
    const ringBefore = ((drawing.features.value.features[0] as DrawnFeature).geometry as { coordinates: number[][][] }).coordinates[0].length

    drawing.insertVertex(id, [1, 0]) // midpoint of bottom edge
    await nextTick()

    const ringAfter = ((drawing.features.value.features[0] as DrawnFeature).geometry as { coordinates: number[][][] }).coordinates[0].length
...
describe('useDrawing — splitSelected', () => {
  ...
  it('handles splitting a 2-vertex line by inserting a midpoint', async () => {
    ...
    expect(coords0[coords0.length - 1]).toEqual(coords1[0])
  })
})
...
describe('useDrawing — joinLines', () => {
  ...
  it('picks the nearest endpoint pair when lines are not already touching', async () => {
    ...
    expect(coords[coords.length - 1]).toEqual([11, 0])
  })
...
describe('useDrawing — idle dblclick vertex ops', () => {
  it('removes a vertex via dblclick on vertex handle in idle mode', async () => {
    ...
    ;(mapRef.value!.queryRenderedFeatures as ReturnType<typeof vi.fn>)
      .mockReturnValueOnce([{ properties: { vertexIndex: 1 } }]) // vertex hit
    ;(mapRef.value!.queryRenderedFeatures as ReturnType<typeof vi.fn>)
      .mockReturnValueOnce([]) // shape hit (not reached)

    drawing!.handleMapDblClick(makeClickEvent(1, 0, 100, 0))
    await nextTick()

    const coords = ((drawing!.features.value.features[0] as DrawnFeature).geometry as { coordinates: number[][] }).coordinates
  ...
  it('inserts a vertex via dblclick on a line segment in idle mode', async () => {
    ...
    const id = (drawing!.features.value.features[0] as DrawnFeature).id
    drawing!.selectFeature(id)
    const coordsBefore = ((drawing!.features.value.features[0] as DrawnFeature).geometry as { coordinates: number[][] }).coordinates.length
    ...
    const coordsAfter = ((drawing!.features.value.features[0] as DrawnFeature).geometry as { coordinates: number[][] }).coordinates.length
...
describe('useDrawing — geodesic toggle', () => {
  ...
  it('flat moveSelected shifts coordinates by raw delta without geodesic warp', async () => {
    ...
    const before = drawing.features.value.features[0]?.geometry as { type: string; coordinates: number[][] }
    drawing.moveSelected(1, 2)

    const after = drawing.features.value.features[0]?.geometry as { type: string; coordinates: number[][] }

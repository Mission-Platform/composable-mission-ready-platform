...
function commitShape(
  ...
  switch (mode) {
    ...
    case 'square': {
      return vertices.length >= 2 && map
        ? buildSquare(vertices[0], vertices.at(-1) ?? [0, 0], map)
        : undefined
    }
    case 'circle': {
      return vertices.length >= 2 && map
        ? buildCircle(vertices[0], vertices.at(-1) ?? [0, 0], map)
        : undefined
    }
    case 'triangle': {
      return vertices.length >= 2 && map
        ? buildTriangle(vertices[0], vertices.at(-1) ?? [0, 0], map)
        : undefined
    }
    default: {
      return undefined
...
export function useDrawing(
  _mapReference: ShallowRef<Map | null>,
  ...
  const features = computed<FeatureCollection>(() =>
    featureCollection(committedFeatures.value as Feature[]),
  )
      ...
      if (geom.type === 'LineString') {
        ...
        if (coords.length >= 2) {
          const mid = midpoint(
            ...
            point(coords.at(-1) as [number, number]),
          )
          labelFeatures.push({
            type: 'Feature',
            id: `${f.id}-label`,
  ...
  const anchorPoint = computed<Feature<Point> | undefined>(() => {
    if (
      !mode.value ||
      !['square', 'circle', 'triangle'].includes(mode.value) ||
      draftVertices.value.length === 0
    ) {
      return
    }
  ...
  function replaceSelected(transform: (f: DrawnFeature) => DrawnFeature): void {
    ...
    committedFeatures.value = committedFeatures.value.map((f) =>
      f.id === selectedId.value ? transform(f) : f,
    )
  }
  ...
  function translateFeatureFlat(f: DrawnFeature, deltaLng: number, deltaLat: number): DrawnFeature {
    function shiftCoords(coords: Position[]): Position[] {
      return coords.map(([lng, lat, ...rest]) => [lng + deltaLng, lat + deltaLat, ...rest])
    }
    const geom = f.geometry
    let newGeom: Geometry
    switch (geom.type) {
      case 'LineString': {
        newGeom = { ...geom, coordinates: shiftCoords(geom.coordinates) }

        break
      }
      case 'Polygon': {
        newGeom = { ...geom, coordinates: geom.coordinates.map(shiftCoords) }
    ...
    if (Array.isArray(properties._anchor)) {
      properties._anchor = [properties._anchor[0] + deltaLng, properties._anchor[1] + deltaLat] as [
        number,
        number,
      ]
    }
    if (Array.isArray(properties._edge)) {
      properties._edge = [properties._edge[0] + deltaLng, properties._edge[1] + deltaLat] as [
        number,
        number,
      ]
    }
  ...
  function scaleFeatureFlat(f: DrawnFeature, factor: number): DrawnFeature {
    ...
    function scaleCoords(coords: Position[]): Position[] {
    ...
    let newGeom: Geometry
    if (geom.type === 'LineString') {
      newGeom = { ...geom, coordinates: scaleCoords(geom.coordinates) }
    } else if (geom.type === 'Polygon') {
      newGeom = { ...geom, coordinates: geom.coordinates.map(scaleCoords) }
    } else {
      newGeom = geom
    }
  ...
  function splitSelected(): void {
    ...
    committedFeatures.value = [
      ...committedFeatures.value.filter((f) => f.id !== selectedId.value),
      feat1,
      feat2,
    ]
  ...
  function joinLines(id1: FeatureId, id2: FeatureId): void {
    ...
    const options: { d: number; merged: Position[] }[] = [
      ...
      { d: distribution2(end1, end2), merged: [...c1, ...[...c2].reverse()] },
      { d: distribution2(start1, end2), merged: [...c2, ...c1] },
      { d: distribution2(start1, start2), merged: [...[...c2].reverse(), ...c1] },
    ]
    const best = options.reduce((a, b) => (b.d < a.d ? b : a), { d: Infinity, merged: [] })
    ...
    committedFeatures.value = [
      ...committedFeatures.value.filter((f) => f.id !== id1 && f.id !== id2),
      merged,
    ]

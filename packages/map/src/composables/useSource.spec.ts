...
function makeFakeMap(existingSource: GeoJSONSource | null = null): Map {
  return {
...
describe('useSource', () => {
  ...
  it('adds the source to the map when the map ref becomes available', async () => {
    ...
    const Wrapper = defineComponent({
      setup() {
        useSource(mapRef, { id: 'my-source', source: geojsonSource })
      },
      template: '<div />',
    })
  ...
  it('removes the source on unmount', async () => {
    ...
    const Wrapper = defineComponent({
      setup() {
        useSource(mapRef, { id: 'removable', source: geojsonSource })
      },
      template: '<div />',
    })
  ...
  it('does not add a source if it already exists', async () => {
    ...
    const Wrapper = defineComponent({
      setup() {
        useSource(mapRef, { id: 'existing', source: geojsonSource })
      },
      template: '<div />',
    })
  ...
  it('calls setData() instead of remove+add when GeoJSON data changes', async () => {
    ...
    const sourceRef = shallowRef<SourceSpecification>({
      type: 'geojson',
      data: { type: 'FeatureCollection', features: [] },
    })
    const mapRef = shallowRef<Map | null>(mapWithSource)

    const Wrapper = defineComponent({
      setup() {
        useSource(mapRef, { id: 'live', source: sourceRef })
      },
      template: '<div />',
    })
    ...
    sourceRef.value = { type: 'geojson', data: newData }
    await nextTick()

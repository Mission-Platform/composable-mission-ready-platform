...
import { BaseThreeCanvas, type ThreeContext } from '@mission-platform/three';
 ...
 * `BaseThreeCanvas` component's `onReady` callback (which hands back the
...
function ThreeSpinningCube({ color = '#4f46e5', wireframe = false, speed = 0.01 }: ThreeSpinningCubeProperties) {
  ...
  return (
    <div style={{ width: '100%', height: '360px', background: '#0b0b0f' }}>
      <BaseThreeCanvas onReady={onReady} />
    </div>
  );
}
...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          '`@mission-platform/three` is the framework-neutral Three.js integration. This story mounts the write-once `BaseThreeCanvas` component and, through its `onReady` callback, adds a cube to the `useThree`-managed scene and animates it every frame — the same neutral source compiled to each supported framework.',
      },
    },
  },
  argTypes: {

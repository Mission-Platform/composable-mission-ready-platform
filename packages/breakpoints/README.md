...
- `@mission-platform/breakpoints` — the neutral JSX source barrel (compiled by `@mission-platform/vite-plugin-jsx`).
...
- `<ShowAt min="md">`: Conditionally renders children/slot content when the viewport is at or above `min` (and/or strictly below `max`).
- `<HideAt min="lg">`: The inverse of `<ShowAt>` — hides children/slot content when the condition is met.
- `<BreakpointDebug />`: Development-only overlay displaying the current active breakpoint (labels localised via i18next, `mp.breakpoints` namespace).
...
> The Vue-only `useBreakpoints` composable has been removed. Build custom reactive viewport logic on the `/core` helpers with your framework's own hooks (see, for example, `apps/service-monitor`'s React `useCompactViewport` built on `maxMediaQuery`).

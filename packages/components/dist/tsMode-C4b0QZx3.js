import { a as e, c as t, i as n, n as r, o as i, r as a } from "./editor.api2-BNt2jrne.js";
import { n as o } from "./monaco.contribution-DP_oi8yQ.js";
import { t as s } from "./workers-B5vBBmRC.js";
...
var c = class {
	...
	_getClient() {
		return this._client ||= (async () => (this._worker = s({
			moduleId: "vs/language/typescript/tsWorker",
			createWorker: () => new Worker(new URL(
				/* @vite-ignore */
				"/assets/ts.worker-LMGWijQO.js",
				"" + import.meta.url
			), { type: "module" }),
			label: this._modeId,
			keepIdleModels: !0,
		...
		}), this._defaults.getEagerModelSync() ? await this._worker.withSyncedResources(i.getModels().filter((e) => e.getLanguageId() === this._modeId).map((e) => e.uri)) : await this._worker.getProxy()))(), this._client;
	}
...
function u(e, t, n = 0) {
	...
	if (r += e.messageText, n++, e.next) for (let i of e.next) r += u(i, t, n);
	return r;
}
function d(e) {
...
var f = class {
	...
	getOrCreateModel(t) {
		...
		if (r) return r;
		if (this.isLibFile(n) && this._hasFetchedLibFiles) return i.createModel(this._libFiles[n.path.slice(1)], "typescript", n);
		let a = o.getExtraLibs()[t];
		return a ? i.createModel(a.content, "typescript", n) : null;
	}
	_containsLibFile(e) {
	...
	constructor(e, t, n, r) {
		...
		let a = (e) => {
			...
			}, r, a = e.onDidChangeContent(() => {
				clearTimeout(r), r = window.setTimeout(t, 500);
			}), o = e.onDidChangeAttached(() => {
				let { onlyVisible: n } = this._defaults.getDiagnosticsOptions();
				n && (e.isAttachedToEditor() ? t() : i.setModelMarkers(e, this._selector, []));
			});
			this._listener[e.uri.toString()] = { dispose() {
				a.dispose(), o.dispose(), clearTimeout(r);
			} }, t();
		}, o = (e) => {
			i.setModelMarkers(e, this._selector, []);
			let t = e.uri.toString();
			this._listener[t] && (this._listener[t].dispose(), delete this._listener[t]);
		};
		this._disposables.push(i.onDidCreateModel((e) => a(e))), this._disposables.push(i.onWillDisposeModel(o)), this._disposables.push(i.onDidChangeModelLanguage((e) => {
			o(e.model), a(e.model);
		})), this._disposables.push({ dispose() {
			for (let e of i.getModels()) o(e);
		} });
		let s = () => {
			for (let e of i.getModels()) o(e), a(e);
		};
		this._disposables.push(this._defaults.onDidChange(s)), this._disposables.push(this._defaults.onDidExtraLibsChange(s)), i.getModels().forEach((e) => a(e));
	}
	dispose() {
		this._disposables.forEach((e) => e && e.dispose()), this._disposables = [];
	}
	async _doValidate(t) {
		let n = await this._worker(t.uri);
		if (t.isDisposed()) return;
		let r = [], { noSyntaxValidation: a, noSemanticValidation: o, noSuggestionDiagnostics: s } = this._defaults.getDiagnosticsOptions();
		a || r.push(n.getSyntacticDiagnostics(t.uri.toString())), o || r.push(n.getSemanticDiagnostics(t.uri.toString())), s || r.push(n.getSuggestionDiagnostics(t.uri.toString()));
		let c = await Promise.all(r);
		if (!c || t.isDisposed()) return;
		let l = c.reduce((e, t) => t.concat(e), []).filter((e) => (this._defaults.getDiagnosticsOptions().diagnosticCodesToIgnore || []).indexOf(e.code) === -1), u = l.map((e) => e.relatedInformation || []).reduce((e, t) => t.concat(e), []).map((t) => t.file ? e.parse(t.file.fileName) : null);
		await this._libFiles.fetchLibFilesIfNecessary(u), !t.isDisposed() && i.setModelMarkers(t, this._selector, l.map((e) => this._convertDiagnostics(t, e)));
	}
	_convertDiagnostics(e, t) {
		let n = t.start || 0, r = t.length || 1, { lineNumber: i, column: o } = e.getPositionAt(n), { lineNumber: s, column: c } = e.getPositionAt(n + r), l = [];
		return t.reportsUnnecessary && l.push(a.Unnecessary), t.reportsDeprecated && l.push(a.Deprecated), {
			severity: this._tsDiagnosticCategoryToMarkerSeverity(t.category),
			startLineNumber: i,
			startColumn: o,
			endLineNumber: s,
			endColumn: c,
			message: u(t.messageText, "\n"),
	...
	_tsDiagnosticCategoryToMarkerSeverity(e) {
		switch (e) {
			case 1: return r.Error;
			case 3: return r.Info;
			case 0: return r.Warning;
			case 2: return r.Hint;
		}
		return r.Info;
	}
}, h = class e extends f {
	...
	async provideCompletionItems(r, i, a, o) {
		let s = r.getWordUntilPosition(i), c = new n(i.lineNumber, s.startColumn, i.lineNumber, s.endColumn), l = r.uri, u = r.getOffsetAt(i), d = await this._worker(l);
		if (r.isDisposed()) return;
		let f = await d.getCompletionsAtPosition(l.toString(), u);
		if (!(!f || r.isDisposed())) return { suggestions: f.entries.map((a) => {
			let o = c;
			if (a.replacementSpan) {
				let e = r.getPositionAt(a.replacementSpan.start), t = r.getPositionAt(a.replacementSpan.start + a.replacementSpan.length);
				o = new n(e.lineNumber, e.column, t.lineNumber, t.column);
			}
			let s = [];
			return a.kindModifiers !== void 0 && a.kindModifiers.indexOf("deprecated") !== -1 && s.push(t.CompletionItemTag.Deprecated), {
				uri: l,
				position: i,
				offset: u,
				range: o,
	...
	static convertKind(e) {
		switch (e) {
			...
			case C.keyword: return t.CompletionItemKind.Keyword;
			case C.variable:
			case C.localVariable: return t.CompletionItemKind.Variable;
			case C.memberVariable:
			case C.memberGetAccessor:
			case C.memberSetAccessor: return t.CompletionItemKind.Field;
			case C.function:
			case C.memberFunction:
			case C.constructSignature:
			case C.callSignature:
			case C.indexSignature: return t.CompletionItemKind.Function;
			case C.enum: return t.CompletionItemKind.Enum;
			case C.module: return t.CompletionItemKind.Module;
			case C.class: return t.CompletionItemKind.Class;
			case C.interface: return t.CompletionItemKind.Interface;
			case C.warning: return t.CompletionItemKind.File;
		}
		return t.CompletionItemKind.Property;
	}
	static createDocumentationString(e) {
		let t = d(e.documentation);
...
var _ = class e extends f {
	constructor() {
		super(...arguments), this.signatureHelpTriggerCharacters = ["(", ","];
	}
	static _toSignatureHelpTriggerReason(e) {
		switch (e.triggerKind) {
			case t.SignatureHelpTriggerKind.TriggerCharacter: return e.triggerCharacter ? e.isRetrigger ? {
				kind: "retrigger",
			...
			case t.SignatureHelpTriggerKind.ContentChange: return e.isRetrigger ? { kind: "retrigger" } : { kind: "invoked" };
			case t.SignatureHelpTriggerKind.Invoke:
	...
	async provideSignatureHelp(t, n, r, i) {
		...
		return c.items.forEach((e) => {
			...
			t.documentation = { value: d(e.documentation) }, t.label += d(e.prefixDisplayParts), e.parameters.forEach((n, r, i) => {
				let a = d(n.displayParts), o = {
					label: a,
					documentation: { value: d(n.documentation) }
				};
				t.label += a, t.parameters.push(o), r < i.length - 1 && (t.label += d(e.separatorDisplayParts));
			}), t.label += d(e.suffixDisplayParts), l.signatures.push(t);
		}), {
			value: l,
			dispose() {}
		};
	...
	async provideHover(e, t, n) {
		...
		let s = d(o.documentation), c = o.tags ? o.tags.map((e) => g(e)).join("  \n\n") : "", l = d(o.displayParts);
		return {
	...
	async provideDocumentHighlights(e, n, r) {
		let i = e.uri, a = e.getOffsetAt(n), o = await this._worker(i);
		if (e.isDisposed()) return;
		let s = await o.getDocumentHighlights(i.toString(), a, [i.toString()]);
		if (!(!s || e.isDisposed())) return s.flatMap((n) => n.highlightSpans.map((n) => ({
			range: this._textSpanToRange(e, n.textSpan),
			kind: n.kind === "writtenReference" ? t.DocumentHighlightKind.Write : t.DocumentHighlightKind.Text
		})));
	}
}, b = class extends f {
	constructor(e, t) {
	...
	async provideDefinition(t, n, r) {
		let i = t.uri, a = t.getOffsetAt(n), o = await this._worker(i);
		if (t.isDisposed()) return;
		let s = await o.getDefinitionAtPosition(i.toString(), a);
		if (!s || t.isDisposed() || (await this._libFiles.fetchLibFilesIfNecessary(s.map((t) => e.parse(t.fileName))), t.isDisposed())) return;
		let c = [];
		for (let e of s) {
	...
	async provideReferences(t, n, r, i) {
		let a = t.uri, o = t.getOffsetAt(n), s = await this._worker(a);
		if (t.isDisposed()) return;
		let c = await s.getReferencesAtPosition(a.toString(), o);
		if (!c || t.isDisposed() || (await this._libFiles.fetchLibFilesIfNecessary(c.map((t) => e.parse(t.fileName))), t.isDisposed())) return;
		let l = [];
		for (let e of c) {
			let t = this._libFiles.getOrCreateModel(e.fileName);
			t && l.push({
	...
	async provideDocumentSymbols(e, n) {
		let r = e.uri, i = await this._worker(r);
		if (e.isDisposed()) return;
		let a = await i.getNavigationTree(r.toString());
		if (!a || e.isDisposed()) return;
		let o = (n, r) => ({
			name: n.text,
			detail: "",
			kind: w[n.kind] || t.SymbolKind.Variable,
			range: this._textSpanToRange(e, n.spans[0]),
			selectionRange: this._textSpanToRange(e, n.spans[0]),
			tags: [],
			children: n.childItems?.map((e) => o(e, n.text)),
			containerName: r
		});
...
w[C.module] = t.SymbolKind.Module, w[C.class] = t.SymbolKind.Class, w[C.enum] = t.SymbolKind.Enum, w[C.interface] = t.SymbolKind.Interface, w[C.memberFunction] = t.SymbolKind.Method, w[C.memberVariable] = t.SymbolKind.Property, w[C.memberGetAccessor] = t.SymbolKind.Property, w[C.memberSetAccessor] = t.SymbolKind.Property, w[C.variable] = t.SymbolKind.Variable, w[C.const] = t.SymbolKind.Variable, w[C.localVariable] = t.SymbolKind.Variable, w[C.variable] = t.SymbolKind.Variable, w[C.function] = t.SymbolKind.Function, w[C.localFunction] = t.SymbolKind.Function;
var T = class extends f {
	...
	async provideDocumentRangeFormattingEdits(e, t, n, r) {
		...
		if (e.isDisposed()) return;
		let c = await s.getFormattingEditsForRange(i.toString(), a, o, T._convertOptions(n));
	...
	async provideOnTypeFormattingEdits(e, t, n, r, i) {
		...
		if (e.isDisposed()) return;
		let c = await s.getFormattingEditsAfterKeystroke(a.toString(), o, n, T._convertOptions(r));
		if (!(!c || e.isDisposed())) return c.map((t) => this._convertTextChanges(e, t));
	}
}, O = class extends T {
	async provideCodeActions(e, t, n, r) {
		let i = e.uri, a = e.getOffsetAt({
			...
			lineNumber: t.endLineNumber,
			column: t.endColumn
		}), s = T._convertOptions(e.getOptions()), c = n.markers.filter((e) => e.code).map((e) => e.code).map(Number), l = await this._worker(i);
		if (e.isDisposed()) return;
	...
	_convertHintKind(e) {
		switch (e) {
			case "Parameter": return t.InlayHintKind.Parameter;
			case "Type": return t.InlayHintKind.Type;
			default: return t.InlayHintKind.Type;
		}
	}
}, j, M;
function N(e) {
...
function I() {
	return new Promise((e, t) => {
		if (!M) return t("TypeScript not registered!");
		e(M);
	});
}
function L(e, n) {
	let r = [], i = new c(n, e), a = (...e) => i.getLanguageServiceWorker(...e), o = new p(a);
	function s() {
		let { modeConfiguration: i } = e;
		R(r), i.completionItems && r.push(t.registerCompletionItemProvider(n, new h(a))), i.signatureHelp && r.push(t.registerSignatureHelpProvider(n, new _(a))), i.hovers && r.push(t.registerHoverProvider(n, new v(a))), i.documentHighlights && r.push(t.registerDocumentHighlightProvider(n, new y(a))), i.definitions && r.push(t.registerDefinitionProvider(n, new b(o, a))), i.references && r.push(t.registerReferenceProvider(n, new x(o, a))), i.documentSymbols && r.push(t.registerDocumentSymbolProvider(n, new S(a))), i.rename && r.push(t.registerRenameProvider(n, new k(o, a))), i.documentRangeFormattingEdits && r.push(t.registerDocumentRangeFormattingEditProvider(n, new E(a))), i.onTypeFormattingEdits && r.push(t.registerOnTypeFormattingEditProvider(n, new D(a))), i.codeActions && r.push(t.registerCodeActionProvider(n, new O(a))), i.inlayHints && r.push(t.registerInlayHintsProvider(n, new A(a))), i.diagnostics && r.push(new m(o, e, n, a));
	}
	return s(), a;
}
function R(e) {
...
//#endregion
export { f as Adapter, O as CodeActionAdaptor, b as DefinitionAdapter, m as DiagnosticsAdapter, y as DocumentHighlightAdapter, E as FormatAdapter, T as FormatHelper, D as FormatOnTypeAdapter, A as InlayHintsAdapter, C as Kind, p as LibFiles, S as OutlineAdapter, v as QuickInfoAdapter, x as ReferenceAdapter, k as RenameAdapter, _ as SignatureHelpAdapter, h as SuggestAdapter, c as WorkerManager, u as flattenDiagnosticMessageText, F as getJavaScriptWorker, I as getTypeScriptWorker, P as setupJavaScript, N as setupTypeScript };

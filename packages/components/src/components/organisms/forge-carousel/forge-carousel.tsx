...
export function ForgeCarousel(properties: Readonly<CarouselProperties>): MpElement {
  ...
  const indicatorButtons: MpChild[] = slides.map((slideItem, index) => (
    <button
      ...
      className={[
        styles['forge-carousel__indicator'],
        {
          [styles['forge-carousel__indicator--active']]: currentIndex === index,
        },
      ]}
      role="tab"
      type="button"
  ...
  const slideNodes: MpChild[] = slides.map((slideItem, index) => (
    <div
      ...
      className={styles['forge-carousel__slide']}
      role="group"
    >
      <Slot
        name="slide"
      ...
      >
        {slideItem.image ? (
          <img
            alt={slideItem.alt ?? ''}
            className={styles['forge-carousel__image']}
            src={slideItem.image}
          />
        ) : (
          slideItem.content
  ...
  return (
    ...
    >
      <div
        className={styles['forge-carousel__viewport']}
      ...
      >
        <div
          className={styles['forge-carousel__track']}
      ...
      {controls && slideCount > 1 ? (
        <button
          ...
          className={[styles['forge-carousel__control'], styles['forge-carousel__control--prev']]}
      ...
      {controls && slideCount > 1 ? (
        <button
          ...
          className={[styles['forge-carousel__control'], styles['forge-carousel__control--next']]}
      ...
      {indicators && slideCount > 1 ? (
        <div
          className={styles['forge-carousel__indicators']}
          role="tablist"
        >
          {indicatorButtons}
        </div>
      ...
      {showAutoplayToggle ? (
        <button
          ...
          className={styles['forge-carousel__autoplay']}
          type="button"
          onClick={() => setUserPaused(!userPaused)}
        >
          {userPaused ? <ForgeIconPlay size="sm" /> : <ForgeIconPause size="sm" />}
        </button>
      ) : undefined}
    </section>
  );

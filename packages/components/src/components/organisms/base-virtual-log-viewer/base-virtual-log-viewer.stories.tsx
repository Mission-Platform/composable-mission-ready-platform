...
import { VirtualLogViewer } from '@mission-platform/components';
...
/**
 * `VirtualLogViewer` is the write-once `BaseVirtualLogViewer` component of
 * `@mission-platform/components`. This single neutral story renders on the
 * framework selected by `STORYBOOK_FRAMEWORK`.
 */

const meta = {
  title: 'Organisms/Data/BaseVirtualLogViewer',
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `VirtualLogViewer` — authored once in the neutral JSX dialect and shipped to all supported frameworks. It virtual-scrolls a large log, with per-level colouring, an optional timestamp/level column, a substring `filter`, follow-tail, and an `onSelect` callback.',
      },
    },
  },
  argTypes: {

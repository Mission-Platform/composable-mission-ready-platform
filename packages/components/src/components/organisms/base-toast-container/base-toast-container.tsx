...
import { BaseToast } from '../../molecules/base-toast';
 ...
 * `:global(.base-toast-*)` rules — matching the `scoped` `<style>` of the
 ...
 * classes are applied to DOM-element children — `BaseToast` being a component,
  ...
  return (
    ...
    >
      ...
      >
        <TransitionGroup
          name="base-toast"
          enterFromClass={styles['base-toast-enter-from']}
          enterActiveClass={styles['base-toast-enter-active']}
          leaveActiveClass={styles['base-toast-leave-active']}
          leaveToClass={styles['base-toast-leave-to']}
        >
          {ordered.map((toast) => (
            <BaseToast
              key={toast.id}

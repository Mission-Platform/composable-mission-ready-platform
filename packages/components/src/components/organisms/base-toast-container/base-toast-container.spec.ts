...
import { clearToasts, dismissToast, getToastsSnapshot, showToast, subscribeToasts } from '../../../stores/toast-store/toast-store';
import { expectSsrParity, renderReactSsr, renderVueSsr } from '../../../test-utils/ssr-parity';

import { BaseToastContainer } from './base-toast-container';

/**
 * Exercises the **neutral** `BaseToastContainer` + shared `toast-store`, the
 * write-once counterpart of the Vue package's `useToast` store +
 * `BaseToastContainer.vue`. Covers cross-framework SSR DOM parity (empty +
 * seeded) and the store's `show`/`dismiss`/`clear`/auto-dismiss behaviour.
 */
describe('BaseToastContainer authors the same component for React and Vue', () => {
  beforeEach(() => {
    clearToasts();
  });

  it('renders an empty positioned region identically on both frameworks', async () => {
    const { html } = await expectSsrParity(BaseToastContainer, { teleport: false, position: 'bottom-center' });
    expect(html).toContain('base-toast-container');
    expect(html).toContain('base-toast-container--bottom-center');
  ...
  it('renders the active toasts from the shared store on both frameworks', async () => {
    ...
    const react = renderReactSsr(BaseToastContainer, { teleport: false });
    const vue = await renderVueSsr(BaseToastContainer, { teleport: false });

    for (const html of [react, vue]) {
      expect(html).toContain('base-toast--success');
      expect(html).toContain('Saved');
      expect(html).toContain('Heads up.');
      expect(html).toContain('base-toast--info');
    }
  });
});

...
import type { TreeViewLabelScope, TreeViewNode } from './base-tree-view';
...
/**
 * `BaseTreeViewItem` — a single, self-recursive tree row extracted from
 * {@link BaseTreeView}'s former `renderNode` closure into its own focused
 ...
 * `BaseTreeViewItem`s — so the recursion compiles to a native recursive
 * child-component tag on every target rather than a local render helper.
 *
 * All behavioural state lives on the root {@link BaseTreeView}: this component is
 ...
 */
export function BaseTreeViewItem(properties: Readonly<TreeViewItemProperties>): MpElement {

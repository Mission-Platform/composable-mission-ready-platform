 ...
 * through the composed neutral {@link BaseTypography}; sort state is held with
 * the neutral {@link useState}/{@link useMemo} hooks. It owns its styling
 * through the co-located CSS Module `base-table.module.scss`.
 *
 * The original Vue SFC was generic over the row type, composed
 * `BaseTableHead`/`BaseTableBody` sub-components, used a `sort` emit, and exposed
...
export function BaseTable(properties: Readonly<TableProperties>): MpElement {
  ...
  const tableClass = classNames(styles['base-table'], styles[`base-table--${variant}`], {
    [styles['base-table--striped']]: striped,
    [styles['base-table--bordered']]: bordered,
    [styles['base-table--hoverable']]: hoverable,
  });

  const headCells = columns.map((column) => {
    const isActive = sortKey === column.key;
    const thClass = classNames(
      styles['base-table__th'],
      { [styles[`base-table__th--align-${column.align ?? 'left'}`]]: true },
      { [styles['base-table__th--sortable']]: Boolean(column.sortable) },
    );
    return (
      <th
        className={thClass}
      ...
      : sortedRows.map((row) => (
          <tr className={styles['base-table__row']}>
            {columns.map((column) => {
              ...
              const tdClass = classNames(styles['base-table__td'], {

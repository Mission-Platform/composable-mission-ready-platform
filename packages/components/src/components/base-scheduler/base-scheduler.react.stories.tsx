...
/**
 * `Scheduler` is the **React** build of the write-once `BaseScheduler` in
 * `@mission-platform/components`. A toolbar sits above a time grid (day / 3-day /
...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `Scheduler` — authored once in the neutral JSX dialect and shipped to both React (this story, via `@mission-platform/components/react`) and Vue 3 (`@mission-platform/components/vue`). A toolbar sits above a time grid (day / 3-day / week), a month grid, or a year grid; events are RFC 5545 `VEvent`s and the heavy logic comes from the shared `@mission-platform/scheduler-core`. Styling comes from the co-located `base-scheduler.module.scss`.',
      },
    },
  },
  render: (arguments_) => {
    const [events, setEvents] = useState<VEvent[]>(arguments_.modelValue ?? []);
    return (
      <Scheduler
        {...arguments_}
        modelValue={events}
        onUpdateModelValue={setEvents}

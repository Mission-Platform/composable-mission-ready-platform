...
import { ForgeLocationInput } from '@mission-platform/components';

import type { Meta, StoryObj } from '@mission-platform/storybook-framework';

/**
 * `ForgeLocationInput` is the write-once component of `@mission-platform/components`.
 ...
 */
const meta = {
  title: 'Molecules/Forms/ForgeLocationInput',
  component: ForgeLocationInput,
  tags: ['autodocs'],
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `ForgeLocationInput` — authored once in the neutral JSX dialect and shipped to all supported frameworks. It composes the migrated `ForgeSelect`/`ForgeInput`/`ForgeTypography` primitives and captures a latitude/longitude point in DD, DM, or DMS form; the canonical model always carries signed decimal-degree `lat`/`lng`. The local text buffers become neutral `useState` resynced via `useEffect`, and the `v-model` + `change` emits become the `onUpdateModelValue`/`onChange` callback props. The conversion logic ships with the package as the co-located `location.ts`.',
      },
    },
  },
  argTypes: {
...
} satisfies Meta<typeof ForgeLocationInput>;

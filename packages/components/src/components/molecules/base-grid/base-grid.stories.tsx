...
import { Grid } from '@mission-platform/components';
...
/**
 * `Grid` is the write-once component of `@mission-platform/components`.
 ...
 */
const meta = {
  title: 'Molecules/Layout/BaseGrid',
  component: Grid,
  tags: ['autodocs'],
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `Grid` — authored once in the neutral JSX dialect and shipped to all supported frameworks. It arranges its default-slot children into a `rows` × `cols` CSS Grid with a configurable `gap`. The demo cell styling comes from the co-located `base-grid.module.scss` CSS Module.',
      },
    },
  },
  argTypes: {
  ...
  render: (arguments_) => {
    ...
    return (
      <Grid {...arguments_} class={styles['grid-demo-container']}>
        {Array.from({ length: count }, (_unused, index) => (
          <div key={index} class={styles['grid-demo-cell']}>
            {index + 1}
          </div>
        ))}
      </Grid>

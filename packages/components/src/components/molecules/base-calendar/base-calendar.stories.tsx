...
import { Calendar } from '@mission-platform/components';

import type { Meta, StoryObj } from '@mission-platform/storybook-framework';

/**
 * `Calendar` is the write-once component of `@mission-platform/components`.
 ...
 */
const meta = {
  title: 'Molecules/Forms/BaseCalendar',
  component: Calendar,
  tags: ['autodocs'],
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `Calendar` — authored once in the neutral JSX dialect and shipped to all supported frameworks. It renders a Sunday-first month grid of selectable days (honouring `min`/`max` and `disabledDates`), with IANA-timezone aware rendering via `luxon`. The `ref` view state becomes `useState`, the `computed` grid becomes `useMemo`, the external-sync `watch` becomes a `useEffect`, the chevron icons become text glyphs, and the `v-model` + `change` emits become the `onUpdateModelValue`/`onChange` callback props. Styling comes from the co-located `base-calendar.module.scss`.',
      },
    },
  },
  argTypes: {

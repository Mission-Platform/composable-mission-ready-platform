...
import { NavbarItem } from '@mission-platform/components';
...
/**
 * `NavbarItem` is the write-once `BaseNavbarItem` component of `@mission-platform/components`. With no `dropdownItems` it renders an `<a>`
 ...
 */
const meta = {
  title: 'Molecules/Navigation/BaseNavbarItem',
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `NavbarItem` — authored once in the neutral JSX dialect and shipped to all supported frameworks. With no `dropdownItems` it resolves to `<a>` or `<button>`; with `dropdownItems` it renders a dropdown trigger + `role="menu"` panel. Styling comes from the co-located `base-navbar-item.module.scss`.',
      },
    },
  },
  argTypes: {

...
/**
 * `MarkdownInline` — the recursive inline half of the `BaseMarkdown` renderer.
 ...
 * `BaseTreeViewItem` so it compiles to a native recursive tag on every target.
 */
export function MarkdownInline(properties: Readonly<MarkdownInlineProperties>): MpElement {
  ...
  return type === 'strong' ? (
    ...
    <img
      alt={image.text}
      className={styles['base-markdown__image']}
      src={image.href}

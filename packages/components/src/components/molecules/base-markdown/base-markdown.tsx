...
/**
 * `BaseMarkdown` — a read-only Markdown renderer authored once in the neutral
 * JSX dialect and compiled straight to React or Vue by
 * `@mission-platform/vite-plugin-forge`.
 *
 * Unlike the editor `BaseMarkdownInput` (whose live preview is injected as raw
 * `innerHTML`), this component renders the Markdown **as real components**:
 * fenced code becomes `BaseCodeBlock`, GFM tables become `BaseTable`, and every
 * heading / paragraph / inline run is styled through `BaseTypography` so the
 ...
 * `BaseTreeView`), so there is no `v-html` and the tree is SSR-safe. It owns its
 * styling through the co-located CSS Module `base-markdown.module.scss`.
 */
export function BaseMarkdown(properties: Readonly<MarkdownProperties>): MpElement {
  ...
  return (
    <div className={[styles['base-markdown'], sizeStyles[`base-size--${size}`]]}>

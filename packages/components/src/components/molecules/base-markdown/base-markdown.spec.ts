...
/**
 * Exercises the **neutral** `BaseMarkdown` renderer authored in this package,
 ...
 * (`BaseTypography`, `BaseCodeBlock`, `BaseTable`) — there is no `v-html`, so the
 * SSR markup already contains the composed component shells.
 */
const ReactMarkdown = toReactComponent(BaseMarkdown, 'Markdown');
const VueMarkdown = toVueComponent(BaseMarkdown, 'Markdown');
...
describe('BaseMarkdown authors the same renderer for React and Vue', () => {
  it('renders headings, inline emphasis, a code block and a table on both frameworks', async () => {
    ...
    for (const html of [react, vue]) {
      ...
      // The fenced code becomes a BaseCodeBlock. Its highlighted body is
      // injected client-side, so the SSR shell carries the language + copy affordance.
      expect(html).toContain('base-code-block');
      expect(html).toContain('typescript');
      expect(html).toContain('Copy');
      // The GFM table becomes a BaseTable with the header label.

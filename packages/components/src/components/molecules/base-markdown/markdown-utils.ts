...
import type { CodeBlockLanguage } from '../../atoms/base-code-block';
import type { TypographyVariant } from '../../atoms/base-typography';

/** Size token — canonical 2xs → 2xl scale. */
export type MarkdownSize = '2xs' | 'xs' | 'sm' | 'md' | 'lg' | 'xl' | '2xl';

/** Languages registered with the bundled `highlight.js` in `BaseCodeBlock`. */
...
/** Maps a heading depth (`1`–`6`) onto the matching `BaseTypography` variant. */
...
/** Build the `BaseTable` column set from a GFM table token. */
...
/** Build the `BaseTable` row records from a GFM table token. */

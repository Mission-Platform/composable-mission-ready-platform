import { Accordion } from '@mission-platform/components';

import type { Meta, StoryObj } from '@mission-platform/storybook-framework';

/**
 * `Accordion` is the write-once component of `@mission-platform/components`.
 ...
 */
const meta = {
  title: 'Molecules/Display/BaseAccordion',
  component: Accordion,
  tags: ['autodocs'],
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `Accordion` — authored once in the neutral JSX dialect and shipped to all supported frameworks. The original `BaseAccordion`/`BaseAccordionItem` pair shared open state through `provide`/`inject`; the neutral version flattens them into one component driven by an `items` array with internal open state (like the migrated `BaseTabs`). Set `exclusive` to `false` to allow several items open at once. The icon chevron becomes a CSS-rotated `▾` glyph; styling comes from the co-located `base-accordion.module.scss`.',
      },
    },
  },
  argTypes: {

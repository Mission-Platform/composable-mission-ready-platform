...
import styles from './forge-markdown-input.module.scss';
...
/**
 * `ForgeMarkdownInput` — a Markdown editor with a live preview authored once in
 ...
 * `forge-markdown-input.module.scss`.
 *
 * Substitutions from the original Vue SFC: the `v-html` preview becomes a
 * **`useRef` host + `useEffect` `innerHTML`** assignment (the `ForgeWindowPopout`
 ...
 */
export function ForgeMarkdownInput(properties: Readonly<MarkdownInputProperties>): MpElement {
  const {
    modelValue = '',
    rows = 6,
    size = 'md',

...
/**
 * `ForgeMarkdown` is the write-once `ForgeMarkdown` renderer of `@mission-platform/components`.
 * It renders a ForgeMarkdown `source` **as real components** — fenced code becomes
 * `ForgeCodeBlock`, GFM tables become `ForgeTable`, and every heading / paragraph /
 * inline run is styled through `ForgeTypography` — driven by the `marked` token
...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `ForgeMarkdown` renderer — authored once in the neutral JSX dialect and shipped to all supported frameworks. It maps the `marked` token stream onto real components (`ForgeCodeBlock`, `ForgeTable`, `ForgeTypography`) so rendered documents match the platform type scale. Styling comes from the co-located `forge-markdown.module.scss`.',
      },
    },
  },
  argTypes: {

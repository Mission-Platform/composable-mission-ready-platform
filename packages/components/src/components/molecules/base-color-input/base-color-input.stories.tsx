...
import { ColorInput } from '@mission-platform/components';

import type { Meta, StoryObj } from '@mission-platform/storybook-framework';

/**
 * `ColorInput` is the write-once component of `@mission-platform/components`.
 ...
 */
const meta = {
  title: 'Molecules/Forms/BaseColorInput',
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `ColorInput` — authored once in the neutral JSX dialect and shipped to all supported frameworks. A native `<input type="color">` swatch is paired with a hex text field; the value is controlled via `modelValue`, and the `v-model` + `change` emit become the `onUpdateModelValue`/`onChange` callback props. Styling comes from the co-located `base-color-input.module.scss`.',
      },
    },
  },
  argTypes: {

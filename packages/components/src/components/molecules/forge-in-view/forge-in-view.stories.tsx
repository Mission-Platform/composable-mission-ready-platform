...
/**
 * `ForgeInView` is the write-once component of `@mission-platform/components`.
 ...
 */
const meta = {
  title: 'Molecules/Layout/ForgeInView',
  component: ForgeInView,
  tags: ['autodocs'],
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `ForgeInView` — authored once in the neutral JSX dialect and shipped to all supported frameworks. It demonstrates the neutral hooks driving an `IntersectionObserver`. The demo card styling comes from the co-located `forge-in-view.module.scss` CSS Module (consumed here through its hashed class map).',
      },
    },
  },
  argTypes: {
  ...
  render: (arguments_) => (
    <ForgeInView {...arguments_}>
      <div class={styles['in-view-demo']}>Revealed when in view</div>
    </ForgeInView>
  ),
} satisfies Meta<typeof ForgeInView>;

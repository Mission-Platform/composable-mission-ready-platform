...
import type { VirtualTableColumn } from './BaseVirtualTable.vue'
...
function makeMissions(count: number): Mission[] {
  return Array.from({ length: count }, (_, i) => ({
    ...
    name: `Operation ${['Nightfall', 'Ironclad', 'Vortex', 'Eclipse', 'Harbinger', 'Cascade'][i % 6]} ${Math.floor(i / 6) + 1}`,
    unit: UNITS[i % UNITS.length],
    status: STATUSES[i % STATUSES.length],
    personnel: 4 + (i % 20),
    startedAt: new Date(Date.now() - i * 3_600_000).toISOString().slice(0, 16).replace('T', ' '),
...
} satisfies Meta<typeof BaseVirtualTable>
...
export const Default: Story = {
  render: () => ({
    components: { BaseVirtualTable, BaseBadge },
    setup() {

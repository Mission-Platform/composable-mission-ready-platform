...
import { ForgeRating } from './forge-rating';

/**
 * Exercises the **neutral** `ForgeRating` authored in this package, rendering it
 ...
 * count.
 */
const ReactRating = toReactComponent(ForgeRating, 'Rating');
const VueRating = toVueComponent(ForgeRating, 'Rating');

describe('ForgeRating authors the same component for React and Vue', () => {
  it('renders an interactive slider with the value on both frameworks', async () => {
    ...
    for (const html of [react, vue]) {
      ...
      // Five `ForgeIconStar` icons are rendered (each as an empty + filled layer).
      expect(html.split('forge-icon-star').length - 1).toBe(10);
    }
  });

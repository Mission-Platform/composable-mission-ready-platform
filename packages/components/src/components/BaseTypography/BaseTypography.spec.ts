...
describe('BaseTypography', () => {
  ...
  it('applies default classes', () => {
    const wrapper = mount(BaseTypography)
    const el = wrapper.find('.base-typography')
    expect(el.classes()).toContain('base-typography')
    expect(el.classes()).toContain('base-typography--body-md')
    expect(el.classes()).toContain('base-typography--color-primary')
  })
  ...
  it('renders heading variants with the correct HTML tag', () => {
    ...
    for (const [variant, expectedTag] of cases) {
      const wrapper = mount(BaseTypography, { props: { variant: variant as any } })
  ...
  it('does not apply weight class when weight prop is not set', () => {
    const wrapper = mount(BaseTypography)
    expect(wrapper.find('.base-typography').classes().some((c) => c.startsWith('base-typography--weight-'))).toBe(false)
  })
  ...
  it('does not apply color class when color is "inherit"', () => {
    ...
    expect(wrapper.find('.base-typography').classes().some((c) => c.startsWith('base-typography--color-'))).toBe(false)
  })
  ...
  it('does not apply align class when align prop is not set', () => {
    const wrapper = mount(BaseTypography)
    expect(wrapper.find('.base-typography').classes().some((c) => c.startsWith('base-typography--align-'))).toBe(false)
  })

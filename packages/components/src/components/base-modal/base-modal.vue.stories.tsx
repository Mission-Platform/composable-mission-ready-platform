...
import { Button, Dropdown, Modal, Select, Stack, Tooltip, Typography } from '@mission-platform/components/vue';
...
export const StackedModals: Story = {
  render: () => ({
    ...
    template: `
      <Button variant="primary" @click="outerOpen = true">Open first modal</Button>
      <Modal :open="outerOpen" title="First modal" @update-open="outerOpen = $event" @close="outerOpen = false">
        <Stack gap="sm">
          <Typography variant="body-md">
            This is the first modal. Open a second modal on top of it — it stacks above via the native top layer.
          </Typography>
          <Button variant="secondary" @click="innerOpen = true">Open second modal</Button>
        </Stack>
        <Modal :open="innerOpen" size="sm" title="Second modal" @update-open="innerOpen = $event" @close="innerOpen = false">
          <Typography variant="body-md">
...
export const PopupsAboveModal: Story = {
  render: () => ({
    components: { Modal, Button, Stack, Typography, Select, Dropdown, Tooltip },
    setup() {
      const open = ref(false);
      const fruit = ref<string | number>('');
      ...
      return { open, fruit, dropdownOpen, options };
    },
    template: `
      <Button variant="primary" @click="open = true">Open modal with popups</Button>
      <Modal :open="open" title="Popups above the modal" @update-open="open = $event" @close="open = false">
        <Stack gap="md">
          <Typography variant="body-md">
          ...
          <Select
            ...
            @update-model-value="fruit = $event"
          />
          <Dropdown :open="dropdownOpen" @update-open="dropdownOpen = $event">
            <template #trigger>
              <Button variant="secondary" @click="dropdownOpen = !dropdownOpen">Open dropdown</Button>
            </template>
            <Stack gap="xs">
...
export const PopupsInStackedModals: Story = {
  render: () => ({
    components: { Modal, Button, Stack, Typography, Select },
    setup() {
      const outerOpen = ref(false);
      const innerOpen = ref(false);
      const fruit = ref<string | number>('');
      ...
      return { outerOpen, innerOpen, fruit, options };
    },
    template: `
      <Button variant="primary" @click="outerOpen = true">Open first modal</Button>
      <Modal :open="outerOpen" title="First modal" @update-open="outerOpen = $event" @close="outerOpen = false">

...
export function BaseMenu(properties: Readonly<MenuProperties>): MpElement {
  ...
  const renderIcon = (item: MenuNode): MpElement | undefined =>
    item.icon ? (
      <span
        aria-hidden="true"
        className={styles['base-menu__icon']}
      >
        {item.icon}
      </span>
    ) : undefined;
      ...
      return (
        ...
        >
          {item.href && !hasChildren ? (
            <a
              href={item.href}
              className={styles['base-menu__link']}
              role="menuitem"
            >
              {renderIcon(item)}
              <span className={styles['base-menu__label']}>{item.label}</span>
            </a>
          ) : (
            <button
            ...
            >
              ...
              {hasChildren ? (
                <span
                  aria-hidden="true"
                  className={styles['base-menu__chevron']}
                >
                  <IconChevron
  ...
  return (
    <nav
      ...
      className={[styles['base-menu'], styles[`base-menu--${orientation}`], sizeStyles[`base-size--${size}`]]}
    >
      <menu
        aria-orientation={orientation}
        className={styles['base-menu__list']}
        role="menubar"
      >

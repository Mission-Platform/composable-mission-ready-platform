...
/**
 * `Typography` is the **React** build of the write-once `BaseTypography` in
 * `@mission-platform/components`. It renders its default-slot content in the
 ...
 * `verticalAlign`, `truncate`, and `truncatePopup` modifiers. Authored once in
...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `Typography` — authored once in the neutral JSX dialect and shipped to both React (this story, via `@mission-platform/components/react`) and Vue 3 (`@mission-platform/components/vue`). It renders its default-slot content in the semantic tag for the chosen `variant` (overridable with `as`), applying the variant type-scale plus optional modifiers. Styling comes from the co-located `base-typography.module.scss`.',
      },
    },
  },
  argTypes: {

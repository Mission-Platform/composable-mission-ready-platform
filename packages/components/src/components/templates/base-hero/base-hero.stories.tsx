...
import { Hero } from '@mission-platform/components';

import type { Meta, StoryObj } from '@mission-platform/storybook-framework';

/**
 * `Hero` is the write-once component of `@mission-platform/components`.
 ...
 */
const meta = {
  title: 'Templates/Layout/BaseHero',
  component: Hero,
  tags: ['autodocs'],
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `Hero` — authored once in the neutral JSX dialect and shipped to all supported frameworks. It composes the write-once `Typography` for the eyebrow/title/subtitle and renders free-form body content (default slot), an optional `media` background slot, and an `actions` slot. Styling comes from the co-located `base-hero.module.scss`.',
      },
    },
  },
  argTypes: {
  ...
  render: (arguments_) => (
    <Hero {...arguments_}>Free-form body content rendered in the hero content column.</Hero>
  ),
...
export const WithActions: Story = {
  render: (arguments_) => (
    <Hero
      ...
      actions={
        <>
          <button type="button" style={{ padding: 'var(--mp-spacing-2) var(--mp-spacing-4)' }}>
            Get started
          </button>
          <button type="button" style={{ padding: 'var(--mp-spacing-2) var(--mp-spacing-4)' }}>
            Learn more
      ...
      media={
        <div
          style={{
            ...
            height: '100%',
            background:
              'linear-gradient(135deg, var(--mp-color-primary-default), var(--mp-color-info-default))',
          }}
        />
      }
    >

<script setup lang="ts">
  ...
  // ── Time state ───────────────────────────────────────────────────────────
  const startH = ref(0); const startM = ref(0); const startS = ref(0)
  const endH = ref(0); const endM = ref(0); const endS = ref(0)
  ...
  watch(() => props.modelValue, (val) => {
    ...
    if (val?.start) {
      ...
      startDate.value = p.date; startH.value = p.h; startM.value = p.m; startS.value = p.s
    }
    if (val?.end) {
      const p = parseDateTime(val.end)
      endDate.value = p.date; endH.value = p.h; endM.value = p.m; endS.value = p.s
    }
  }, { immediate: true })
  ...
  function setStartH(n: number) { startH.value = clamp(n, 0, 23); emitValue() }
  function setStartM(n: number) { startM.value = clamp(n, 0, 59); emitValue() }
  function setStartS(n: number) { startS.value = clamp(n, 0, 59); emitValue() }
  function setEndH(n: number) { endH.value = clamp(n, 0, 23); emitValue() }
  function setEndM(n: number) { endM.value = clamp(n, 0, 59); emitValue() }
  function setEndS(n: number) { endS.value = clamp(n, 0, 59); emitValue() }
  ...
  function effectiveEndDate() { return endDate.value || hoverDate.value || null }
  ...
  function prevMonth() {
    if (leftMonth.value === 0) { leftYear.value--; leftMonth.value = 11 } else leftMonth.value--
  }

  function nextMonth() {
    if (leftMonth.value === 11) { leftYear.value++; leftMonth.value = 0 } else leftMonth.value++
  }
  ...
  function toggleOpen() {
    ...
    if (!open.value) {
      ...
      if (d) { leftYear.value = d.getFullYear(); leftMonth.value = d.getMonth() }
    }
...
<template>
  <div :class="['base-dtr', `base-dtr--${size}`, { 'base-dtr--error': !!error, 'base-dtr--disabled': disabled }]">
    <label v-if="label" :for="resolvedId" :class="['base-dtr__label', { 'base-dtr__label--hidden': labelHidden }]">
      <BaseTypography variant="label" as="span" color="primary">{{ label }}</BaseTypography>
      <span v-if="required" class="base-dtr__required" aria-hidden="true">*</span>
    </label>
    ...
    >
      ...
      <span class="base-dtr__icon" aria-hidden="true">
    ...
    >
      ...
      <div class="base-dtr__tz-row">
        <BaseTypography variant="caption" as="span" color="secondary">Timezone:</BaseTypography>
        <div class="base-dtr__tz-toggle" role="group" aria-label="Timezone selection">
          <button
            type="button"
      ...
      <div class="base-dtr__phase-hint">
        <BaseTypography variant="caption" as="span" color="secondary">{{ phaseLabel }}</BaseTypography>
      </div>
        ...
        <div class="base-dtr__cal-panels">
          ...
          <div class="base-dtr__cal-panel">
            <div class="base-dtr__cal-header">
              <button type="button" class="base-dtr__nav-btn" @click.stop="prevMonth" aria-label="Previous month">
                <IconChevron size="xs" direction="left" />
              </button>
              <BaseTypography variant="label" as="span" color="primary">{{ leftLabel }}</BaseTypography>
              <span style="width: 28px;" />
            </div>
            <div class="base-dtr__cal-grid">
              <span v-for="d in DAYS" :key="`ld-${d}`" class="base-dtr__weekday">{{ d }}</span>
              <button
          ...
          <div class="base-dtr__cal-panel">
            <div class="base-dtr__cal-header">
              <span style="width: 28px;" />
              <BaseTypography variant="label" as="span" color="primary">{{ rightLabel }}</BaseTypography>
              <button type="button" class="base-dtr__nav-btn" @click.stop="nextMonth" aria-label="Next month">
                <IconChevron size="xs" direction="right" />
              </button>
            </div>
            <div class="base-dtr__cal-grid">
              <span v-for="d in DAYS" :key="`rd-${d}`" class="base-dtr__weekday">{{ d }}</span>
              <button
      ...
      <template v-if="showStartTime">
        <div class="base-dtr__time-section">
          <BaseTypography variant="label" as="div" color="primary" class="base-dtr__time-date-label">
          ...
          <div class="base-dtr__time-columns">
            <div class="base-dtr__time-col">
              ...
              <div class="base-dtr__time-scroll">
                <button v-for="h in hours" :key="`sh-${h}`" type="button" :class="['base-dtr__unit-btn', { 'base-dtr__unit-btn--active': startH === h }]" @click.stop="setStartH(h)">{{ pad(h) }}</button>
              </div>
            </div>
            ...
            <div class="base-dtr__time-col">
              ...
              <div class="base-dtr__time-scroll">
                <button v-for="m in minutes" :key="`sm-${m}`" type="button" :class="['base-dtr__unit-btn', { 'base-dtr__unit-btn--active': startM === m }]" @click.stop="setStartM(m)">{{ pad(m) }}</button>
              </div>
            </div>
            <template v-if="showSeconds">
              ...
              <div class="base-dtr__time-col">
                ...
                <div class="base-dtr__time-scroll">
                  <button v-for="s in seconds" :key="`ss-${s}`" type="button" :class="['base-dtr__unit-btn', { 'base-dtr__unit-btn--active': startS === s }]" @click.stop="setStartS(s)">{{ pad(s) }}</button>
                </div>
              </div>
            </template>
          </div>
          <div class="base-dtr__time-footer">
            <button type="button" class="base-dtr__next-btn" @click.stop="selectingPhase = 'end-time'">Next: End time →</button>
          </div>
        </div>
      </template>
        ...
        <div class="base-dtr__time-section">
          ...
          <div class="base-dtr__time-columns">
            <div class="base-dtr__time-col">
              <div class="base-dtr__time-col-header">HH</div>
              <div class="base-dtr__time-scroll">
                <button v-for="h in hours" :key="`eh-${h}`" type="button" :class="['base-dtr__unit-btn', { 'base-dtr__unit-btn--active': endH === h }]" @click.stop="setEndH(h)">{{ pad(h) }}</button>
              </div>
            </div>
            <span class="base-dtr__time-sep">:</span>
            <div class="base-dtr__time-col">
              <div class="base-dtr__time-col-header">MM</div>
              <div class="base-dtr__time-scroll">
                <button v-for="m in minutes" :key="`em-${m}`" type="button" :class="['base-dtr__unit-btn', { 'base-dtr__unit-btn--active': endM === m }]" @click.stop="setEndM(m)">{{ pad(m) }}</button>
              </div>
            </div>
            <template v-if="showSeconds">
              <span class="base-dtr__time-sep">:</span>
              <div class="base-dtr__time-col">
                <div class="base-dtr__time-col-header">SS</div>
                <div class="base-dtr__time-scroll">
                  <button v-for="s in seconds" :key="`es-${s}`" type="button" :class="['base-dtr__unit-btn', { 'base-dtr__unit-btn--active': endS === s }]" @click.stop="setEndS(s)">{{ pad(s) }}</button>
                </div>
              </div>
            </template>
          </div>
          <div class="base-dtr__time-footer">
            <button type="button" class="base-dtr__back-btn" @click.stop="selectingPhase = 'start-time'">← Back</button>
            <button type="button" class="base-dtr__done-btn" @click.stop="emitValue(); open = false">Done</button>
          </div>
        </div>
      </template>
    </div>

    <BaseTypography v-if="error" :id="`${resolvedId}-error`" variant="caption" as="p" color="inherit" class="base-dtr__error" role="alert">{{ error }}</BaseTypography>
    <BaseTypography v-else-if="hint" :id="`${resolvedId}-hint`" variant="caption" as="p" color="secondary" class="base-dtr__hint">{{ hint }}</BaseTypography>
  </div>
</template>
...
<style scoped lang="scss">
  ...
  .base-dtr {
    ...
    &__required { color: var(--mp-color-danger-default); margin-left: 2px; }
    ...
    &__trigger {
      ...
      &:focus { outline: none; border-color: var(--mp-color-border-focus); box-shadow: var(--mp-shadow-focus-primary); }
    }
    ...
    &__value {
      ...
      &--placeholder { color: var(--mp-color-text-tertiary); }
    }

    &__icon { color: var(--mp-color-text-secondary); display: flex; align-items: center; flex-shrink: 0; }

    // Sizes
    &--sm .base-dtr__trigger {
      padding: var(--mp-spacing-1) var(--mp-spacing-2);
      .base-dtr__value { font-size: var(--mp-font-size-sm); }
    }

    &--md .base-dtr__trigger {
      padding: var(--mp-spacing-2) var(--mp-spacing-3);
      .base-dtr__value { font-size: var(--mp-font-size-md); }
    }

    &--lg .base-dtr__trigger {
      padding: var(--mp-spacing-3) var(--mp-spacing-4);
      .base-dtr__value { font-size: var(--mp-font-size-lg); }
    }

    &--error .base-dtr__trigger { border-color: var(--mp-color-danger-default); &:focus { box-shadow: var(--mp-shadow-focus-danger); } }

    &--disabled {
      opacity: 0.5; pointer-events: none;
      .base-dtr__trigger { background-color: var(--mp-color-bg-muted); cursor: not-allowed; }
    }

    &__error { color: var(--mp-color-danger-text); margin: 0; }
    &__hint { margin: 0; }
    ...
    &__tz-btn {
      ...
      &:hover:not(&--active) { background-color: var(--mp-color-bg-muted); }
      ...
      &:focus-visible { outline: 2px solid var(--mp-color-border-focus); outline-offset: 2px; }
    }

    &__phase-hint { text-align: center; margin-bottom: var(--mp-spacing-2); }

    // Calendar
    &__cal-panels { display: flex; gap: var(--mp-spacing-4); }

    &__cal-panel { min-width: 252px; }

    &__cal-sep { width: 1px; background: var(--mp-color-border-default); align-self: stretch; }
    ...
    &__nav-btn {
      ...
      &:hover { background-color: var(--mp-color-bg-muted); }
      &:focus-visible { outline: 2px solid var(--mp-color-border-focus); outline-offset: 2px; }
    }

    &__cal-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 2px; }
    ...
    &__day {
      ...
      &:hover:not(:disabled):not(.base-dtr__day--range-start):not(.base-dtr__day--range-end) {
      ...
      &--range-start { border-radius: var(--mp-radius-sm) 0 0 var(--mp-radius-sm); }
      &--range-end { border-radius: 0 var(--mp-radius-sm) var(--mp-radius-sm) 0; }
    ...
    &__time-date-label { margin-bottom: var(--mp-spacing-1); }

    &__time-columns { display: flex; align-items: flex-start; gap: var(--mp-spacing-1); }
    ...
    &__time-col { display: flex; flex-direction: column; min-width: 52px; }
    ...
    &__time-scroll {
      ...
      &::-webkit-scrollbar-thumb { background: var(--mp-color-border-default); border-radius: 2px; }
    }
    ...
    &__next-btn {
      ...
      &:hover { opacity: 0.9; background-color: var(--mp-color-primary-default); }
    }

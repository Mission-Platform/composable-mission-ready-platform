<script setup lang="ts">
  ...
  function setH(n: number) { localH.value = clamp(n, 0, 23); emitValue() }
  function setM(n: number) { localM.value = clamp(n, 0, 59); emitValue() }
  function setS(n: number) { localS.value = clamp(n, 0, 59); emitValue() }
...
<template>
  <div :class="['base-time-input', `base-time-input--${size}`, { 'base-time-input--error': !!error, 'base-time-input--disabled': disabled }]">
    <label v-if="label" :for="resolvedId" :class="['base-time-input__label', { 'base-time-input__label--hidden': labelHidden }]">
      <BaseTypography variant="label" as="span" color="primary">{{ label }}</BaseTypography>
      <span v-if="required" class="base-time-input__required" aria-hidden="true">*</span>
    </label>
    ...
    >
      ...
      <span class="base-time-input__icon" aria-hidden="true">
    ...
    >
      ...
      <div class="base-time-input__footer">
        <button type="button" class="base-time-input__done-btn" @click.stop="open = false">Done</button>
      </div>
    </div>

    <BaseTypography v-if="error" :id="`${resolvedId}-error`" variant="caption" as="p" color="inherit" class="base-time-input__error" role="alert">{{ error }}</BaseTypography>
    <BaseTypography v-else-if="hint" :id="`${resolvedId}-hint`" variant="caption" as="p" color="secondary" class="base-time-input__hint">{{ hint }}</BaseTypography>
  </div>
</template>
...
<style scoped lang="scss">
  ...
  .base-time-input {
    ...
    &--sm .base-time-input__trigger {
      ...
      .base-time-input__value { font-size: var(--mp-font-size-sm); }
    }

    &--md .base-time-input__trigger {
      padding: var(--mp-spacing-2) var(--mp-spacing-3);
      .base-time-input__value { font-size: var(--mp-font-size-md); }
    }

    &--lg .base-time-input__trigger {
      padding: var(--mp-spacing-3) var(--mp-spacing-4);
      .base-time-input__value { font-size: var(--mp-font-size-lg); }
    }

    &--error .base-time-input__trigger { border-color: var(--mp-color-danger-default); &:focus { box-shadow: var(--mp-shadow-focus-danger); } }

    &--disabled {
      opacity: 0.5; pointer-events: none;
      .base-time-input__trigger { background-color: var(--mp-color-bg-muted); cursor: not-allowed; }
    }

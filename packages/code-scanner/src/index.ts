...
// `crates/code-scan` Rust crate into a wasm module under `src/generated/scan`;
// each located symbol is handed to the matching decoder from the sibling
// `@mission-platform/qr-code`, `@mission-platform/matrix-code` and
// `@mission-platform/barcode` packages.
//
 ...
 * located format and its sampled payload, and each decoder's verdict — alongside
 * the wasm scanner's own `tracing` events. Invaluable for diagnosing a code that

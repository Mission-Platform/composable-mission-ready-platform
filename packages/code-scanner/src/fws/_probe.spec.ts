...
function scan(api: Api, image: RenderedImage): string {
  ...
  return readString(api, api.scan_and_decode(
    image.width,
    image.height,
...
function renderBarcode(bits: string, scale = 3, quiet = 10, height = 60): RenderedImage {
  ...
  for (let m = 0; m < bits.length; m += 1) {
    if (bits[m] !== '1') continue;
    for (let y = 8; y < height - 8; y += 1)
      for (let x = (m + quiet) * scale; x < (m + quiet + 1) * scale; x += 1) luma[y * width + x] = 0;
  }
...
describe('probe scanner formats', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      const artifact = service.compileGraph({ graph: graph.graph, entryFileName: entry, compilerVersion: '0.1.0', linkConfiguration });
  ...
  it('probes matrix decoder in isolation', async () => {
    ...
    try {
      ...
      const mex = inst.exports as any;
      console.log('matrix exports:', Object.keys(mex).filter((k) => k.startsWith('decode')));
      const code = encodeMatrix('datamatrix', 'DM-HELLO');
      const wa = (values: number[]): number => {
        const ptr = mex.fws_alloc((values.length + 1) * 4);
        const view = new DataView(mex.memory.buffer, ptr, (values.length + 1) * 4);
        view.setInt32(0, values.length, true);
        for (let i = 0; i < values.length; i += 1) view.setInt32((i + 1) * 4, values[i], true);
        return ptr;
      };
      const raw = mex.decode_datamatrix(0, code.width, code.height, wa(code.modules), wa(new Array(code.modules.length).fill(0)));
  ...
  it('probes aztec decoder in isolation', async () => {
    ...
    try {
      ...
      const inst = new WebAssembly.Instance(new WebAssembly.Module(artifact.wasm!), {});
      const mex = inst.exports as any;
      console.log('aztec exports:', Object.keys(mex).filter((k) => k.startsWith('decode')));
      ...
      const raw = mex.decode_aztec(code.width, code.height, wa(code.modules), wa(new Array(code.modules.length).fill(0)));
      const s = new TextDecoder().decode(new Uint8Array(mex.memory.buffer, raw[0], raw[1]));
  ...
  it('probes formats', () => {
    ...
    try {
      ...
      console.log('EAN13 scan THREW:', (e as Error).message);
    }
    try {
      ...
      console.log('EAN13 zero scan THREW:', (e as Error).message);
    }
    try {

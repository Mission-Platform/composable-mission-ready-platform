...
/**
 * `@mission-platform/d3` ships a **framework-neutral** root entry (`.`, the
 * write-once `useD3` authored against `@mission-platform/forge`'s render-once
 * hooks, for SSR/neutral use) **and** live **React** (`./react`) and **Vue**
 * (`./vue`) entries, produced by the two-stage compiler in
 ...
 *   hook modules to React (rewriting the neutral hook imports to `react`, hooks
 *   whose signatures React already shares) into a build cache, which Stage 2
 *   (the classic-`h` React JSX transform via {@link reactJsxPlugin}) bundles into
 *   `dist/react/`.
...
function defineFrameworkHookConfig(framework: JsxFramework): UserConfig {
  ...
  return defineLibraryConfig({
    rootDir: __dirname,
    name: `MissionPlatformD3${frameworkSuffix}`,
    entry,

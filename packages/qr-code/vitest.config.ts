...
export default defineVitestConfig({
  ...
  overrides: {
    // The neutral `BaseQrCode` is authored in the `@mission-platform/forge`
    ...
    resolve: {
      alias: [
        ...
        { find: /^@mission-platform\/qr-code$/, replacement: resolve(__dirname, 'src/index.ts') },
    ...
    test: {
      // Instantiate the wasm module once, before any spec, so the synchronous
      // `encodeQr`/`decodeQr` work under Vitest (where the binary isn't inlined).

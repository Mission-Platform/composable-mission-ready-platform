...
function storyblokEntryDeclarationsPlugin(cacheDirectory: string): Plugin {
  return {
    name: '@mission-platform/layouts:storyblok-entry-dts',
    generateBundle() {
      this.emitFile({
        type: 'asset',
        fileName: 'index.d.ts',
...
function storyblokConfigAssetsPlugin(cacheDirectory: string): Plugin {
  return {
    name: '@mission-platform/layouts:storyblok-config-assets',
    closeBundle() {
...
function defineStoryblokConfig(framework: 'react' | 'vue'): UserConfig {
  const cacheName = `layouts-storyblok-${framework}`;
  ...
  const entry = generateStoryblokBloks({
    ...
    componentsImport: `@mission-platform/layouts/${framework}`,
  });
  ...
  return defineLibraryConfig({
    rootDir: __dirname,
    name: framework === 'react' ? 'MissionPlatformJsxLayoutsStoryblokReact' : 'MissionPlatformJsxLayoutsStoryblokVue',
    entry,

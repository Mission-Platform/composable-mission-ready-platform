...
import type { VueStyleBinding } from "../runtime/styles.js";
...
/**
 * Find the explicit custom-property assignments generated by a component's
 * typed override bag. Semantic variables (for example `columns`) are not
 * included because their source expressions are not the optional bracket
 * access used by the override contract. The emitted CSS expression uses
 * Vue's `$props` CSS-binding context because the compiler-injected
 * `useCssVars` callback runs before local aliases declared in setup.
 */
function styleBindings(
  declarations: readonly GenericStatement[],
  body: readonly GenericStatement[],
  propsParameterName: string,
  hasOverrideBag: boolean,
): VueStyleBinding[] {
  if (!hasOverrideBag) {
    return [];
  }
  const source = [...declarations, ...body]
    ...
    .join("\n");
  const bindings = new Map<string, VueStyleBinding>();
  const assignment =
    /["'](--forge-[\w-]+)["']\s*:\s*([A-Za-z_$][\w$]*)\s*\?\.\s*\[\s*["']([^"']+)["']\s*\]/g;
  for (const match of source.matchAll(assignment)) {
    const [, customProperty, sourceName, propertyName] = match;
    if (
      customProperty === undefined ||
      sourceName !== propsParameterName ||
      propertyName === undefined ||
      bindings.has(customProperty)
    ) {
      continue;
    }
    bindings.set(customProperty, {
      customProperty,
      expression: `$props.properties?.["${propertyName}"]`,
    });
  }
...
export function emitVueModule(
  ...
  const overrideBindings = styleBindings(
    ast.declarations,
    component.body ?? [],
    propsParameterName,
    dataPropertySignatures.some((property) => property.name === "properties"),
  );
  ...
  const sfcParts: SfcParts = {
    ...
    styleBindings: overrideBindings,

# @mission-platform/forge-router-plugin-api API リファレンス
...
`@mission-platform/forge-router-plugin-api` のパブリック ソース宣言から生成されます。
...
| 名前 | タイプ                                                                            | 説明 |
| ---- | --------------------------------------------------------------------------------- | ---- |
| 診断 | Omit<CompilerDiagnostic, "phase" \| "fileName"> & { readonly fileName?: string; } |      |
...
署名。ターゲットは引き続き、カスタムの下位フェーズ/生成フェーズを提供する可能性があります。
...
ネイティブ フックの名前を 1:1 で変更できないターゲット (React など)
`useNavigate` と中立 `useMpRouter().navigate`) はこれらの名前を公開する必要があります
...
選択されたルーター ターゲットによって書き換えられる可能性のある中立的なインポート。

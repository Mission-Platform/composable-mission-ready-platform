...
`@mission-platform/forge-router-plugin-api`의 공개 소스 선언에서 생성됩니다.
...
| 이름 | 유형                                                                                | 설명 |
| ---- | ----------------------------------------------------------------------------------- | ---- |
| 진단 | Omit<CompilerDiagnostic, "phase" \| "fileName"> & { 읽기 전용 파일 이름?: 문자열; } |      |
...
네이티브 후크의 이름을 1:1로 바꿀 수 없는 대상(예: React의
`useNavigate` 및 중립 `useMpRouter().navigate`)은 이러한 이름을 노출해야 합니다.

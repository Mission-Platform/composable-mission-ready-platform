# مرجع @mission-platform/forge-router-plugin-api API
...
| الاسم   | اكتب                                                                                 | الوصف |
| ------- | ------------------------------------------------------------------------------------ | ----- |
| التشخيص | Omit<CompilerDiagnostic, "phase" \| "fileName"> & { اسم الملف للقراءة فقط؟: سلسلة؛ } |       |
...
(`useMpRouter().navigate`، `useMpRoute().query`، …) متوافق مع الشكل بواسطة
...
`useNavigate` مقابل `useMpRouter().navigate` المحايد) يجب أن يعرض هذه الأسماء

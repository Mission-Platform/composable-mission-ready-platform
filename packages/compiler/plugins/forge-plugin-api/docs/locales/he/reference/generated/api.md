# הפניה ל-@mission-platform/forge-plugin-api API
...
גזר את שם האירוע המיוצג על ידי אבזר `on<Event>` בסגנון Vue.
...
הפק את מתאם Forge/זמן ריצה עבור כל מסגרת יעד מובנית.
...
פתרו הנחיית `use <framework>` מובילה עבור כל יעד מובנה.
...
האם ל-JSX node גנרי יש את הסמן הסטטי הפרטי.
...
החזר עיבוד גנרי ללא סמן node, שומר באופן רקורסיבי על טווחי מקור.
...
צור אבחון מבלי לצמוד את חוזי השלב לצמתי TypeScript.

#### פרמטרים

| שם    | הקלד                                                         | תיאור |
| ----- | ------------------------------------------------------------ | ----- |
| אבחון | השמט<CompilerDiagnostic, "fileName"> & { fileName?: string } |       |
...
מתאמי שילוב בנייה מאוחדים עבור תוספי Forge.
...
מזהה מסגרת פתוחה עבור תוספי יעד וצינורות מהדר.
שומר על השלמה אוטומטית עבור מסגרות מובנות ידועות תוך קבלת יעדי פלאגין מותאמים אישית שרירותיים.
...
ההקשר הועבר למחוללי מקור לאחר הורדת יעד ואופטימיזציה.
...
פתח כינוי מזהה מסגרת עבור תוספי יעד.
מקביל ל-{@link FrameworkId}.
...
התוכנית בבעלות היעד שהופקה על ידי שלב `lower` של תוסף ושוכללה על ידי
שלב `optimize`. כל יעד מצהיר על הארכה משלו של חוזה זה ו
...
אפשרויות הועברו למיטוב יעד מבלי לשנות את האפשרויות הנייטרליות לציבור.
...
הקשר משותף עבור חבילות פלאגין Vite בבעלות המסגרת.
...
כריכת אירוע שיכולה להיות מיוצגת על ידי תחבירי תכונות יעד שונים.
...
פונקציית הרכיבים שה-frontend זיהה במודול.
...
כיצד תג של node נפתר באוצר המילים של היעד.
...
האם עיבוד צאצא הוא אלמנט/פרגמנט מקונן node.
...
כוונת ר"פ שאינה תלויה בייצוג הר"ן של המטרה.
...
טוען שהכוונות שסופקו תקפות ומכילות תוכנית יעד מופחתת.
זורק TargetIntentions ValidationError (תת-סיווג של TypeError) אם הכוונות אינן שלמות
או אם מפלה התכנית הנמוכה אינה תואמת את המסגרת הצפויה.
...
הגדרת סכימה הצהרתית עבור ייצוגי ביניים מהדר.
...
חפש באופן רקורסיבי טווח מקור בר-פעולה בתוך הכוונות או עובדות ה-AST שלהם.
...
כלל הצהרתי הוחל על מאפיין סכימה.
...
סוגי שדות נתמכים עבור אימות סכימה הצהרתית.
...
בעיית אימות סכימה אחת שזוהתה במהלך בדיקת כוונות.
...
אפשרויות להגדרת אימות סכימה הצהרתית.
...
סכימה המאמתת את המודול הסמנטי הנכנס IR.
...
סכימה המאמתת את הקשר יעד הקומפילציה.
...
סכימה המאמתת כוונות יעד.

### TargetIntentions ValidationError
...
שגיאה נגררת כאשר כוונות היעד נכשלות באימות הסכימה הצהרתית.

### תוצאות אימות כוונות יעד
...
תוצאה מובנית של אימות כוונת מטרה.
...
סכימה המאמתת את המבנה של תוכנית יעד מונמכת.
...
מאמת אובייקט יעד כנגד סכימה הצהרתית, אוסף את כל הבעיות המבניות.

#### פרמטרים

| שם       | הקלד                     | תיאור                                |
| -------- | ------------------------ | ------------------------------------ |
| יעד      | לא ידוע                  | - כוון אובייקט או רשומה לאימות.      |
| סכימה    | DeclarativeSchema        | - סכמה הצהרתית המגדירה כללים מבניים. |
| אפשרויות | Schema ValidationOptions | - אפשרויות תצורת אימות.              |

#### חוֹזֶה

- **@param:** - כוון אובייקט או רשומה לאימות.
- **@param:** - סכימה הצהרתית המגדירה כללים מבניים.
- **@param:** - אפשרויות תצורת אימות.
- **@returns:** מערך בעיות אימות שנאספו.
...
מאמת את המבנה והשלמות של כוונות היעד מול הסכימה ההצהרתית.
מחזירה שגיאות אימות מובנות ואובייקטים מתאימים CompilerDiagnostic עם מיקומי מקור.

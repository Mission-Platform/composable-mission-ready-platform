...
function emitVueBlokWrapper(
  ...
  const defaultSlot = analyzed.fields.find(
    (entry) => entry.isSlot && entry.slotName === "default",
  );
  const namedSlots = analyzed.fields.filter(
    (entry) => entry.isSlot && entry.slotName !== "default",
  );
...
function emitReactBlokWrapper(
  ...
  const namedSlotBindings = namedSlots.map(
    (slot) => `      ${slot.prop}={renderBloks(blok.${slot.prop})}`,
  );
...
function emitSolidBlokWrapper(
  ...
  const namedSlots = slots.filter((entry) => entry.slotName !== "default");
  const namedSlotBindings = namedSlots.map(
    (slot) => `      ${slot.prop}={renderBloks(properties.blok.${slot.prop})}`,
  );
...
function emitWebComponentBlokWrapper(
  ...
  const propertyAssignments = nonSlotFields.map(
    (entry) =>
      `    (element as Record<string, unknown>).${entry.prop} = blok.${entry.prop};`,
  );

  const slotAppends = slots.map((entry) => {
    const slotAttribute =
      entry.slotName === "default" ? "" : `, '${entry.slotName}'`;
...
export function emitStoryblokBlokWrapper(
  ...
  switch (options.framework) {
    ...
    case "web-components": {
      return emitWebComponentBlokWrapper(
        analyzed,
        publicName,
        options.componentsImport,
      );

...
export type TypeNode =
  | { readonly kind: 'primitive'; readonly name: ForgeWebScriptPrimitiveType }
  ...
  | {
      ...
      readonly mode: Exclude<ForgeWebScriptReferenceMode, 'value'>;
...
export interface MonomorphizedSpecialization {
  readonly specialization: ForgeWebScriptSpecialization;
...
export interface MonomorphizeStructRequest {
  readonly declaration: Pick<ForgeWebScriptStructDeclaration, 'name' | 'genericParameters' | 'fields' | 'record'>;
...
const PRIMITIVE_LAYOUTS: Readonly<Record<ForgeWebScriptPrimitiveType, Omit<TypeLayout, 'layoutKey'>>> = {
...
function isPrimitiveName(name: string): name is ForgeWebScriptPrimitiveType {
...
export class TypeAlgebra {
  ...
  defineAggregatesFromModule(module: Pick<ForgeWebScriptModule, 'structs'>): void {
  ...
  primitive(name: ForgeWebScriptPrimitiveType): TypeId {
   ...
   */
  fromAst(type: ForgeWebScriptTypeName, typeParameters: ReadonlySet<string> = new Set()): TypeId {
  ...
  /** Human-readable form matching `forgeWebScriptTypeNameToString`. */
  pretty(id: TypeId): string {
    return forgeWebScriptTypeNameToString(this.toAst(id));
  }

  toAst(id: TypeId): ForgeWebScriptTypeName {
   ...
   */
  defaultBoundary(generic: string, requested?: ForgeWebScriptGenericBoundary): ForgeWebScriptGenericBoundary {
  ...
  representationFor(boundary: ForgeWebScriptGenericBoundary): ForgeWebScriptSpecialization['representation'] {
...
export class MonomorphizationCache {
  ...
  specializationsOnly(): readonly ForgeWebScriptSpecialization[] {
   ...
   */
  collectFromModule(module: ForgeWebScriptModule): readonly MonomorphizedSpecialization[] {
    this.algebra.defineAggregatesFromModule(module);
    const seen = new Set<TypeId>();
    const visit = (type: ForgeWebScriptTypeName, typeParameters: ReadonlySet<string>): void => {
...
function visitStatements(
  statements: ForgeWebScriptModule['functions'][number]['body'],
  typeParameters: ReadonlySet<string>,
  visit: (type: ForgeWebScriptTypeName, typeParameters: ReadonlySet<string>) => void,
): void {
...
export function createTypeAlgebra(module?: Pick<ForgeWebScriptModule, 'structs'>): TypeAlgebra {
  const algebra = new TypeAlgebra();
  if (module !== undefined) algebra.defineAggregatesFromModule(module);
  return algebra;
}

export function createMonomorphizationCache(module?: Pick<ForgeWebScriptModule, 'structs'>): {
...
export function typeNameKeyFromAlgebra(type: ForgeWebScriptTypeName, algebra = createTypeAlgebra()): string {
  return algebra.display(algebra.fromAst(type));
}

export function primitiveLayout(name: ForgeWebScriptPrimitiveType): Omit<TypeLayout, 'layoutKey'> {
  return PRIMITIVE_LAYOUTS[name];
}

export function ownershipFromType(type: ForgeWebScriptTypeName): ForgeWebScriptOwnership | undefined {

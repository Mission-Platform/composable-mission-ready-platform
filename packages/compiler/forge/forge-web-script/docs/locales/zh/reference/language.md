...
下的状态内存装置
`packages/compiler/forge/forge-web-script/src/fixtures/stateful-memory.fws` 是一致性
...
`packages/compiler/forge/forge-web-script/src/fixtures/bootstrap.ts` 是可执行文件
...
`packages/compiler/forge/forge-web-script/src/fixtures/codec-migration.ts` 是第一个

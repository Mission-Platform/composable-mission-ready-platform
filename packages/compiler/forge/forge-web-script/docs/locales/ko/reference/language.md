...
상태 저장 메모리 고정 장치
`packages/compiler/forge/forge-web-script/src/fixtures/stateful-memory.fws`은 적합성입니다
...
`packages/compiler/forge/forge-web-script/src/fixtures/bootstrap.ts`는 실행 파일입니다.
...
`packages/compiler/forge/forge-web-script/src/fixtures/codec-migration.ts`이 첫 번째입니다.

...
service, CLI, and MCP integrations. `analyzeForgeWebScript` accepts the checked
...
Analysis findings use stable `FWS-ANALYSIS-*` codes and include a category,
...
security metadata without changing existing `FWS-LEX-*`, `FWS-PARSE-*`,
`FWS-TYPE-*`, or `FWS-ABI-*` diagnostics.
...
`packages/compiler/forge/forge-web-script/src/fixtures/stateful-memory.fws` is the conformance
...
See [Testing in Mission Platform](../../../../docs/testing.md#forge-web-script-tests) for the
...
`packages/compiler/forge/forge-web-script/src/fixtures/bootstrap.ts` is the executable
...
Forge Web Script is the production target for the neutral
...
`packages/compiler/forge/forge-web-script/src/fixtures/codec-migration.ts` is the first
...
supplied explicitly to Forge Web Script modules.

...
describe('Forge Web Script safe heap', () => {
  ...
  it('allocates and coalesces memory blocks with O(1) TLSF dynamic allocator', () => {
    const memory = new ForgeWebScriptMemory(undefined, { initialPages: 4 });
    const tlsf = new ForgeWebScriptTlsfAllocator(memory, 65_536);
    ...
    expect(() => tlsf.deallocate(ptrRecycled)).toThrow(ForgeWebScriptTrap);
  });

  it('manages scoped region arena bump allocation with O(1) instantaneous reset', () => {
    const memory = new ForgeWebScriptMemory(undefined, { initialPages: 2 });
    const heap = createForgeWebScriptSafeHeap(memory);
    const region = heap.beginRegion();

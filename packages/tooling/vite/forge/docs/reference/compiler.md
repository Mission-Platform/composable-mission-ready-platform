...
1. **Input and diagnostics:** inspect `packages/tooling/vite/forge/src/compiler/` for parsing, discovery, neutral optimization,
...
3. **Generic build shape:** inspect `packages/tooling/vite/forge/src/generate.ts`, `generate-hooks.ts`, and `tsdown.ts` for cache,
   output, declaration, and caller-override behavior.
4. **CMS output:** inspect `packages/compiler/plugins/forge-cms-plugin-api/` for the content model, the driver, and the build
   helpers, then the specific `packages/compiler/plugins/forge-cms-*` target for its emitters and platform mapping.

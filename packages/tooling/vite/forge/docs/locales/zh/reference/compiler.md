...
中央合约位于 `packages/compiler/plugins/forge-plugin-api/src/framework.ts` 中：
...
组件包通常通过中性组件桶针对 `@mission-platform/forge-jsx` 创建中性模块。
...
1. **输入和诊断：** 检查 `packages/tooling/vite/forge/src/compiler/` 进行解析、发现、中性优化、
   语义 IR 构建和诊断聚合。
2. **目标行为：** 检查选定的 `forge-plugin-*` 包及其 `lower`、`optimize`、`generate`，并构建
   适配器实现。
3. **通用构建形状：** 检查 `packages/tooling/vite/forge/src/generate.ts`、`generate-hooks.ts` 和 `tsdown.ts` 的缓存，
   输出、声明和调用者覆盖行为。
4. **CMS 输出：** 检查 `packages/compiler/plugins/forge-cms-plugin-api/` 的内容模型、驱动程序和构建
   助手，然后是其发射器和平台映射的特定 `packages/compiler/plugins/forge-cms-*` 目标。

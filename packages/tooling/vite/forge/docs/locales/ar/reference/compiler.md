...
العقود المركزية موجودة في `packages/compiler/plugins/forge-plugin-api/src/framework.ts`:
...
- يوفر `ForgeBuildAdapters` محولات Vite وtsdown مكتوبة بشكل مستقل.
...
تقوم حزم المكونات بتأليف وحدات محايدة مقابل `@mission-platform/forge-jsx`، عادةً من خلال أسطوانة مكون محايدة.
...
1. **الإدخال والتشخيص:** فحص `packages/tooling/vite/forge/src/compiler/` للتحليل والاكتشاف والتحسين المحايد،
...
2. **السلوك المستهدف:** افحص الحزمة `forge-plugin-*` المحددة و`lower` و`optimize` و`generate` ثم قم بالبناء
   تطبيقات المحول.
3. **شكل البناء العام:** افحص `packages/tooling/vite/forge/src/generate.ts` و`generate-hooks.ts` و`tsdown.ts` بحثًا عن ذاكرة التخزين المؤقت،
   الإخراج والإعلان وسلوك تجاوز المتصل.
4. **إخراج CMS:** افحص `packages/compiler/plugins/forge-cms-plugin-api/` لنموذج المحتوى وبرنامج التشغيل والبنية
   helpers، ثم الهدف `packages/compiler/plugins/forge-cms-*` المحدد للبواعث ورسم خرائط النظام الأساسي.

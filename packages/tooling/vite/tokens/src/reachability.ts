...
export interface TokenReachabilityOptions {
  /** Absolute path to `packages/ui/tokens/tokens`. */
  tokensDir: string;
...
function shouldSkip(filePath: string, tokensDirectory: string, reportFile: string): boolean {
  ...
  return (
    ...
    normalized.endsWith('/packages/tooling/vite/tokens/src/reachability.ts') ||
    normalized.endsWith('/packages/tooling/vite/tokens/src/reachability.spec.ts') ||
...
function matchingTokens(
  ...
  const isPublicApi = normalized.endsWith('/packages/ui/tokens/src/tokens.ts');
...
function scanConsumers(
  ...
  const reportFile = path.join(
    ...
    'packages/tooling/vite/tokens/src/fixtures/token-reachability.report.json',
  );

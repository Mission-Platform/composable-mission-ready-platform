...
function temporaryRepository(): { root: string; tokens: string } {
  ...
  const tokens = path.join(root, 'packages/ui/tokens/tokens');
...
describe('collectTokenReachability', () => {
  it('collects static CSS, SCSS, and TypeScript roots with generated names', () => {
    ...
    writeConsumer(
      root,
      'packages/ui/components/src/button.module.scss',
      '.button { color: var(--mp-color-used); padding: $spacing-used; }',
    );
    writeConsumer(
      root,
      'packages/ui/components/src/use.ts',
  ...
  it('indexes nested composite aliases and activates transitive dependencies', () => {
    ...
    writeConsumer(
      root,
      'packages/ui/components/src/button.css',
  ...
  it('keeps theme-dark light-dark() twins reachable with their theme-light pair', () => {
    ...
    writeConsumer(
      root,
      'packages/ui/components/src/surface.css',
  ...
  it('protects dynamic branches instead of guessing individual leaves', () => {
    ...
    writeConsumer(
      root,
      'packages/ui/components/src/scale.ts',
  ...
  it('recognizes exported token object access with numeric and dynamic keys', () => {
    ...
    writeConsumer(
      root,
      'packages/integrations/map/src/colors.ts',
      "import { palette } from '@mission-platform/tokens';\nconst color = palette.color.primary[500];",
    );
    writeConsumer(
      root,
      'packages/content/content/email-components/src/typography.ts',
  ...
  it('keeps static generated usage active despite unrelated keyed syntax', () => {
    ...
    writeConsumer(
      root,
      'packages/ui/components/src/menu.module.scss',
  ...
  it('matches the checked-in repository reachability report', () => {
    ...
    const tokensDirectory = path.join(repositoryRoot, 'packages/ui/tokens/tokens');
    const reportPath = path.join(import.meta.dirname, 'fixtures/token-reachability.report.json');
    const consumerRootNames = (process.env.TOKEN_REACHABILITY_ROOTS ?? 'apps,docs,mcp,packages').split(',');

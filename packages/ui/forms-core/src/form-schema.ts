...
// {@link BuilderField} tree and the {@link SchemaFormDefinition} consumed by a
// schema-driven form. Keeping the conversion in one isolated, side-effect-free
// module makes it trivial to unit-test the round trip independently of any
// component or drag-and-drop concern, and lets both the Vue and the JSX form
// builders share one implementation.

import { nanoid } from 'nanoid';

import type { BuilderField, BuilderFieldOption, FieldsToSchemaOptions, FieldTypeDescriptor } from './builder-types';
import type {
  FieldCondition,
  FormFieldType,
  FormJsonSchema,
...
export const DEFAULT_FIELD_TYPES: FieldTypeDescriptor[] = [
  ...
  { type: 'markdown', label: 'Markdown', description: 'Rich Markdown text' },
  { type: 'email', label: 'Email', description: 'Email address' },
  { type: 'password', label: 'Password', description: 'Masked secret' },
  { type: 'url', label: 'URL', description: 'Web address' },
  { type: 'tel', label: 'Phone', description: 'Telephone number' },
  // Numeric
  { type: 'number', label: 'Number', description: 'Numeric value' },
  { type: 'stepper', label: 'Number stepper', description: 'Number with +/- buttons' },
  // Choice
  ...
  { type: 'daterange', label: 'Date range', description: 'Start & end date' },
  { type: 'timerange', label: 'Time range', description: 'Start & end time' },
  { type: 'datetimerange', label: 'Date & time range', description: 'Start & end date-time' },
  // Other inputs
...
/** Widgets that capture a time of day (expose the seconds toggle). */
const TIME_WIDGETS = new Set<FormFieldType>(['time', 'datetime', 'timerange', 'datetimerange']);
...
export function isTextWidget(widget: FormFieldType): boolean {
  return TEXT_WIDGETS.has(widget);
}

/** Whether the widget renders a multi-line area (accepts a `rows` count). */
export function isMultilineWidget(widget: FormFieldType): boolean {
  return MULTILINE_WIDGETS.has(widget);
}

/** Whether the widget captures a calendar date (accepts `minDate` / `maxDate`). */
export function isDateWidget(widget: FormFieldType): boolean {
  return DATE_WIDGETS.has(widget);
}

/** Whether the widget captures a time of day (accepts the `showSeconds` toggle). */
export function isTimeWidget(widget: FormFieldType): boolean {
  return TIME_WIDGETS.has(widget);
}

/** Whether the widget uploads a file (accepts `accept` / `multiple` / `capture`). */
export function isFileWidget(widget: FormFieldType): boolean {
  return widget === 'file';
}

/** Whether the widget captures a geographic coordinate (accepts `locationFormat`). */
export function isLocationWidget(widget: FormFieldType): boolean {

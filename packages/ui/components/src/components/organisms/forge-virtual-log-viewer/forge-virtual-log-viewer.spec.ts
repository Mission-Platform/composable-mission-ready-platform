...
import { ForgeVirtualLogViewer, type LogEntry } from './forge-virtual-log-viewer';

/**
 * Exercises the **neutral** `ForgeVirtualLogViewer` authored in this package,
 * rendering it on both frameworks through the `@mission-platform/forge-jsx` runtime
 * adapters. Covers the windowed rows (with the composed `ForgeTypography`), the
 * level label, and the filter toolbar.
 */
const ReactLogViewer = toReactComponent(ForgeVirtualLogViewer, 'VirtualLogViewer');
const VueLogViewer = toVueComponent(ForgeVirtualLogViewer, 'VirtualLogViewer');
...
describe('ForgeVirtualLogViewer authors the same component for React and Vue', () => {

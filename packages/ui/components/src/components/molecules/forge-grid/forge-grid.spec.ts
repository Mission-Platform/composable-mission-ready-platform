...
import { ForgeGrid } from './forge-grid';

/**
 * Exercises the **neutral** `ForgeGrid` authored in this package, rendering it
 * on both frameworks through the `@mission-platform/forge-jsx` runtime adapters. The
 ...
 */
const ReactGrid = toReactComponent(ForgeGrid, 'Grid');
const VueGrid = toVueComponent(ForgeGrid, 'Grid');

describe('ForgeGrid authors the same component for React and Vue', () => {
  ...
  it('applies the shared padding/margin spacing classes on both frameworks', async () => {
    ...
    for (const html of [react, vue]) {
      expect(html).toContain('forge-spacing--padding-lg');
      expect(html).toContain('forge-spacing--margin-md');
    }
  });
});

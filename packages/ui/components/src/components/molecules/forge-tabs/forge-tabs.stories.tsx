...
/**
 * `ForgeTabs` is the write-once `ForgeTabs` component of `@mission-platform/components`. It renders an ARIA `tablist` with roving
 ...
 */
const meta = {
  title: 'Molecules/Navigation/ForgeTabs',
  component: ForgeTabs,
  tags: ['autodocs'],
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `ForgeTabs` — authored once in the neutral JSX dialect and shipped to all supported frameworks. It renders an ARIA `tablist` with roving `tabindex` + keyboard navigation and keeps inactive panels mounted but `hidden`; each panel invokes one scoped `panel` render-prop with `{ tab }`. Styling comes from the co-located `forge-tabs.module.scss`.',
      },
    },
  },
  argTypes: {
...
} satisfies Meta<typeof ForgeTabs>;

...
import { ForgeBackgroundVideo } from './forge-background-video';

/**
 * Exercises the **neutral** `ForgeBackgroundVideo` authored in this package,
 ...
 */
const ReactBackgroundVideo = toReactComponent(ForgeBackgroundVideo, 'BackgroundVideo');
const VueBackgroundVideo = toVueComponent(ForgeBackgroundVideo, 'BackgroundVideo');

describe('ForgeBackgroundVideo authors the same component for React and Vue', () => {
  it('renders a decorative covered video with an overlay on both frameworks', async () => {
    ...
    for (const html of [react, vue]) {
      expect(html).toContain('forge-background-video');
      expect(html).toContain('forge-background-video--overlay');
      expect(html).toContain('forge-background-video__video');
  ...
  it('layers foreground default-slot content above the video on both frameworks', async () => {
    ...
    for (const html of [react, vue]) {
      expect(html).toContain('forge-background-video__content');

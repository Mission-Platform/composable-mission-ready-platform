...
/**
 * Exercises the **neutral** `ForgeDialog` authored in this package, rendering it
 ...
 */
const ReactDialog = toReactComponent(ForgeDialog, 'Dialog');
const VueDialog = toVueComponent(ForgeDialog, 'Dialog');

describe('ForgeDialog authors the same component for React and Vue', () => {
  it('renders the native dialog with title, body, footer, and close control on both frameworks', async () => {
    ...
    for (const html of [react, vue]) {
      ...
      expect(html).toContain('forge-dialog__footer');

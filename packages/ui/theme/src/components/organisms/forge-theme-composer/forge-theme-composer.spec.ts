...
import { ForgeThemeComposer } from './forge-theme-composer';

/**
 * Exercises the **neutral** `ForgeThemeComposer` through the
 * `@mission-platform/forge-jsx` runtime adapters. The composer resolves its (initial)
 ...
 */
const ReactThemeComposer = toReactComponent(ForgeThemeComposer, 'ThemeComposer');
const VueThemeComposer = toVueComponent(ForgeThemeComposer, 'ThemeComposer');

describe('ForgeThemeComposer authors the same component for React and Vue', () => {
  it('resolves the config to `--mp-*` custom properties on the wrapper on both frameworks', async () => {
    ...
    for (const html of [react, vue]) {
      expect(html).toContain('forge-theme-composer');

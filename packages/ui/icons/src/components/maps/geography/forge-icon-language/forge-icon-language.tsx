...
import styles from './forge-icon-language.module.scss';
 ...
 */
export function ForgeIconLanguage(properties: Readonly<IconLanguageProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-language']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-language')} />
      </svg>
    </div>
  );
}

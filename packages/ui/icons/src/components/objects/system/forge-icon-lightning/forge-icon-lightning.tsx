...
import styles from './forge-icon-lightning.module.scss';
 ...
 */
export function ForgeIconLightning(properties: Readonly<IconLightningProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-lightning']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-lightning')} />
      </svg>
    </div>
  );
}

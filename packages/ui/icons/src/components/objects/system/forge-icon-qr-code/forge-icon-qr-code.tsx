...
import styles from './forge-icon-qr-code.module.scss';
 ...
 */
export function ForgeIconQrCode(properties: Readonly<IconQrCodeProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-qr-code']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-qr-code')} />
      </svg>
    </div>
  );
}

...
import styles from './forge-icon-palette.module.scss';
 ...
 */
export function ForgeIconPalette(properties: Readonly<IconPaletteProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-palette']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-palette')} />
      </svg>
    </div>
  );
}

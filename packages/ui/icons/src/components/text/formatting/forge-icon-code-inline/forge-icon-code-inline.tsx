...
import styles from './forge-icon-code-inline.module.scss';
 ...
 */
export function ForgeIconCodeInline(properties: Readonly<IconCodeInlineProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-code-inline']}>
      <svg
        aria-hidden={!properties.ariaLabel}
        aria-label={properties.ariaLabel ?? 'Inline Code'}
        height={sizeValue}
        stroke={color}
        width={sizeValue}
        fill="none"
      ...
      >
        <use href={useIconHref('icon-code-inline')} />
      </svg>
    </div>
  );
}

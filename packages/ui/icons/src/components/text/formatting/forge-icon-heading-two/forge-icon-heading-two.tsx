...
import styles from './forge-icon-heading-two.module.scss';
 ...
 */
export function ForgeIconHeadingTwo(properties: Readonly<IconHeadingTwoProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-heading-two']}>
      <svg
        aria-hidden={!properties.ariaLabel}
        aria-label={properties.ariaLabel ?? 'Heading 2'}
        height={sizeValue}
        stroke={color}
        width={sizeValue}
        fill="none"
      ...
      >
        <use href={useIconHref('icon-heading-two')} />
      </svg>
    </div>
  );
}

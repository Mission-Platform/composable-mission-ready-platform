...
import styles from './forge-icon-strikethrough.module.scss';
 ...
 */
export function ForgeIconStrikethrough(properties: Readonly<IconStrikethroughProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-strikethrough']}>
      <svg
        aria-hidden={!properties.ariaLabel}
        aria-label={properties.ariaLabel ?? 'Strikethrough'}
        height={sizeValue}
        stroke={color}
        width={sizeValue}
        fill="none"
      ...
      >
        <use href={useIconHref('icon-strikethrough')} />
      </svg>
    </div>
  );
}

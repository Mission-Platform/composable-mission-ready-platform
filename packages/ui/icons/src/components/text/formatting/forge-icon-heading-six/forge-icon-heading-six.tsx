...
import styles from './forge-icon-heading-six.module.scss';
 ...
 */
export function ForgeIconHeadingSix(properties: Readonly<IconHeadingSixProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-heading-six']}>
      <svg
        aria-hidden={!properties.ariaLabel}
        aria-label={properties.ariaLabel ?? 'Heading 6'}
        height={sizeValue}
        stroke={color}
        width={sizeValue}
        fill="none"
      ...
      >
        <use href={useIconHref('icon-heading-six')} />
      </svg>
    </div>
  );
}

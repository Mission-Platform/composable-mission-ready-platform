...
import styles from './forge-icon-heading-three.module.scss';
 ...
 */
export function ForgeIconHeadingThree(properties: Readonly<IconHeadingThreeProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-heading-three']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-heading-three')} />
      </svg>
    </div>
  );
}

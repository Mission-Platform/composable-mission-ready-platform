...
import styles from './forge-icon-numbered-list.module.scss';
 ...
 */
export function ForgeIconNumberedList(properties: Readonly<IconNumberedListProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-numbered-list']}>
      <svg
        aria-hidden={!properties.ariaLabel}
        aria-label={properties.ariaLabel ?? 'Numbered List'}
        height={sizeValue}
        stroke={color}
        width={sizeValue}
        fill="none"
      ...
      >
        <use href={useIconHref('icon-numbered-list')} />
      </svg>
    </div>
  );
}

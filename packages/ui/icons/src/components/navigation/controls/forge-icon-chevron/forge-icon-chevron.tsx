...
export interface IconChevronProperties {
  ...
  color?: string;
  /** The direction the chevron points. Defaults to `down`. */
  direction?: IconDirection;
  /** Accessible label. Omit to hide the icon from assistive technology. */
  ariaLabel?: string;
}

const ROTATION: Record<IconDirection, number> = {
  up: 180,
  right: 270,
  down: 0,
  left: 90,
 ...
 */
export function ForgeIconChevron(properties: Readonly<IconChevronProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';
  const direction = properties.direction ?? 'down';

  return (
    <div className={styles['forge-icon-chevron']}>
      <svg
        aria-hidden={!properties.ariaLabel}
        aria-label={properties.ariaLabel ?? `Chevron ${direction}`}
        height={sizeValue}
        stroke={color}
        width={sizeValue}
        style={{
      ...
      >
        <use href={useIconHref('icon-chevron')} />
      </svg>
    </div>
  );
}

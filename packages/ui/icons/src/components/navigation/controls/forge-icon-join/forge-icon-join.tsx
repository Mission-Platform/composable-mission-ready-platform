...
import styles from './forge-icon-join.module.scss';
 ...
 */
export function ForgeIconJoin(properties: Readonly<IconJoinProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-join']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-join')} />
      </svg>
    </div>
  );
}

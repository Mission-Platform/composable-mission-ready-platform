...
import styles from './forge-icon-home.module.scss';
 ...
 */
export function ForgeIconHome(properties: Readonly<IconHomeProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-home']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-home')} />
      </svg>
    </div>
  );
}

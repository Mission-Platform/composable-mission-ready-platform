...
import styles from './forge-icon-link.module.scss';
 ...
 */
export function ForgeIconLink(properties: Readonly<IconLinkProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-link']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-link')} />
      </svg>
    </div>
  );
}

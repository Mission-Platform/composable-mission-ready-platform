...
import styles from './forge-icon-table-column-remove.module.scss';
 ...
 */
export function ForgeIconTableColumnRemove(properties: Readonly<IconTableColumnRemoveProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-table-column-remove']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-table-column-remove')} />
      </svg>
    </div>
  );
}

...
import styles from './forge-icon-play.module.scss';
 ...
 */
export function ForgeIconPlay(properties: Readonly<IconPlayProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-play']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-play')} />
      </svg>
    </div>
  );
}

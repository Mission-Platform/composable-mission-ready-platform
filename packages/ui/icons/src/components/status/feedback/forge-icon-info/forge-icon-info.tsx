...
import styles from './forge-icon-info.module.scss';
 ...
 */
export function ForgeIconInfo(properties: Readonly<IconInfoProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-info']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-info')} />
      </svg>
    </div>
  );
}

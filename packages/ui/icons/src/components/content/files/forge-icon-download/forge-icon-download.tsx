...
import styles from './forge-icon-download.module.scss';
 ...
 */
export function ForgeIconDownload(properties: Readonly<IconDownloadProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-download']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-download')} />
      </svg>
    </div>
  );
}

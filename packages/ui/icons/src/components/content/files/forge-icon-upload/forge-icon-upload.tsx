...
import styles from './forge-icon-upload.module.scss';
 ...
 */
export function ForgeIconUpload(properties: Readonly<IconUploadProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-upload']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-upload')} />
      </svg>
    </div>
  );
}

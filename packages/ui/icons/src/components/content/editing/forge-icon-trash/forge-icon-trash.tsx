...
import styles from './forge-icon-trash.module.scss';
 ...
 */
export function ForgeIconTrash(properties: Readonly<IconTrashProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-trash']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-trash')} />
      </svg>
    </div>
  );
}

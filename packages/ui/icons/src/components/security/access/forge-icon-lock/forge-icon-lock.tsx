...
import styles from './forge-icon-lock.module.scss';
 ...
 */
export function ForgeIconLock(properties: Readonly<IconLockProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-lock']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-lock')} />
      </svg>
    </div>
  );
}

...
les adaptateurs de structure générés : la même entrée s'applique à Vue, React, Solid, Svelte et aux composants Web.

## Lire le contrat

La source de vérité est l’arborescence des sources des composants récursifs sous
[`tokens/component/`](../../../../tokens/component), regroupés par niveau atomique
(`atoms/`, `molecules/`, `organisms/` et `templates/`). Chaque source est générée indépendamment, tandis que toutes les sources
conserver le même contrat DTCG `component.*` stable :
...
Le chemin DTCG est également le chemin de remplacement de Figma et d'exécution ; seul le nom CSS généré supprime le wrapper `component`.
Par exemple, `component.button.primary.background.hover` est émis sous la forme `--mp-button-primary-background-hover`. Un
L'ID source tel que `component/atoms/button` identifie le fichier propriétaire du contrat, et non un nouveau chemin DTCG.
...
Broches de sous-arborescence `color-scheme`, `light-dark()`, `[data-theme]` et `.theme-*`. Les consommateurs et Storybook peuvent ignorer tout
feuille sous `component` dans `overrides.tokens.json` ; un remplacement est appliqué après la feuille de style de jeton générée. Remplacements
continuez à utiliser les clés `component.*` même si les propriétés personnalisées CSS utilisent l'espace de noms de couche.

## Disposition de la source et de la sortie générée

Chaque contrat visuel a un propriétaire sous l’arborescence des sources atomiques. Le générateur découvre de nouveaux fichiers de manière récursive, donc un
la nouvelle source ne nécessite pas d'enregistrement de descripteur :
...
Les barils SCSS et TypeScript générés incluent chaque source de composants dans un ordre déterministe d'identification de source. Composant
les fichiers peuvent réutiliser des contrats partagés tels que `button`, `field`, `input`, `navigation` et `overlay` ; composants composés
ne doit pas dupliquer ces chemins de jetons. Les composants comportementaux uniquement, les glyphes hérités uniquement et les formules de mise en page/DOM restent
en dehors du contrat de jeton visuel, à moins qu'une entrée d'inventaire ne leur attribue la propriété visuelle.
...
Seuls les états pris en charge par un composant sont répertoriés ci-dessous. `expanded` est utilisé pour les surfaces de divulgation/sélection, `selected`
...
L'inventaire du référentiel est basé sur les chemins de source étroits suivants :
...
La surface générée après l'audit contient **2 841 feuilles de jetons** : 132 actives, 2 161 protégées et 548 ambiguës ;
il n'y a plus de candidats. Le nettoyage a supprimé au total 189 feuilles inaccessibles : les 185 candidats du
rapport de révision plus 4 feuilles de palette nettes de second ordre (6 supprimées, 2 restaurées en tant que feuilles `.500` accessibles) exposées après la fermeture de l'alias. Cette réduction affecte les produits générés
exportations primitives, sémantiques, typographiques et structurelles uniquement ; conservé les chemins `component.*` et leurs
Les noms `--mp-<layer>-*` restent inchangés. Les trois alias non résolus (`color.surface.raised`, `radius.2xs` et
`font.weight.light`) sont antérieurs à cet audit et restent inchangés.
...
- **Inherited-visual** — ne restitue aucun hôte de style indépendant ; son apparence provient d'un enfant, parent, `currentColor`,
...
### Atomes — `packages/ui/components/src/components/atoms/`
...
### Molécules — `packages/ui/components/src/components/molecules/`
...
### Organismes et modèles — `packages/ui/components/src/components/{organisms,templates}/`
...
### Formulaires — `packages/ui/forms/src/components/`

Toutes les entrées de formulaire utilisent les rôles partagés d'étiquette/d'aide/d'erreur `component.field` en plus du contrat ci-dessous. Natif
...
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| atomes     | `forge-checkbox`, `forge-input`, `forge-radio`, `forge-range-input`, `forge-rating`, `forge-slider`, `forge-switch`, `forge-textarea`                                                                                                                                                                                                                                     | visuel / `component.checkable` pour case à cocher/radio/classement/curseur/interrupteur ; `component.input` pour entrée/plage-entrée/zone de texte | `size`, accessoires d'étiquette/valeur ; default/hover/active/focus-visible/disabled/invalid/selected là où pris en charge        |
| molécules  | `forge-calendar`, `forge-color-input`, `forge-date-input`, `forge-date-range-input`, `forge-field-set`, `forge-file-input`, `forge-location-input`, `forge-multiselect`, `forge-number-stepper`, `forge-otp-input`, `forge-phone-input`, `forge-radio-group`, `forge-search-input`, `forge-segment-control`, `forge-select`, `forge-time-input`, `forge-time-range-input` | visuel / `component.input`, `component.select`, `component.checkable` ou `component.field` selon le contrôle composé                               | `size`, `disabled`, accessoires de validation et de sélection ; par défaut/focus-visible/désactivé/développé/sélectionné/invalide |
| organismes | `forge-date-time-range-input`, `forge-form-builder`, `forge-form-wizard`, `forge-schema-form-dialog`, `forge-schema-form`                                                                                                                                                                                                                                                 | visuel / `component.field` + contrats de saisie/sélection/superposition composés                                                                   | schéma, étapes, validation ; par défaut/focus-visible/désactivé/développé/sélectionné/invalide                                    |

### Icônes — `packages/ui/icons/src/components/`

Les 106 entrées d'icônes sont **inherited-visual**. Les glyphes utilisent `currentColor` ; leur taille est contrôlée par le consommateur ou correspond à
...
est visuel et utilise `component.email`, avec `component.button`, `component.typography` ou `component.media` lorsque cela est indiqué.
...
assistants récursifs `components/organisms/forge-tree-view/forge-tree-view-item`,
...
L'aperçu partagé de Storybook charge `@mission-platform/tokens/scss/tokens`, le plug-in de remplacement de Storybook et le
`theme` global. Pour inspecter le contrat, définissez le thème global sur clair ou foncé et utilisez les contrôles des histoires des composants ;
pour tester les remplacements du consommateur, modifiez `apps/storybook/design-tokens/overrides.tokens.json` sous `component` à l'aide d'un
Valeur `{ "light": "...", "dark": "..." }`. Le schéma de remplacement est
...
Les feuilles suivantes sont intentionnellement limitées aux composants et peuvent également être remplacées sur un hôte de composant individuel
avec la propriété personnalisée CSS générée. Les valeurs de secours dans les composants composés conservent la valeur par défaut lorsqu'un hôte
ne définit pas de remplacement.

| Composant            | Chemin de remplacement DTCG                        | Modèle de variable CSS généré                          |
| -------------------- | -------------------------------------------------- | ------------------------------------------------------ |
...
1. Créez la collection de variables `Mission Platform / Component` avec les modes Clair et Sombre.
2. Importez les chemins des composants depuis l'arborescence source `component/<atomic-level>/`, en préservant le composant, la variante, l'emplacement,
   et les segments étatiques.

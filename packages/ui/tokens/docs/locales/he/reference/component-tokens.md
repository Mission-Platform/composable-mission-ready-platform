...
מתאמי המסגרת שנוצרו: אותו ערך חל על Vue, React, Solid, Svelte ו-Web Components.

## קריאת החוזה

מקור האמת הוא עץ המקור של הרכיב הרקורסי תחתיו
[`tokens/component/`](../../../../tokens/component), מקובצים לפי רמה אטומית
(`atoms/`, `molecules/`, `organisms/` ו-`templates/`). כל מקור נוצר באופן עצמאי, בעוד כל המקורות
לשמור על אותו חוזה יציב `component.*` DTCG:
...
נתיב ה-DTCG הוא גם נתיב העקיפת Figma וזמן הריצה; רק שם ה-CSS שנוצר מוריד את מעטפת `component`.
לדוגמה, `component.button.primary.background.hover` נפלט כ-`--mp-button-primary-background-hover`. א
מזהה מקור כגון `component/atoms/button` מזהה את הקובץ שבבעלותו החוזה, ולא נתיב DTCG חדש.
...
סיכות תת-עץ `color-scheme`, `light-dark()`, `[data-theme]` ו-`.theme-*`. צרכנים וספר סיפורים עשויים לעקוף כל
עלה למטה `component` ב-`overrides.tokens.json`; חלה דריסה לאחר גיליון הסגנונות האסימון שנוצר. עוקפים
המשך להשתמש במפתחות `component.*` למרות שמאפייני CSS מותאמים אישית משתמשים במרחב השמות של השכבות.

## פריסת המקור והפלט שנוצרה

לכל חוזה חזותי יש בעלים אחד מתחת לעץ המקור האטומי. המחולל מגלה קבצים חדשים באופן רקורסיבי, אז א
מקור חדש אינו דורש רישום מתאר:
...
חביות ה-SCSS וה-TypeScript שנוצרו כוללות כל מקור רכיב בסדר מזהה מקור דטרמיניסטי. רכיב
קבצים עשויים לעשות שימוש חוזר בחוזים משותפים כגון `button`, `field`, `input`, `navigation` ו-`overlay`; רכיבים מורכבים
אסור לשכפל את הנתיבים האסימונים האלה. נותרו רכיבי התנהגות בלבד, גליפים שעברו בירושה ונוסחאות פריסה/DOM
מחוץ לחוזה האסימון החזותי אלא אם ערך מלאי מקצה להם בעלות חזותית.
...
רק מדינות הנתמכות על ידי רכיב מופיעות למטה. `expanded` משמש לגילוי/בחירת משטחים, `selected`
עבור אפשרויות/כרטיסיות/ניווט, ו-`invalid` עבור אימות טופס; אין צורך במשתני מצב שאינם בשימוש.
...
המשטח שנוצר לאחר הביקורת מכיל **2,841 עלי אסימונים**: 132 פעילים, 2,161 מוגנים ו-548 מעורפלים;
לא נותרו מועמדים. הניקוי הסיר 189 עלים בלתי ניתנים להשגה בסך הכל: 185 המועמדים מה-
דוח סקירה בתוספת 4 דפי פלטה נטו מסדר שני (6 הוסרו, 2 שוחזרו כדפים `.500` שניתן להגיע אליהם) שנחשפו לאחר סגירת הכינוי. הפחתה זו משפיעה שנוצרה
ייצוא פרימיטיבי, סמנטי, טיפוגרפי ויצוא מבני בלבד; שמירת נתיבים `component.*` ושלהם
שמות `--mp-<layer>-*` ללא שינוי. שלושת הכינויים הלא פתורים (`color.surface.raised`, `radius.2xs` ו-
`font.weight.light`) לפני ביקורת זו ונותרה ללא שינוי.
...
כל כדור למטה הוא ערך מלאי אחד. אלא אם כן כתבה מסומנת `story: missing`, לרכיב יש התאמה
`<component>.stories.tsx` לצד המקור. כותרת חבילה/רמה מספקת את קידומת נתיב המקור היציבה.

## `@mission-platform/components`

### אטומים - `packages/ui/components/src/components/atoms/`
...
| `forge-icon-button`      | חזותי | `component.button.<variant>` + `component.icon` | תווית, `variant`, `size`; default/hover/active/focus-visible/disabled/loading               |
...
### מולקולות - `packages/ui/components/src/components/molecules/`
...
| `forge-collapse`          | חזותי        | `component.collapse`                         | פתוח, וריאנט, מושבת; ברירת מחדל/רחף/פוקוס-נראה/מורחב/מושבת                                  |
...
| `forge-language-switcher` | בירושה-חזותי | `component.navigation` + חוזה בחירת ילד      | מקומי; ברירת מחדל/מורחב/נבחר                                                                |
...
### אורגניזמים ותבניות - `packages/ui/components/src/components/{organisms,templates}/`
...
| `forge-menubar`            | חזותי        | `component.navigation.menubar`                          | פריטים, תוחמים, גודל; ברירת מחדל/רחף/פוקוס-נראה/מורחב/מושבת                                            |
...
| `forge-virtual-tree-view`  | חזותי        | `component.data.virtual-tree`                           | צמתים, גודל, itemHeight, גובה, סריקה יתרה, defaultOpen, renderer שורה; ברירת מחדל/רחף/פוקוס גלוי/מורחב |
| `forge-hero`               | חזותי        | `component.layout.hero`                                 | מדיה, יישור, גודל, שכבת-על; ברירת מחדל                                                                 |

## חבילות מיוחדות של Forge
...
### טפסים - `packages/ui/forms/src/components/`

כל הרשומות בטופס משתמשות בתפקידי התווית/עוזר/שגיאה המשותפים `component.field` בנוסף לחוזה שלהלן. יליד
מצבי שליטה מיוצגים רק כאשר השליטה תומכת בהם.

| רמה        | רכיבים (ערך אחד לכל שם מופרד בפסיקים)                                                                                                                                                                                                                                                                                                                                     | סיווג / חוזה                                                                                                      | אביזרי ומצבי הופעה משותפים                                                                        |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
...
### סמלים - `packages/ui/icons/src/components/`

כל 106 ערכי הסמלים הם **עובר בירושה**. גליפים משתמשים ב-`currentColor`; הגודל שלהם הוא בשליטת הצרכן או מפות ל
...
| `three/organisms`            | `forge-three-canvas`                                                                                                                               | בירושה-חזותי | `component.media`                                            | ממדי מארח בד הם מבניים; משטח בירושה                                                        |
| `typography/atoms`           | `forge-typography`                                                                                                                                 | חזותי        | `component.typography`                                       | גרסה, צבע, `as`; default/link/disabled                                                     |
...
הוא ויזואלי ומשתמש ב-`component.email`, עם `component.button`, `component.typography` או `component.media` היכן שצוין.
...
התצוגה המקדימה של Storybook המשותפת טוענת את `@mission-platform/tokens/scss/tokens`, את הפלאגין Storybook לעקוף, ואת
`theme` גלובלי. כדי לבדוק את החוזה, הגדר את הנושא הגלובלי לבהיר או כהה והשתמש בפקדים של סיפורי הרכיבים;
כדי לבדוק עקיפות צרכנים, ערוך את `apps/storybook/design-tokens/overrides.tokens.json` תחת `component` באמצעות
ערך `{ "light": "...", "dark": "..." }`. סכימת העקיפה היא
...
העלים הבאים כוללים רכיבים בכוונה וניתן לעקוף אותם גם על מארח רכיב בודד
עם מאפיין ה-CSS המותאם אישית שנוצר. ערכי החלפה ברכיבים מורכבים משמרים את ברירת המחדל כאשר הוא מארח
אינו מגדיר עקיפה.

| רכיב                 | נתיב עקיפת DTCG                                    | דפוס משתנה CSS שנוצר                                   |
| -------------------- | -------------------------------------------------- | ------------------------------------------------------ |
...
1. צור את אוסף המשתנים `Mission Platform / Component` עם מצבי אור וחושך.
2. ייבא את נתיבי הרכיבים מעץ המקור `component/<atomic-level>/`, שימור רכיב, וריאנט, חריץ,
   and state segments.

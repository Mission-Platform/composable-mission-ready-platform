...
import { ForgeMultiselect } from './forge-multiselect';

/**
 * Exercises the **neutral** `ForgeMultiselect` authored in this package,
 ...
 * `ForgeDropdown`, whose panel is mounted only while open, so the closed
 * (default) SSR render exposes the option labels through the always-present
 * native `<select multiple>` rather than a `role="listbox"`.
 */
const ReactMultiselect = toReactComponent(ForgeMultiselect, 'Multiselect');
const VueMultiselect = toVueComponent(ForgeMultiselect, 'Multiselect');
...
describe('ForgeMultiselect authors the same component for React and Vue', () => {

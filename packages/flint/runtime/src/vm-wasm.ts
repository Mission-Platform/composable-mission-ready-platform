...
import { validateFlintVmModule } from './vm-executor.js';
import { FLINT_VM_DEFAULT_MAX_MEMORY_PAGES, FLINT_VM_WASM_ABI_VERSION, FLINT_VM_WASM_LOWERING_VERSION } from './vm.js';

import type { FlintTraceOptions } from './trace.js';
import type {
...
function instructionRep(
  ...
  functions: ReadonlyMap<string, FlintVmFunction>,
): VmRep | undefined {
...
export function compileFlintVmWasm(
  ...
  const wasm = buildModule(module, options.maxMemoryPages ?? FLINT_VM_DEFAULT_MAX_MEMORY_PAGES);
  const artifact = {
    format: 'forge-web-script-vm-wasm' as const,
    moduleVersion: '1.0' as const,
    sourceHash: module.sourceHash,
    compilerVersion,
    loweringVersion: FLINT_VM_WASM_LOWERING_VERSION,
    abiVersion: FLINT_VM_WASM_ABI_VERSION,
...
function importValue(
  ...
  if (
    ...
    throw new FlintTrap('GuestTrap', 'VM WASM received an argument with an invalid type.');
...
export function prepareFlintVmWasm(
  ...
  const maximumPages = options.maxMemoryPages ?? FLINT_VM_DEFAULT_MAX_MEMORY_PAGES;
  const artifact =
    'wasm' in moduleOrArtifact
      ? options.maxMemoryPages === undefined
        ? moduleOrArtifact
        : compileFlintVmWasm(moduleOrArtifact.module, {
            compilerVersion: options.compilerVersion,
            maxMemoryPages: maximumPages,
          })
      : compileFlintVmWasm(moduleOrArtifact, {
          compilerVersion: options.compilerVersion,
          maxMemoryPages: maximumPages,
        });
  if (
    ...
    artifact.abiVersion !== FLINT_VM_WASM_ABI_VERSION ||
    artifact.loweringVersion !== FLINT_VM_WASM_LOWERING_VERSION
  )
    throw new FlintTrap('InvalidAbi', 'VM WASM artifact format or ABI version is unsupported.');
      ...
      try {
        ...
        observe(() =>
          activeTrace?.recordCapability(
            ...
            'allowed',
            steps,
            arguments_.map((value) => summarizeFlintVmValue(value, activeRedact)).join(','),
          ),
        );
        ...
        pendingTrap = error instanceof FlintTrap ? error : toFlintHostError(error, imported.capability);
  ...
  const imports: WebAssembly.Imports = {
    [RUNTIME_IMPORT_MODULE]: {
      ...
      [TRAP_IMPORT]: (functionIndex: number, instructionIndex: number) => {
        ...
        pendingTrap =
          instruction?.opcode === 'trap'
            ? new FlintTrap('GuestTrap', `${instruction.code}: ${instruction.message}`)
            : new FlintTrap('GuestTrap', 'VM WASM encountered an invalid trap instruction reference.');
        observe(() =>
  ...
  const execute = (
    ...
    executionOptions: Omit<FlintVmExecutionOptions, 'mode' | 'capabilities'> = {},
  ): FlintVmExecutionResult => {
    if (closed) throw new FlintTrap('InvalidAbi', 'VM WASM prepared executor is closed.');
    const function_ = artifact.module.functions.find((candidate) => candidate.name === functionName);
    if (function_ === undefined) throw new FlintTrap('GuestTrap', `Function '${functionName}' does not exist.`);
    if (arguments_.length !== function_.parameters.length)
      throw new FlintTrap('GuestTrap', `Function '${functionName}' received an invalid argument count.`);
    if (executionInProgress)
      throw new FlintTrap('InvalidAbi', 'VM WASM prepared executor does not support nested execution.');
    ...
    try {
      ...
      activeTrace =
        executionOptions.trace === undefined
          ? undefined
          : createFlintTraceRecorder(executionOptions.trace, functionName);
      ...
      if (pendingTrap === undefined && error instanceof FlintTrap) pendingTrap = error;
      if (pendingTrap === undefined)
        pendingTrap = new FlintTrap(
          'GuestTrap',
      ...
      if (activeTrace !== undefined && !traceFinished) {
        ...
        if (trapError !== undefined) attachFlintTrace(trapError, report);
      }
      try {
        (exports().fws_reset as () => void)();
      } finally {
...
export const prepareFlintVm = prepareFlintVmWasm;

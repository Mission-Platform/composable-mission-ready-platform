...
import { compileFlintVmWasm, prepareFlintVmWasm } from './vm-wasm.js';
import { createFlintVmExecutor } from './vm.js';

import type { FlintVmModule, FlintVmValue } from './vm.js';

const number = (value: number): FlintVmValue => ({ kind: 'number', type: 'i32', value });
const unsignedNumber = (value: number): FlintVmValue => ({ kind: 'number', type: 'u32', value });
const wideNumber = (value: bigint): FlintVmValue => ({ kind: 'number', type: 'i64', value });
...
describe('Forge Web Script VM WASM backend', () => {
  it('produces deterministic artifacts and matches numeric/control-flow execution', () => {
    const first = compileFlintVmWasm(module, { compilerVersion: 'test' });
    const second = compileFlintVmWasm(module, { compilerVersion: 'test' });
    expect(first).toEqual(second);
    const prepared = prepareFlintVmWasm(first);
  ...
  it('matches interpreter semantics for move, unary, jump, byte-at, and trap instructions', () => {
    const prepared = prepareFlintVmWasm(module);
    const interpreter = createFlintVmExecutor();
    const executeBoth = (functionName: string, arguments_: readonly FlintVmValue[], memory?: Uint8Array) => {
    ...
    const runTrap = (run: () => unknown): FlintTrap => {
      try {
        run();
      } catch (error) {
  ...
  it('binds declared capabilities and converts denied capabilities to traps', () => {
    const prepared = prepareFlintVmWasm(module, { capabilities: { now: () => number(7) } });
    expect(prepared.execute('clock', []).value).toEqual(number(7));
    const denied = prepareFlintVmWasm(module);
    expect(() => denied.execute('clock', [])).toThrowError(FlintTrap);
  });
  ...
  it('rejects malformed artifacts before instantiation', () => {
    const artifact = compileFlintVmWasm(module);
    expect(() => prepareFlintVmWasm({ ...artifact, wasm: new Uint8Array([0]) })).toThrowError(FlintTrap);
    expect(() => prepareFlintVmWasm({ ...artifact, reproducibilityHash: 'tampered' })).toThrowError(FlintTrap);
  });

  it('preserves linear-memory reads and max-step traps', () => {
    const prepared = prepareFlintVmWasm(module);
    expect(prepared.execute('memory', []).value).toEqual(number(40));
    expect(() => prepared.execute('memory', [], { maxSteps: 1 })).toThrowError(FlintTrap);
    prepared.reset();
    expect(prepared.execute('memory', []).value).toEqual(number(40));
  });
});

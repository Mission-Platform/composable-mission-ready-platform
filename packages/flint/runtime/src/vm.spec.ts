...
import { prepareFlintVmWasm } from './vm-wasm.js';
import {
...
import type { FlintVmModule, FlintVmValue } from './vm.js';

const number = (value: number): FlintVmValue => ({ kind: 'number', type: 'i32', value });

const module: FlintVmModule = {
...
const artifactModule: FlintVmModule = {
  ...module,
...
describe('Forge Web Script VM', () => {
  it('has equivalent interpreter, JIT, and AOT results', () => {
    const executor = createFlintVmExecutor({ compilerVersion: 'test', jitThreshold: 1 });
  ...
  it.each(['interpret', 'jit', 'aot'] as const)('constructs and reads variable-size bytes in %s mode', (mode) => {
    const result = runFlintVmBootstrap(artifactModule, 'bytes', [], mode);
  ...
  it('rejects artifact opcodes during WASM lowering and falls back consistently', () => {
    expect(() => prepareFlintVmWasm(artifactModule, { compilerVersion: 'test' })).toThrow(
      "VM WASM lowering failed: unsupported opcode 'alloc'",
    );

    const interpreted = createFlintVmExecutor({ compilerVersion: 'test' }).execute(
      artifactModule,
      'bytesArtifact',
      [],
      { mode: 'interpret' },
    );
    const aotExecutor = createFlintVmExecutor({ compilerVersion: 'test' });
    expect(() => aotExecutor.prepare(artifactModule, 'aot')).toThrow(
      "VM WASM lowering failed: unsupported opcode 'alloc'",
    );
    const aot = aotExecutor.execute(artifactModule, 'bytesArtifact', [], { mode: 'aot' });

    const jitExecutor = createFlintVmExecutor({ compilerVersion: 'test', jitThreshold: 2 });
  ...
  it.each(['interpret', 'jit', 'aot'] as const)(
    ...
    (mode) => {
      const result = runFlintVmBootstrap(artifactModule, 'aggregate', [], mode);
  ...
  it('executes the prepared WASM backend with reusable state', () => {
    const prepared = prepareFlintVmWasm(module, { compilerVersion: 'test' });
    const interpreted = createFlintVmExecutor().execute(module, 'add', [number(8), number(5)], {
      mode: 'interpret',
    });
  ...
  it('caches prepared JIT and AOT contexts and preserves mode labels', () => {
    const thresholdExecutor = createFlintVmExecutor({ compilerVersion: 'test', jitThreshold: 2 });
    ...
    const executor = createFlintVmExecutor({ compilerVersion: 'test', jitThreshold: 2 });
    ...
    const artifact = createFlintVmAotArtifact(module, 'test');
    ...
    expect(() =>
      executor.prepare(module, 'aot', {
        aotArtifact: createFlintVmAotArtifact(module, 'different-compiler'),
      }),
    ).toThrow('compiler version');
  });

  it('creates reproducible AOT artifacts and runs the bootstrap entrypoint', () => {
    const first = createFlintVmAotArtifact(module, 'seed-compiler');
    const second = createFlintVmAotArtifact(module, 'seed-compiler');
    const changed = createFlintVmAotArtifact({ ...module, sourceHash: 'changed' }, 'seed-compiler');
    expect(first).toEqual(second);
    expect(runFlintVmBootstrap(module, 'add', [number(1), number(2)], 'interpret').value).toEqual(number(3));
    expect(changed).not.toEqual(first);
    expect(() => createFlintVmExecutor().execute(module, 'clock', [], { mode: 'interpret' })).toThrowError(FlintTrap);
    expect(() =>
      executeFlintVmAotArtifact({ ...first, reproducibilityHash: 'tampered' }, 'add', [number(1), number(2)]),
    ).toThrowError(FlintTrap);
  });

  it('traps on maxSteps overrun', () => {
    const executor = createFlintVmExecutor();
    const arguments_ = [number(40), number(2)];
    expect(() => executor.execute(module, 'add', arguments_, { mode: 'interpret', maxSteps: 1 })).toThrow(FlintTrap);
  });

  it('accepts a valid JIT cache produced by the same executor', () => {
    const executor = createFlintVmExecutor({ compilerVersion: 'test-v1', jitThreshold: 1 });
    const arguments_ = [number(40), number(2)];
    ...
    const executor2 = createFlintVmExecutor({ compilerVersion: 'test-v1' });
  ...
  it('rejects JIT cache with mismatched compiler version', () => {
    const executor = createFlintVmExecutor({ compilerVersion: 'v1', jitThreshold: 1 });
    const arguments_ = [number(40), number(2)];
    executor.execute(module, 'add', arguments_, { mode: 'jit' });
    const cache = executor.getJitCache?.();

    // Executor with different version rejects the cache
    const executor2 = createFlintVmExecutor({ compilerVersion: 'v2' });
  ...
  it('rejects JIT cache with mismatched source hash', () => {
    ...
    const executor2 = createFlintVmExecutor({ compilerVersion: 'test' });
  ...
  it('produces deterministic, source-mapped traces with bounded events', () => {
    const tracedModule: FlintVmModule = {
      ...module,
      functions: module.functions.map((function_) =>
        function_.name === 'add'
          ? {
              ...function_,
              debugSpans: [{ instruction: 0, fileName: 'main.flint', start: 10, end: 20, line: 2, column: 3 }],
            }
          : function_,
      ),
    };
    ...
    const first = createFlintVmExecutor().execute(tracedModule, 'add', [number(40), number(2)], options);
    const second = createFlintVmExecutor().execute(tracedModule, 'add', [number(40), number(2)], options);
    ...
    expect(first.trace?.events[0]?.source).toMatchObject({ fileName: 'main.flint', line: 2 });
  });

  it.each(['interpret', 'jit', 'aot'] as const)('returns a forensic report on the %s path', (mode) => {
    const result = runFlintVmBootstrap(module, 'add', [number(40), number(2)], mode, {
  ...
  it('attaches bounded trap evidence without exposing raw memory', () => {
    try {
      createFlintVmExecutor().execute(module, 'add', [number(40), number(2)], {
        mode: 'interpret',
        maxSteps: 0,

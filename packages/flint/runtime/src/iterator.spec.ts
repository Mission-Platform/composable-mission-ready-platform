...
import { createFlintWasmIterator, createFlintWasmIteratorFactory } from './iterator.js';
...
describe('Forge Web Script WASM iterator adapter', () => {
  it('decodes packed i64 results and keeps completion stable', () => {
    ...
    const iterator = createFlintWasmIterator(3, () => values.shift() ?? 1n << 32n, { descriptor });
  ...
  it('adapts backend factories without leaking the numeric handle', () => {
    const create = createFlintWasmIteratorFactory(
      (start: number) => start,

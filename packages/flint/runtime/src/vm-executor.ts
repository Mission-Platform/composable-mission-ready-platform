import { FlintMemory } from './memory.js';
import { createFlintTraceRecorder, summarizeFlintVmValue } from './trace.js';
import { attachFlintTrace, toFlintHostError, FlintTrap } from './traps.js';
import { prepareFlintVmWasm } from './vm-wasm.js';
import { FLINT_VM_DEFAULT_MAX_MEMORY_PAGES } from './vm.js';
...
function isValue(value: unknown): value is FlintVmValue {
  if (value === null || typeof value !== 'object' || typeof (value as { kind?: unknown }).kind !== 'string')
    return false;
  const candidate = value as FlintVmValue;
...
function writeNumber(
  memory: FlintMemory,
  address: number,
  value: Extract<FlintVmValue, { readonly kind: 'number' }>,
): void {
...
function executeFunction(
  ...
  executeNamed: (name: string, arguments_: readonly FlintVmValue[], state: ExecutionState) => FlintVmValue,
): FlintVmValue {
  ...
  const registers = Array.from({ length: function_.registers }, () => ({ kind: 'unit' }) as FlintVmValue);
        ...
        try {
          observe(() =>
            state.trace?.recordCapability(
              ...
              'allowed',
              state.steps,
              capabilityArguments.map((value) => summarizeFlintVmValue(value, state.options.trace?.redact)).join(','),
            ),
          );
          ...
          throw toFlintHostError(error, imported.capability);
        }
        instructionPointer += 1;
        break;
      }
...
function createMemory(
  ...
  const memory = new FlintMemory(undefined, {
    initialPages,
    maximumPages: maxMemoryPages ?? FLINT_VM_DEFAULT_MAX_MEMORY_PAGES,
    trace,
  });
...
export function createFlintVmExecutor(executorOptions: FlintVmExecutorOptions = {}): FlintVmExecutor {
  ...
  const entries: Record<string, FlintVmJitEntry> = {};
  const validatedJitCaches = new WeakMap<FlintVmJitCache, WeakSet<FlintVmModule>>();
  const wasmFallbacks = new WeakMap<FlintVmModule, Set<Exclude<FlintVmExecutionMode, 'interpret'>>>();
  const preparedByModule = new WeakMap<
    FlintVmModule,
    Map<
      Exclude<FlintVmExecutionMode, 'interpret'>,
      {
        capabilities?: FlintVmExecutionOptions['capabilities'];
        maxMemoryPages?: number;
        prepared: FlintVmPreparedExecutor;
      }
    >
  >();
  const prepare = (
  ...
  const execute: FlintVmExecutor['execute'] = (module, functionName, arguments_, options) => {
    ...
    try {
      ...
      const outputMemory = new Uint8Array(memory.bytes);
      if (trace !== undefined) {
        const trapError = error instanceof FlintTrap ? error : undefined;
        observe(() =>
          trace.recordTrap(
...
export function executeFlintVm(
  ...
  return createFlintVmExecutor().execute(module, functionName, arguments_, options);
}
...
function validateAotArtifact(artifact: FlintVmAotArtifact, expectedCompilerVersion?: string): void {
  ...
  if (artifact.format !== 'forge-web-script-vm-aot' || artifact.moduleVersion !== '1.0')
    throw new FlintTrap('InvalidAbi', 'AOT artifact format or version is unsupported.');
  if (artifact.module.sourceHash !== artifact.sourceHash)
    throw new FlintTrap('InvalidAbi', 'AOT artifact source hash does not match its module.');
  if (expectedCompilerVersion !== undefined && artifact.compilerVersion !== expectedCompilerVersion)
    throw new FlintTrap('InvalidAbi', 'AOT artifact compiler version does not match the prepared executor.');
  if (artifact.reproducibilityHash !== aotHash(artifact))
    throw new FlintTrap('InvalidAbi', 'AOT artifact reproducibility hash does not match its contents.');
...
export function executeFlintVmAotArtifact(
  ...
  if (executor === undefined) {
    executor = createFlintVmExecutor({ compilerVersion: artifact.compilerVersion });
...
function validateJitCache(jitCache: FlintVmJitCache, module: FlintVmModule, executorCompilerVersion: string): void {
  ...
  for (const [name, entry] of Object.entries<FlintVmJitEntry>(jitCache.entries)) {
    if (entry.functionName !== name) {
      throw new FlintTrap('InvalidAbi', `JIT cache entry mismatch: ${name} !== ${entry.functionName}`);
    }
    if (entry.sourceHash !== module.sourceHash) {
      throw new FlintTrap('InvalidAbi', `JIT cache source hash mismatch: ${entry.sourceHash} !== ${module.sourceHash}`);
    }
  }
}

...
} from '../fixtures/valid/collections.flint';
import { abiManifest, load, loadSync, manifest } from '../fixtures/valid/scalar.flint';

import { defineFlintVitestConfig, flintVitestPlugin } from './vitest.js';
...
describe('Forge Web Script Vitest adapter', () => {
  it('imports the generated module contract and virtual artifact queries', async () => {
    const scalarManifestQuery = await import('../fixtures/valid/scalar.flint?flint-manifest');
    const scalarWasmQuery = await import('../fixtures/valid/scalar.flint?flint-wasm');
    const scalarSourceMapQuery = await import('../fixtures/valid/scalar.flint?flint-source-map');
  ...
  it('loads generated collection and enum artifacts with ordered scalar exports', async () => {
    const collectionManifestQuery = await import('../fixtures/valid/collections.flint?flint-manifest');
    const collectionWatQuery = await import('../fixtures/valid/collections.flint?flint-wat');
  ...
  it('loads graph declarations through the plugin graph path', async () => {
    const { graphMetadata } = await import('../fixtures/graphs/entry.flint?flint-declarations');
  ...
  it('preserves consumer Vite plugins and settings while composing the config', () => {
    ...
    const config = defineFlintVitestConfig({
      environment: 'node',
      overrides: {
  ...
  it('invalidates graph artifacts when a dependency changes in the Vite server lifecycle', async () => {
    const root = await mkdtemp(path.join(tmpdir(), 'flint-vitest-'));
    temporaryDirectories.push(root);
    const entry = path.join(root, 'entry.flint');
    const dependency = path.join(root, 'dependency.flint');
    await writeFile(
      entry,
      'import "./dependency.flint" as dependency;\nexport fn answer() -> i32 { return 42; }\n',
      'utf8',
    );
    await writeFile(dependency, 'export fn dependency() -> i32 { return 1; }\n', 'utf8');

    const plugin = flintVitestPlugin({ root });
    const server = await createServer({ root, appType: 'custom', plugins: [plugin] });
    try {
      const query = `${entry}?flint-source-map`;

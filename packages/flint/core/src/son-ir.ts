...
function graphIdentity(module: FlintSoNModule | Omit<FlintSoNModule, 'graphHash' | 'optimizationReport'>): string {
  const { graphHash: _graphHash, optimizationReport: _report, ...base } = module as FlintSoNModule;
  const sanitized = {
    ...base,
...
function sonAnnotateBounds(module: FlintIrModule): number {
  ...
  for (const function_ of module.functions) {
    ...
    function visitExpression(expression: FlintIrExpression): void {
    ...
    function visitStatement(statement: FlintIrStatement): void {
      switch (statement.kind) {
        case 'let': {

...
import { compileFlint, createFlintCompilerService } from './compiler.ts';
import { createFlintSelfHostedCompilerSourceManifest, prepareFlintSelfHostedCompilation } from './self-hosted.ts';
...
describe('Forge Web Script bootstrap compiler', () => {
  it('compiles and executes the stateful memory fixture through Wasm', () => {
    const fileName = path.join(import.meta.dirname, 'fixtures/stateful-memory.flint');
    const artifact = compileFlint({ ...input(readFileSync(fileName, 'utf8')), fileName });
  ...
  it('compiles the package-local file fixture through the artifact contract', () => {
    const fileName = path.join(import.meta.dirname, 'fixtures/file-compile.flint');
    const artifact = compileFlint({ ...input(readFileSync(fileName, 'utf8')), fileName });
  ...
  return left + right;
    ...
    const first = compileFlint(input(source));
    const second = compileFlint(input(source));
...
`),
  ...
  it('releases the guest output before propagating a fatal UTF-8 decode error', async () => {
    const artifact = compileFlint(input('export fn corrupt(value: string) -> string { return value; }'));
    expect(artifact.diagnostics).toEqual([]);
  ...
  it('releases packed string inputs when the guest traps', async () => {
    const artifact = compileFlint(input('export fn trap(value: string) -> i32 { return string_byte_at(value, 1); }'));
    expect(artifact.diagnostics).toEqual([]);
...
export fn invoke(value: bytes) -> bytes { return reverse(value); }`,
  ...
  it('keeps generated bytes exports raw and exposes linear memory', async () => {
    const artifact = compileFlint(input('export fn roundTrip(value: bytes) -> bytes { return value; }'));
    const generated = await import(`data:text/javascript,${encodeURIComponent(artifact.esmSource)}`);
  ...
  it('does not emit string marshalling helpers for scalar-only exports', () => {
    const artifact = compileFlint(input('export fn answer() -> i32 { return 42; }'));
  ...
  it('returns callable guest exports from both generated loaders', async () => {
    const artifact = compileFlint(input('export fn answer() -> i32 { return 42; }'));
    expect(artifact.diagnostics).toEqual([]);
    expect(artifact.esmSource).toContain('Flint export');
  ...
  it('rejects undeclared capabilities before generating a loader', () => {
    ...
    expect(artifact.diagnostics).toEqual(expect.arrayContaining([expect.objectContaining({ code: 'FLINT-ABI-002' })]));
...
`);
  ...
  it('does not emit an artifact when diagnostics are present', () => {
    const artifact = compileFlint(input('export fn f() -> i32 { return true; }'));
    expect(artifact.wasm).toBeUndefined();
    expect(artifact.esmSource).toBe('');
    expect(artifact.diagnostics.map((diagnostic) => diagnostic.code)).toContain('FLINT-TYPE-005');
  });
...
export iter fn one() -> Iterator<i32> { yield 1; }`;
    const artifact = compileFlint(input(source));
  ...
  it('tracks service cache hits and invalidation, then rejects use after disposal', () => {
    ...
    service.invalidate(['test.flint']);
    expect(service.report().invalidatedFiles).toEqual(['test.flint']);
    service.compile(request);
  ...
  it('fails the tooling artifact when the bounded self-hosted stage loses parity', () => {
    ...
    expect(artifact.wasm).toBeUndefined();
    expect(artifact.diagnostics).toMatchObject([{ code: 'FLINT-BOOTSTRAP-001', phase: 'lex' }]);
  ...
  it('exposes a checked-in FWS compiler pipeline and a fixed-point seed fingerprint', () => {
    const stages = createFlintSelfHostedCompilerSourceManifest();
    ...
    expect(stages.flatMap(({ name, source }) => validateFlint(source, `${name}.flint`).diagnostics)).toEqual([]);
    const request = {
      source: 'export fn answer() -> i32 { return 42; }',
      fileName: 'fixed-point.flint',
      compilerVersion: '0.1.0',
    };
    const first = prepareFlintSelfHostedCompilation(request);
    const second = prepareFlintSelfHostedCompilation(request);

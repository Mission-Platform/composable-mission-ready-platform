...
function readExpression(reader: BinaryReader): FlintExpression {
  ...
  switch (kind) {
    case 'literal': {
      const type = reader.string() as Extract<FlintExpression, { kind: 'literal' }>['type'];
    ...
    case 'binary': {
      const operator = reader.string() as Extract<FlintExpression, { kind: 'binary' }>['operator'];
    ...
    case 'unary': {
      const operator = reader.string() as Extract<FlintExpression, { kind: 'unary' }>['operator'];
...
function readFunction(reader: BinaryReader): FlintFunction {
  ...
  const inlinePolicy = reader.optionalString() as FlintFunction['inlinePolicy'] | undefined;

 ...
 * `FlintModule` payload byte-for-byte compatible with
 * {@link ./stage-codec.ts}'s `encodeFlintSelfHostedModule` framing.
 *
 ...
 * is recognized and rejected with the same `FLINT-PARSE-052` diagnostic the seed
...
export const FLINT_PARSER_MODULE_STAGE_ENTRY = 'parse_module_stage';
const SOURCE_LAYOUT = 'FlintSourceBytes';
...
/** Token peek kinds (distinct from `FlintTokenKind`; internal to this stage). */
const PK_EOF = 0;
const PK_ALPHA = 1;
const PK_NUMBER = 2;
const PK_OP = 3;
 ...
 * `deriveFlintModuleId` does for bare, extension-suffixed file names
 ...
 */
export function createFlintParserModuleVmModule(
  sourceHash: string,
  fileName: string,
  ...
  const functions: FlintSelfHostedVmFunction[] = [
    buildReadByte(pool),
    buildIsAlpha(pool),
    buildIsDigit(pool),
    buildIsAlnum(),
  ...
  if (saltXor !== 0) {
    ...
    if (magicIndex !== undefined) {
      const original = pool.values[magicIndex] as Extract<FlintSelfHostedVmValue, { readonly kind: 'aggregate' }>;
  ...
  const bytesBlobLayout: FlintAggregateLayout = {

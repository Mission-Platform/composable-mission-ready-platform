...
export const FLINT_SELF_HOSTED_STAGE_ARTIFACT_FORMAT = 'forge-web-script-self-hosted-stage' as const;
...
const diagnosticPhases = new Set<FlintDiagnosticPhase>(['lex', 'parse', 'type-check', 'abi', 'graph', 'link', 'emit']);
...
function readDiagnostic(reader: BinaryReader): FlintDiagnostic {
  ...
  const diagnostic = {
    code,
    severity: severity as FlintDiagnosticSeverity,
    phase: phase as FlintDiagnosticPhase,
    message,
    fileName,
    span,
    ...(hint.length === 0 ? {} : { hint }),
...
export function decodeFlintSelfHostedStageArtifact(
  ...
  reader.done();
  if (schemaVersion !== FLINT_SELF_HOSTED_STAGE_SCHEMA_VERSIONS[stage]) invalid('stage schema version is unsupported');
  ...
  if (diagnosticLength > 0) decodeFlintSelfHostedDiagnostics(diagnosticPayload);
  return {
    format: FLINT_SELF_HOSTED_STAGE_ARTIFACT_FORMAT,
    version: FLINT_SELF_HOSTED_STAGE_ARTIFACT_VERSION,
    stage,

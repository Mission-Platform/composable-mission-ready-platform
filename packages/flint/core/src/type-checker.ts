        ...
        if (tag === undefined) {
          ...
          diagnostics.push(
            createDiagnostic(
              ...
              'type-check',
              'FLINT-TYPE-016',
              `Duplicate switch case '${arm.value}'.`,
              arm.span,
            ),
          );
        } else {
    ...
    if (callable === undefined) {
      diagnostics.push(
        createDiagnostic(
          ...
          locals.has(expression.callee) ? 'FLINT-TYPE-023' : 'FLINT-ABI-004',
...
function validateType(
  ...
  if (!primitiveTypes.has(baseType as FlintPrimitiveType) && !builtIn.has(baseType) && !declared && !generic)
    diagnostics.push(
      createDiagnostic(
        fileName,
        'type-check',
...
function parseCheckerTypeKey(type: string): ReturnType<TypeAlgebra['fromAst']> | undefined {
  ...
  const parse = (value: string): FlintTypeName | undefined => {
    ...
    if (genericStart !== -1 && trimmed.endsWith('>')) {
      ...
      const primitive = primitiveTypes.has(name as FlintPrimitiveType)
        ? (name as FlintPrimitiveType)
        : ('unit' as const);
      return {
        kind: 'type-name',
        name: primitive,
    ...
    const primitive = primitiveTypes.has(trimmed as FlintPrimitiveType)
      ? (trimmed as FlintPrimitiveType)
      : ('unit' as const);
    return {
      kind: 'type-name',
      name: primitive,
...
function builtInEnumFields(
  ...
  const typeArguments = aggregateType?.startsWith(`${enumName}<`)
    ...
    : [];
  const span = { start: 0, end: 0, line: 1, column: 1, endLine: 1, endColumn: 1 };
  const field = (index: number): FlintParameter => ({
    kind: 'parameter',
    name: 'value',
    type: { kind: 'type-name', name: (typeArguments[index] ?? 'unit') as FlintPrimitiveType, span },
    span,
  });

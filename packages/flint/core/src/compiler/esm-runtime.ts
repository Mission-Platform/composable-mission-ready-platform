import type { FlintHostImport } from '../manifest.js';
...
function checkedBytes(memory, pointer, length) {
  if (!Number.isSafeInteger(pointer) || pointer < 0 || !Number.isSafeInteger(length) || length < 0) {
    throw new RangeError('Flint string range is not a non-negative safe integer pair.');
  }
  const buffer = memory.buffer;
  if (pointer > buffer.byteLength || length > buffer.byteLength - pointer) {
    throw new RangeError('Flint string range [' + pointer + ', ' + length + '] is outside linear memory.');
  }
  return new Uint8Array(buffer, pointer, length);
}
function arraySnapshot(value) {
  if (value === null || (typeof value !== 'object' && typeof value !== 'function')) {
    throw new TypeError('Flint Array<i32> argument must be ArrayLike<number>.');
  }
  const length = value.length;
  if (!Number.isSafeInteger(length) || length < 0) {
    throw new RangeError('Flint Array<i32> length must be a non-negative safe integer.');
  }
  if (length > 0x3ffffffe || (length + 1) * 4 > 0xffffffff) {
    throw new RangeError('Flint Array<i32> is too large.');
  }
  const values = new Int32Array(length);
  for (let index = 0; index < length; index += 1) {
    const element = value[index];
    if (!Number.isInteger(element) || element < -0x80000000 || element > 0x7fffffff) {
      throw new TypeError('Flint Array<i32> element at index ' + index + ' must be a signed 32-bit integer.');
    }
...
`
  ...
  const range = pair(runtime, value, 'Flint string result is not a pointer-length pair.');
...
${
  ...
  if (layout === undefined) throw new TypeError('Unknown Flint record: ' + name);
...
function encodeRecord(wasmExports, value, name) {
  const layout = recordLayouts[name];
  if (value === null || typeof value !== 'object') throw new TypeError('Flint record argument must be an object.');
  if (layout === undefined) throw new TypeError('Unknown Flint record: ' + name);
...
`
  ...
  for (let index = 0; index < metadata.parameters.length; index += 1) {
    ...
    stringCount += 1;
    if (typeof args[index] !== 'string') throw new TypeError('Flint string argument must be a JavaScript string.');
    const bytes = encoder.encode(args[index]);
    encoded.push(bytes);
    total += bytes.byteLength;
    if (!Number.isSafeInteger(total) || total > 0xffffffff) throw new RangeError('Flint string input is too large.');
  }
  ...
  try {
    ...
    for (let index = 0; index < metadata.parameters.length; index += 1) {
      ...
      if (parameter.type === 'bytes') {
        ...
        if (!Array.isArray(value) || value.length < 2) throw new TypeError('Flint bytes value is not a pointer-length pair.');
...
function invokeCapability(hostFunction, metadata, args, runtime) {
  ...
  for (const type of metadata.parameters) {
    if (type === 'string') {
      const value = pair(runtime, [args[rawIndex], args[rawIndex + 1]], 'Flint string import argument is not a pointer-length pair.');
      hostArgs.push(decoder.decode(checkedBytes(runtime.wasmExports.memory, value[0], value[1])));
      rawIndex += 2;
    } else if (type === 'bytes') {
      const value = pair(runtime, [args[rawIndex], args[rawIndex + 1]], 'Flint bytes import argument is not a pointer-length pair.');
      hostArgs.push(value);
      rawIndex += 2;
    } else {
  ...
  if (metadata.result === 'string') {
    if (typeof result !== 'string') throw new TypeError('Flint string capability result must be a JavaScript string.');
  ...
  if (metadata.result === 'bytes') return pair(runtime, result, 'Flint bytes capability result is not a pointer-length pair.');
  return result;
}

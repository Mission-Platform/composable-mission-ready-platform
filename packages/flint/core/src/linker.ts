...
import type { FlintLinkConfiguration, FlintModuleEdge, FlintModuleGraph } from './graph.js';
...
function namespacePrivateFunctions(
  ...
  const expression = (value: FlintExpression): FlintExpression => {
    switch (value.kind) {
      case 'call': {
        return {
          ...value,
  ...
  const statements = (values: readonly FlintStatement[]): FlintStatement[] =>
    values.map((value) => {
      switch (value.kind) {
        case 'let': {

...
function typeCheckGeneratedConsumer(declarations: string, includeRawConsumer = false): readonly string[] {
  const rawConsumer = includeRawConsumer
    ? `
async function useRawRuntime(): Promise<FlintRawExports> {
  const imports: FlintRawImports = {};
  const exports: FlintRawExports = await loadRaw(imports);
  const syncExports: FlintRawExports = loadRawSync(imports);
  const currentTime: () => bigint = exports.currentTime;
  const syncCurrentTime: () => bigint = syncExports.currentTime;
  const echo: (valuePointer: number, valueLength: number) => FlintBytes = exports.echo;
  const syncEcho: (valuePointer: number, valueLength: number) => FlintBytes = syncExports.echo;
  const roundTrip: (valuePointer: number, valueLength: number) => FlintBytes = exports.roundTrip;
...
import library, {
  ...
  type FlintImports,
  type FlintManifest,
  type FlintRawExports,
  type FlintRawImports,
} from 'generated-runtime';

const imports: FlintImports = {
  'clock.now': { now: () => 1n },
  'text.transform': { transform: (value: string): string => value },
  'codec.bytes': { reverse: (value: FlintBytes): FlintBytes => value },
};

async function useRuntime(): Promise<bigint> {
  const exports: FlintExports = await load(imports);
  const syncExports: FlintExports = loadSync(imports);
  const libraryExports: FlintExports = library(imports);
  const currentTime: () => bigint = exports.currentTime;
  const syncCurrentTime: () => bigint = syncExports.currentTime;
...
function inspectManifest(value: FlintManifest): void {
...
describe('Forge Web Script generated type contracts', () => {
  it('generates FlintExports and FlintImports that are valid TypeScript', () => {
...
`,
    ...
    expect(artifact.declarations).toContain('readonly roundTrip: (value: FlintBytes) => FlintBytes;');
    ...
    expect(artifact.declarations).toContain('export interface FlintImports');
    expect(artifact.declarations).toContain('export interface FlintRawExports');
    expect(artifact.declarations).toContain('export type FlintRawImports = WebAssembly.Imports;');
    expect(artifact.declarations).toContain('export interface FlintManifest');
    expect(artifact.declarations).toContain('export interface FlintAggregateLayout');
    expect(artifact.declarations).toContain('export const abiManifest: FlintManifest;');
    expect(artifact.declarations).toContain('readonly "clock.now": {');
    expect(artifact.declarations).toContain('readonly now: () => bigint;');
    expect(artifact.declarations).toContain('load(imports?: FlintImports): Promise<FlintExports>');
    expect(artifact.declarations).toContain('loadSync(imports?: FlintImports): FlintExports');
    ...
    expect(artifact.declarations).toContain('loadRaw(imports?: FlintRawImports): Promise<FlintRawExports>');
    expect(artifact.declarations).toContain('loadRawSync(imports?: FlintRawImports): FlintRawExports');
  });
  ...
  it('generates valid TypeScript that can be parsed and type-checked', () => {
    ...
    // 1. FlintExports interface with exported functions
    // 2. FlintImports interface for capability imports
    // 3. load() function signature
    const decl = artifact.declarations;
    expect(decl).toMatch(/export interface FlintExports/);
    ...
    expect(decl).toMatch(/export interface FlintImports/);
    expect(decl).toMatch(/export function load\(imports\?: FlintImports\): Promise<FlintExports>/);
    expect(decl).toMatch(/export function loadSync\(imports\?: FlintImports\): FlintExports/);
...
export fn echo(value: string) -> string { return value; }`,
    ...
    expect(artifact.diagnostics).toEqual([]);
    expect(artifact.declarations).toContain('readonly process: (data: FlintBytes) => FlintBytes;');
    expect(artifact.declarations).toContain('readonly echo: (value: string) => string;');
  });
...
export fn scanArray(values: Array<i32>) -> i32 { return values[0]; }`,
    ...
    expect(artifact.declarations).toContain(
      'readonly process: (dataPointer: number, dataLength: number) => FlintBytes;',
    );
    expect(artifact.declarations).toContain(
      'readonly echo: (valuePointer: number, valueLength: number) => FlintBytes;',
    );
...
`,
    ...
    expect(artifact.declarations).toContain('readonly encode: (payload: string) => FlintBytes;');
    expect(artifact.declarations).toContain('readonly "codec.bytes": {');
    expect(artifact.declarations).toContain('readonly reverse: (value: FlintBytes) => FlintBytes;');
  });

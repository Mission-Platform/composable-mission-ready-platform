...
import type { FlintSoNModule, FlintSoNNode } from './son-ir.js';
...
export function validateFlintSoN(value: unknown, expected?: FlintSoNIdentity): value is FlintSoNModule {
  try {
    ...
    const candidate = value as Partial<FlintSoNModule>;
    if (
    ...
    if (
      ...
      return false;
    const nodes = candidate.nodes as readonly FlintSoNNode[];
    ...
    return flintSoNGraphHash(candidate as FlintSoNModule) === candidate.graphHash;
  } catch {
    return false;
  }
}

...
import { validateFlint } from '../validate.js';
...
describe('Forge iterator-first control flow and string helpers', () => {
  ...
  it('accepts imperative while loops with a deterministic AST and IR shape', () => {
    const result = validateFlint(source, 'loop.flint');
  ...
  it('keeps while loops in AST and IR executable paths', () => {
    const parsed = parseFlint(source, 'loop.flint');
  ...
  return value;
    ...
    expect(result.diagnostics.map(({ code }) => code)).toEqual(['FLINT-TYPE-005', 'FLINT-TYPE-005']);
  });

  it('compiles a source module containing a while loop', () => {
    const artifact = compileFlint({ source, fileName: 'loop.flint', compilerVersion: 'test' });
  ...
  return total;
    ...
    const validation = validateFlint(source, 'control-flow.flint');
    expect(validation.valid).toBe(false);
    expect(validation.diagnostics.map(({ code }) => code)).toEqual(['FLINT-PARSE-076']);

    const parsed = parseFlint(source, 'control-flow.flint');
    expect(parsed.diagnostics.map(({ code }) => code)).toEqual(['FLINT-PARSE-076']);
    ...
    expect(artifact.diagnostics.map(({ code }) => code)).toEqual(['FLINT-PARSE-076']);
...
export fn toNumber(value: string) -> i32 {
  ...
  return string_concat(result, "");
      ...
      fileName: 'string-loop-accumulator.flint',
      compilerVersion: 'test',
    });
    expect(artifact.diagnostics).toEqual([]);
    const exports = new WebAssembly.Instance(new WebAssembly.Module(artifact.wasm!), {})
...
export fn wrappedDigits(value: string) -> string {
  return string_concat(digits(value), "");
      ...
      fileName: 'string-digit-filter.flint',
      compilerVersion: 'test',

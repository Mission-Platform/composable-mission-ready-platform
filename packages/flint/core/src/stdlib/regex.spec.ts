...
describe('Forge regex standard-library contract', () => {
  it('type-checks compiler-owned calls without capabilities', () => {
    const result = validateFlint(VALID_SOURCE, 'regex.flint');
  ...
  return regex_search_capture_start(pattern, value, start, group);
    ...
    const result = validateFlint(source, 'captures.flint');
    expect(result.valid).toBe(true);
    expect(result.diagnostics).toEqual([]);
  });
  ...
  return regex_full_match(pattern);
    ...
    const result = validateFlint(source, 'invalid.flint');
    expect(result.valid).toBe(false);
    expect(result.diagnostics.map(({ code }) => code)).toContain('FLINT-TYPE-003');
  });
...
export fn captureEnd(value: string, group: i32) -> i32 {
  ...
  return regex_full_match(pattern, value);
    ...
    expect(() => compileFlint({ source, fileName: 'dynamic-pattern.flint', compilerVersion: 'test' })).not.toThrow();
    const artifact = compileFlint({ source, fileName: 'dynamic-pattern.flint', compilerVersion: 'test' });
    expect(artifact.wasm).toBeUndefined();
    expect(artifact.diagnostics).toHaveLength(1);
    expect(artifact.diagnostics[0]?.code).toBe('FLINT-EMIT-001');
    expect(artifact.diagnostics[0]?.message).toContain('FLINT-REGEX-004');
  });
});

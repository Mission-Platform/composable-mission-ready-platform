...
Flint (`.flint`) is a small, general-purpose language for WebAssembly
...
authoritative v1 language and module contract. `@mission-platform/flint`
...
and LSP. `@mission-platform/flint-wasm` is the deterministic backend
that lowers checked IR to validated WebAssembly and WAT. The Node-only
`@mission-platform/flint-cli` package provides the `flint`
...
The source format is UTF-8 text with the `.flint` extension. A source file is a
...
## Source analysis and release policy

The core package exposes one analysis contract for the compiler, language
service, CLI, and MCP integrations. `analyzeFlint` accepts the checked
frontend result and optional registered rules, then returns facts, findings, and
the same stable diagnostics used by the rest of the compiler. Analysis context
includes source files, optional source-map entries, raw and optimized IR, the
ABI manifest, graph/link metadata, the target profile, and normalized policy.

Analysis findings use stable `FLINT-ANALYSIS-*` codes and include a category,
severity, UTF-16-compatible source span, evidence, remediation hint, and
optional OWASP/CWE references. Their diagnostics add `phase: "analysis"` and
security metadata without changing existing `FLINT-LEX-*`, `FLINT-PARSE-*`,
`FLINT-TYPE-*`, or `FLINT-ABI-*` diagnostics.

Compilation uses the strict profile by default. In strict mode, error-severity
findings (or findings explicitly marked `blocking`) prevent Wasm and ESM output;
the complete report remains available on the returned artifact. The development
profile is intended for editor and investigation workflows: it reports findings
but does not use them as a release gate. Policy includes an explicit capability
allow-list and bounded limits for findings, call depth, loops, allocations, async
tasks, and regular-expression input.

Compiler-service cache keys include the normalized analysis policy, registered
rule identifiers, and source-map input. Changing any of these analysis inputs
therefore cannot reuse an artifact produced under a different policy.

## Exception-free outcomes and structured control flow

Flint represents recoverable outcomes with the standard-library
...
- `linkProfile: "dynamic"`: Explicit source-module boundaries are preserved.
  `FlintDynamicLinkCache` is used to resolve decoder modules at runtime,
...
[`src/grammar/flint.ebnf`](../../src/grammar/flint.ebnf).
...
scanner consumes a bounded region, emits the stable `FLINT-LEX-*` diagnostic, and
...
diagnostic `FLINT-LEX-003` and parser recovery remains available for the rest of
the source.
...
interface FlintDocumentationTag {
...
Flint provides a deterministic regular-expression standard library.
...
The shared package `@mission-platform/flint-regex` owns the stable
...
Production regex execution belongs to the Flint backend and the
...
`FLINT-PARSE-052`; immutable structs, tagged enums, interfaces, and function
...
There is no nested `module` declaration. Every `.flint` file is a module and its
stable name is derived from its normalized file ID. For example,
`src/time.flint` in project `/workspace/app` has module ID `src/time`. Nested
...
Flint uses a WASI-inspired _logical_ boundary, not a claim of full
...
| Flint             | WebAssembly representation                 |
| ----------------- | ------------------------------------------ |
...
`packages/flint/core/src/fixtures/stateful-memory.flint` is the conformance
...
```ts
type FlintBytes = readonly [pointer: number, length: number];
...
interface FlintExports {
  ...
  readonly processBytes: (value: FlintBytes) => FlintBytes;
}
```
...
callers continue to pass and receive `FlintBytes`, and `memory` is
...
`@mission-platform/flint-wasm`. The backend validates its bytes before
...
For filesystem workflows, install `@mission-platform/flint-cli` and
use its standalone `flint` binary:

```text
flint check <entry.flint> [--root <directory>] [--project-root <directory>]
flint compile <entry.flint> --out-dir <directory>
...
Use `@mission-platform/flint-vitest` when a Vitest suite needs to
...
`defineFlintVitestConfig` helper installs the production
`forgeWebScriptPlugin` while preserving consumer Vite plugins and settings.
See [Testing in Mission Platform](../../../../docs/testing.md#flint-tests) for the
...
```ts
const exports = await harness.load<{ current: () => bigint }>('capabilities/clock-now.flint', {
  'clock.now': { now: () => 123n },
});
```

Missing declared imports and undeclared supplied imports are failures. Test
projects that import `.flint` or its virtual artifact queries should add the
type-only declaration subpath
`@mission-platform/flint-vitest/flint` to their
TypeScript `types` list or a referenced test type entrypoint.

The shared harness fixtures under
`packages/flint-vitest/fixtures/` are the cross-package corpus for
...
| Code family     | Meaning                                                                                   |
| --------------- | ----------------------------------------------------------------------------------------- |
| `FLINT-LEX-*`   | invalid characters/escapes, raw string line terminators, or unterminated strings/comments |
| `FLINT-PARSE-*` | invalid module, declaration, statement, or expression syntax                              |
| `FLINT-TYPE-*`  | invalid primitive type, name, operator, argument, or return                               |
| `FLINT-ABI-*`   | duplicate names, denied capabilities, exports, or imports                                 |
...
`packages/flint/core/src/fixtures/bootstrap.ts` is the executable
...
`packages/flint-vitest/fixtures` covers the same boundary:
`valid/collections.flint` exercises collection literals, indexing, contextual
empty vectors, `length()`, and valid escaped strings;
`valid/aggregates.flint` exercises function values, qualified `Result::Ok` and
`Result::Error` constructors, and arm-local match bindings; and
`diagnostics/collections.flint` exercises invalid collection calls and aggregate
...
period; Flint does not reinterpret or replace them.
...
Flint is the production target for the neutral
`@mission-platform/code-scanner` artifact. Its scanner graph statically links
the QR, matrix, and barcode decoder sources into one self-contained WebAssembly
artifact; the dynamic profile keeps those source-module boundaries explicit and
caches resolved exports. The Rust `code-scan` crate remains available as a
native/reference implementation and is not a runtime dependency of the package.
The public QR, matrix, and barcode packages retain their own typed wrappers;
those APIs are not silently redirected through the scanner graph.

The `codecMigrationFixture` in
`packages/flint/core/src/fixtures/codec-migration.ts` is the first
...
fake for conformance tests while the fixture proves the Flint
...
| ---------------------------------------------------------------------- | ------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| Existing QR or matrix package behavior                                 | `@mission-platform/qr-code` / `@mission-platform/matrix-code` | Package-specific typed ESM wrappers remain available for those public APIs.                                       |
| Neutral image and camera scanner behavior                              | `@mission-platform/code-scanner`                              | Uses a statically linked FWS graph by default, or an explicit dynamic source-module profile with cached dispatch. |
| Existing barcode behavior                                              | `@mission-platform/barcode`                                   | Package-local Flint graphs provide the typed barcode façade.                                                      |
| New general-purpose browser-safe compute with explicit host effects    | Flint plus `@mission-platform/vite-plugin-flint`              | Versioned `.flint` source, manifest, typed loader, and deny-by-default capabilities.                              |
...
| Framework-neutral UI/component compilation                             | Forge component compiler                                      | Flint is not a replacement for `FrameworkOutputPlugin` or component targets.                                      |

Use the Flint Vite plugin only for `.flint` entries. Use the
AssemblyScript plugin for existing AssemblyScript entries. During migration, an
application may bundle both kinds of module: each loader owns its own
initialization, memory, and ABI validation, and capability imports must be
supplied explicitly to Flint modules.
...
evidence. The existing barcode wrapper and decoder package tests remain the
behavior and legacy regression oracle; run them alongside the fixture rather
than treating the fixture as a replacement benchmark. Forge Web
...
The VM bytecode contract is backend-independent. A `FlintVmModule`
...
`FlintSelfHostedStageReport` rather than being presented as complete
self-hosting.

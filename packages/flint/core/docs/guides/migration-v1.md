...
```bash
flint compile src/main.flint --bounds-checks runtime --format json
```
...
```bash
flint check src/main.flint --bounds-checks proven-safe --optimizer-report
```
...
```bash
flint inspect-sonir .cache/main.sonir.json --format json
```

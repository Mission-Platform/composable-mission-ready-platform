...
このガイドは、Flint パーサーを変更する寄稿者を対象としています。
契約、または適合備品。
...
公開する前に `pnpm --filter @mission-platform/flint build` を実行します。
...
Node 固有のツールは `@mission-platform/flint-cli` に属します。

## 生成されたアーティファクトとソース アーティファクト

`src/self-hosted/fws/` の下にチェックインされた `.flint` ソースはソース アーティファクトです。

...
Flint (`.flint`) è un piccolo linguaggio generico per WebAssembly
...
contratto di linguaggio e modulo v1 autorevole. `@mission-platform/flint`
...
e LSP. `@mission-platform/flint-wasm` è il backend deterministico
che abbassa l'IR controllato a WebAssembly e WAT convalidati. Solo Node
Il pacchetto `@mission-platform/flint-cli` fornisce `flint`
...
Il formato di origine è testo UTF-8 con estensione `.flint`. Un file sorgente è un file
...
integrazioni di servizi, CLI e MCP. `analyzeFlint` accetta il segno di spunta
...
I risultati dell'analisi utilizzano codici `FLINT-ANALYSIS-*` stabili e includono una categoria,
...
metadati di sicurezza senza modificare `FLINT-LEX-*`, `FLINT-PARSE-*` esistenti,
Diagnostica `FLINT-TYPE-*` o `FLINT-ABI-*`.
...
Flint rappresenta risultati recuperabili con la libreria standard
...
- `linkProfile: "dynamic"`: i limiti espliciti del modulo di origine vengono conservati.
  `FlintDynamicLinkCache` viene utilizzato per risolvere i moduli decoder in fase di esecuzione,
...
lo scanner consuma una regione delimitata, emette la diagnostica stabile `FLINT-LEX-*` e
...
la diagnostica `FLINT-LEX-003` e il ripristino del parser rimangono disponibili per il resto
la fonte.
...
Flint fornisce una libreria standard deterministica di espressioni regolari.
...
e ancoraggi `^`/`Flint fornisce una libreria standard deterministica di espressioni regolari.
Le chiamate`regex_full_match(pattern, value) -> bool`,
...
Il pacchetto condiviso `@mission-platform/flint-regex` possiede la scuderia `$`
...
L'esecuzione di regex di produzione appartiene al backend Flint e al file
...
`FLINT-PARSE-052`; strutture immutabili, enumerazioni con tag, interfacce e funzioni
...
Non esiste alcuna dichiarazione `module` nidificata. Ogni file `.flint` è un modulo e il suo
il nome stabile deriva dal suo ID file normalizzato. Ad esempio,
`src/time.flint` nel progetto `/workspace/app` ha l'ID modulo `src/time`. Nidificato
...
Flint utilizza un confine _logico_ ispirato a WASI, non una pretesa di pieno
...
`packages/flint/core/src/fixtures/stateful-memory.flint` è la conformità
...
i chiamanti continuano a passare e ricevere `FlintBytes` e `memory` è
...
`@mission-platform/flint-wasm`. Il backend convalida prima i suoi byte
...
Per i flussi di lavoro del file system, installare `@mission-platform/flint-cli` e
usa il suo binario `flint` autonomo:
...
Utilizzare `@mission-platform/flint-vitest` quando è necessario una suite Vitest
...
L'helper `defineFlintVitestConfig` installa la produzione
`forgeWebScriptPlugin` preservando i plugin e le impostazioni Vite consumer.
Vedere [Test in Mission Platform](../../../../../../docs/locales/it/testing.md#flint-tests) per
...
i progetti che importano `.flint` o le relative query sugli artefatti virtuali dovrebbero aggiungere il file
sottopercorso della dichiarazione di solo tipo
`@mission-platform/flint-vitest/flint` al loro
...
`packages/flint-vitest/fixtures/` sono il corpus di pacchetti incrociati per
...
| ------------------ | ----------------------------------------------------------------------------------------------------------------- |
| `FLINT-LEX-*`      | caratteri/escape non validi, terminatori di riga di stringhe non elaborate o stringhe/commenti senza terminazione |
| `FLINT-PARSE-*`    | sintassi di modulo, dichiarazione, istruzione o espressione non valida                                            |
| `FLINT-TYPE-*`     | tipo primitivo, nome, operatore, argomento o ritorno non valido                                                   |
| `FLINT-ABI-*`      | nomi duplicati, funzionalità negate, esportazioni o importazioni                                                  |
...
`packages/flint/core/src/fixtures/bootstrap.ts` è l'eseguibile
...
`packages/flint-vitest/fixtures` copre lo stesso limite:
`valid/collections.flint` esercita valori letterali di raccolta, indicizzazione, contestuali
vettori vuoti, `length()` e stringhe con escape valide;
`valid/aggregates.flint` esercita i valori della funzione, `Result::Ok` qualificato e
Costruttori `Result::Error` e associazioni di corrispondenza arm-locale; e
`diagnostics/collections.flint` esercita chiamate di raccolta e aggregazione non valide
...
periodo; Flint non li reinterpreta né li sostituisce.
...
Flint è l'obiettivo di produzione per il neutrale
...
`packages/flint/core/src/fixtures/codec-migration.ts` è il primo
...
falso per i test di conformità mentre il dispositivo dimostra il Flint
...
| Comportamento del codice a barre esistente                                       | `@mission-platform/barcode`                                   | I grafici Flint locali del pacchetto forniscono la facciata del codice a barre digitato.                                                                             |
| Nuovo calcolo sicuro per i browser per scopi generici con effetti host espliciti | Forgia script Web più `@mission-platform/vite-plugin-flint`   | Funzionalità di origine `.flint` con versione, manifest, caricatore tipizzato e negazione per impostazione predefinita.                                              |
...
| Compilazione dell'interfaccia utente/componente indipendente dal framework       | Compilatore di componenti Forge                               | Flint non sostituisce `FrameworkOutputPlugin` o le destinazioni dei componenti.                                                                                      |

Utilizzare il plug-in Flint Vite solo per le voci `.flint`. Usa il
...
fornito esplicitamente ai moduli Flint.
...
Il contratto del bytecode della VM è indipendente dal backend. A`FlintVmModule`
...
`FlintSelfHostedStageReport` anziché essere presentato come completo
self-hosting.

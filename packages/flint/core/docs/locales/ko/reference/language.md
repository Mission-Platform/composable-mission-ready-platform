...
소스 형식은 확장자가 `.flint`인 UTF-8 텍스트입니다. 소스 파일은
...
서비스, CLI 및 MCP 통합. `analyzeFlint`은 확인된 내용을 수락합니다.
...
분석 결과는 안정적인 `FLINT-ANALYSIS-*` 코드를 사용하고 카테고리를 포함합니다.
심각도, UTF-16 호환 소스 범위, 증거, 해결 힌트 및
선택적 OWASP/CWE 참조. 진단에는 `phase: "analysis"`가 추가되고
기존 `FLINT-LEX-*`, `FLINT-PARSE-*`,
`FLINT-TYPE-*` 또는 `FLINT-ABI-*` 진단.
...
- `linkProfile: "dynamic"`: 명시적인 소스 모듈 경계가 유지됩니다.
  `FlintDynamicLinkCache`은 런타임에 디코더 모듈을 확인하는 데 사용됩니다.
...
스캐너는 경계 영역을 소비하고 안정적인 `FLINT-LEX-*` 진단을 내보냅니다.
...
진단 `FLINT-LEX-003` 및 파서 복구는 나머지 부분에 대해 계속 사용 가능합니다.
소스.
...
공유 패키지 `@mission-platform/flint-regex`은 안정 패키지를 소유합니다. `$`
...
`FLINT-PARSE-052`; 불변 구조체, 태그가 지정된 열거형, 인터페이스 및 함수
...
중첩된 `module` 선언이 없습니다. 모든 `.flint` 파일은 모듈이며 해당 파일은
안정적인 이름은 정규화된 파일 ID에서 파생됩니다. 예를 들어,
`/workspace/app` 프로젝트의 `src/time.flint`에는 모듈 ID `src/time`가 있습니다. 중첩됨
...
상태 저장 메모리 고정 장치
`packages/flint/core/src/fixtures/stateful-memory.flint`은 적합성입니다
...
발신자는 계속해서 `FlintBytes`을 전달하고 수신하며 `memory`은
...
`@mission-platform/flint-wasm`. 백엔드는 이전에 해당 바이트의 유효성을 검사합니다.
반환; 오류는 실행 가능한 출력을 억제합니다. Vite 어댑터 및 LSP 사용
외관은 Node CLI에 의존할 필요가 없습니다.

파일 시스템 워크플로의 경우 `@mission-platform/flint-cli`을 설치하고
독립형 `flint` 바이너리를 사용하십시오.
...
Vitest 제품군이 필요한 경우 `@mission-platform/flint-vitest`을 사용하세요.
...
`defineFlintVitestConfig` 도우미가 프로덕션을 설치합니다.
소비자 Vite 플러그인 및 설정을 유지하면서 `forgeWebScriptPlugin`.
자세한 내용은 [미션 플랫폼 테스트](../../../../../../docs/locales/ko/testing.md#flint-tests)를 참조하세요.
구성 및 고정 장치 예.
...
`.flint` 또는 해당 가상 아티팩트 쿼리를 가져오는 프로젝트는
유형 전용 선언 하위 경로
`@mission-platform/flint-vitest/flint`을 자신에게
TypeScript `types` 목록 또는 참조된 테스트 유형 진입점입니다.

아래의 공유 하네스 고정 장치
`packages/flint-vitest/fixtures/`은 다음을 위한 크로스 패키지 코퍼스입니다.
...
| 코드 계열       | 의미                                                                         |
| --------------- | ---------------------------------------------------------------------------- |
| `FLINT-LEX-*`   | 잘못된 문자/이스케이프, 원시 문자열 줄 종결자 또는 종료되지 않은 문자열/주석 |
| `FLINT-PARSE-*` | 잘못된 모듈, 선언, 명령문 또는 표현식 구문                                   |
| `FLINT-TYPE-*`  | 잘못된 기본 유형, 이름, 연산자, 인수 또는 반환                               |
| `FLINT-ABI-*`   | 중복된 이름, 거부된 기능, 내보내기 또는 가져오기                             |
...
`packages/flint/core/src/fixtures/bootstrap.ts`는 실행 파일입니다.
...
공유 소스 코퍼스
`packages/flint-vitest/fixtures`은 동일한 경계를 포함합니다.
`valid/collections.flint`은 컬렉션 리터럴, 인덱싱, 상황별 연습을 수행합니다.
빈 벡터, `length()` 및 유효한 이스케이프 문자열;
`valid/aggregates.flint`은 함수 값, 정규화된 `Result::Ok` 및
`Result::Error` 생성자 및 arm-local 일치 바인딩. 그리고
`diagnostics/collections.flint`은 잘못된 수집 호출 및 집계를 실행합니다.
...
`packages/flint/core/src/fixtures/codec-migration.ts`이 첫 번째입니다.
...
| 명시적인 호스트 효과를 갖춘 새로운 범용 브라우저 안전 컴퓨팅   | Forge 웹 스크립트 플러스 `@mission-platform/vite-plugin-flint` | 버전이 지정된 `.flint` 소스, 매니페스트, 형식화된 로더 및 기본 거부 기능.                                              |
...
| 프레임워크 중립적인 UI/구성 요소 컴파일                        | Forge 구성 요소 컴파일러                                       | Flint는 `FrameworkOutputPlugin` 또는 구성 요소 대상을 대체하지 않습니다.                                               |

`.flint` 항목에만 Flint Vite 플러그인을 사용하세요. 사용
...
VM 바이트코드 계약은 백엔드 독립적입니다. `FlintVmModule`
...
`FlintSelfHostedStageReport`이 완전한 것으로 표시되지 않고
자체 호스팅.

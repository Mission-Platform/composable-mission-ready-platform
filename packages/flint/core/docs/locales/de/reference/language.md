...
Flint (`.flint`) ist eine kleine Allzwecksprache für WebAssembly
...
Maßgeblicher V1-Sprach- und Modulvertrag. `@mission-platform/flint`
...
und LSP. `@mission-platform/flint-wasm` ist das deterministische Backend
Dadurch wird die überprüfte IR auf validiertes WebAssembly und WAT gesenkt. Nur Node
Das Paket `@mission-platform/flint-cli` stellt das Paket `flint` bereit
...
Das Quellformat ist UTF-8-Text mit der Erweiterung `.flint`. Eine Quelldatei ist eine
...
Service-, CLI- und MCP-Integrationen. `analyzeFlint` akzeptiert das Geprüfte
...
Analyseergebnisse verwenden stabile `FLINT-ANALYSIS-*`-Codes und umfassen eine Kategorie,
...
Sicherheitsmetadaten ohne Änderung der vorhandenen `FLINT-LEX-*`, `FLINT-PARSE-*`,
`FLINT-TYPE-*`- oder `FLINT-ABI-*`-Diagnose.
...
Flint stellt wiederherstellbare Ergebnisse mit der Standardbibliothek dar
...
- `linkProfile: "dynamic"`: Explizite Quellmodulgrenzen bleiben erhalten.
  `FlintDynamicLinkCache` wird verwendet, um Decoder-Module zur Laufzeit aufzulösen.
...
Der Scanner verbraucht einen begrenzten Bereich, gibt die stabile `FLINT-LEX-*`-Diagnose aus und
...
Diagnose `FLINT-LEX-003` und Parser-Wiederherstellung bleiben für den Rest verfügbar
die Quelle.
...
Flint bietet eine deterministische Standardbibliothek für reguläre Ausdrücke.
...
und `^`/`Flint bietet eine deterministische Standardbibliothek für reguläre Ausdrücke.
Die Aufrufe`regex_full_match(pattern, value) -> bool`,
...
Das freigegebene Paket `@mission-platform/flint-regex` besitzt den Stable `$`
...
Die Ausführung von Produktions-Regex gehört zum Flint-Backend und zum
...
`FLINT-PARSE-052`; unveränderliche Strukturen, getaggte Aufzählungen, Schnittstellen und Funktionen
...
Es gibt keine verschachtelte `module`-Deklaration. Jede `.flint`-Datei ist ein Modul und seine
Der stabile Name wird von seiner normalisierten Datei-ID abgeleitet. Zum Beispiel,
`src/time.flint` im Projekt `/workspace/app` hat die Modul-ID `src/time`. Verschachtelt
...
Flint verwendet eine WASI-inspirierte _logische_ Grenze, keinen Anspruch auf Vollständigkeit
...
`packages/flint/core/src/fixtures/stateful-memory.flint` ist die Konformität
...
Anrufer passieren weiterhin `FlintBytes` und erhalten `memory`
...
`@mission-platform/flint-wasm`. Das Backend validiert seine Bytes vorher
...
Für Dateisystem-Workflows installieren Sie `@mission-platform/flint-cli` und
Verwenden Sie die eigenständige `flint`-Binärdatei:
...
Verwenden Sie `@mission-platform/flint-vitest`, wenn eine Vitest-Suite dies erfordert
...
Der `defineFlintVitestConfig`-Helfer installiert die Produktion
`forgeWebScriptPlugin` unter Beibehaltung der Verbraucher-Plugins und -Einstellungen Vite.
Siehe [Testen in Mission Platform](../../../../../../docs/locales/de/testing.md#flint-tests) für
...
Projekte, die `.flint` oder seine virtuellen Artefaktabfragen importieren, sollten das hinzufügen
Unterpfad für Nur-Typ-Deklaration
`@mission-platform/flint-vitest/flint` zu ihrem
TypeScript `types` Liste oder ein referenzierter Testtyp-Einstiegspunkt.

Die gemeinsamen Kabelbaumbefestigungen unten
`packages/flint-vitest/fixtures/` sind das paketübergreifende Korpus für
...
| --------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `FLINT-LEX-*`   | ungültige Zeichen/Escapezeichen, rohe Zeichenfolgenzeilen-Abschlusszeichen oder nicht abgeschlossene Zeichenfolgen/Kommentare |
| `FLINT-PARSE-*` | Ungültige Modul-, Deklarations-, Anweisungs- oder Ausdruckssyntax                                                             |
| `FLINT-TYPE-*`  | Ungültiger primitiver Typ, Name, Operator, Argument oder Rückgabewert                                                         |
| `FLINT-ABI-*`   | doppelte Namen, verweigerte Funktionen, Exporte oder Importe                                                                  |
...
`packages/flint/core/src/fixtures/bootstrap.ts` ist die ausführbare Datei
...
`packages/flint-vitest/fixtures` deckt dieselbe Grenze ab:
`valid/collections.flint` übt Sammlungsliterale, Indizierung und Kontext aus
leere Vektoren, `length()` und gültige Escapezeichenfolgen;
`valid/aggregates.flint` übt Funktionswerte aus, qualifiziert `Result::Ok` und
`Result::Error`-Konstruktoren und Arm-Local-Match-Bindungen; und
`diagnostics/collections.flint` führt ungültige Sammlungsaufrufe und Aggregate aus
...
Zeitraum; Flint interpretiert oder ersetzt sie nicht neu.
...
Flint ist das Produktionsziel für Neutrale
...
`packages/flint/core/src/fixtures/codec-migration.ts` ist der erste
...
| Vorhandenes Barcode-Verhalten                                                   | `@mission-platform/barcode`                                   | Paketlokale Flint-Diagramme stellen die typisierte Barcode-Fassade bereit.                                                                        |
| Neue universelle, browsersichere Datenverarbeitung mit expliziten Hosteffekten  | Flint plus `@mission-platform/vite-plugin-flint`              | Versionierte `.flint`-Quelle, Manifest, typisierter Loader und Standardverweigerungsfunktionen.                                                   |
...
| Frameworkneutrale UI-/Komponentenkompilierung                                   | Forge-Komponenten-Compiler                                    | Flint ist kein Ersatz für `FrameworkOutputPlugin` oder Komponentenziele.                                                                          |

Verwenden Sie das Flint Vite-Plugin nur für `.flint`-Einträge. Benutzen Sie die
...
Wird explizit für Flint-Module bereitgestellt.
...
Der VM-Bytecode-Vertrag ist Backend-unabhängig. Ein `FlintVmModule`
...
`FlintSelfHostedStageReport` anstatt als vollständig dargestellt zu werden
Selbsthosting.

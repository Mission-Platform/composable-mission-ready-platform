...
Flint (`.flint`) is een kleine, algemene taal voor WebAssembly
...
gezaghebbend v1 taal- en modulecontract. `@mission-platform/flint`
...
en LSP. `@mission-platform/flint-wasm` is de deterministische backend
dat de gecontroleerde IR verlaagt tot gevalideerde WebAssembly en WAT. Alleen de Node
Het `@mission-platform/flint-cli`-pakket bevat de `flint`
...
Het bronformaat is UTF-8-tekst met de extensie `.flint`. Een bronbestand is een
...
service-, CLI- en MCP-integraties. `analyzeFlint` accepteert de aangevinkte
...
Analysebevindingen gebruiken stabiele `FLINT-ANALYSIS-*`-codes en omvatten een categorie,
...
beveiligingsmetagegevens zonder de bestaande `FLINT-LEX-*`, `FLINT-PARSE-*` te wijzigen,
`FLINT-TYPE-*`- of `FLINT-ABI-*`-diagnostiek.
...
Flint vertegenwoordigt herstelbare resultaten met de standaardbibliotheek
...
- `linkProfile: "dynamic"`: Expliciete grenzen van bronmodules blijven behouden.
  `FlintDynamicLinkCache` wordt gebruikt om decodermodules tijdens runtime op te lossen,
...
scanner verbruikt een begrensd gebied, zendt de stabiele `FLINT-LEX-*`-diagnose uit, en
...
diagnostische `FLINT-LEX-003` en parserherstel blijven beschikbaar voor de rest van de periode
de bron.
...
Flint biedt een deterministische standaardbibliotheek met reguliere expressies.
...
en `^`/`Flint biedt een deterministische standaardbibliotheek met reguliere expressies.
De oproepen`regex_full_match(pattern, value) -> bool`,
`regex_prefix_match(pattern, value) -> bool`, en
`regex_search(pattern, value, start: i32) -> bool`voert gehele waarde uit,
respectievelijk positie-nul voorvoegsel en meest linkse zoekopdracht. Grenzen vastleggen
...
Het gedeelde pakket `@mission-platform/flint-regex` is eigenaar van de stal `$`
...
Uitvoering van productieregex behoort tot de Flint-backend en de
...
`FLINT-PARSE-052`; onveranderlijke structuren, getagde enums, interfaces en functie
...
Er is geen geneste `module`-declaratie. Elk `.flint`-bestand is een module en zijn
stabiele naam is afgeleid van de genormaliseerde bestands-ID. Bijvoorbeeld
`src/time.flint` in project `/workspace/app` heeft module-ID `src/time`. Genest
...
Flint gebruikt een WASI-geïnspireerde _logische_ grens, geen claim van volledig
...
`packages/flint/core/src/fixtures/stateful-memory.flint` is de conformiteit
...
bellers blijven `FlintBytes` doorgeven en ontvangen, en `memory` is
...
`@mission-platform/flint-wasm`. De backend valideert zijn bytes eerder
...
Voor bestandssysteemworkflows installeert u `@mission-platform/flint-cli` en
gebruik het zelfstandige `flint` binaire bestand:
...
Gebruik `@mission-platform/flint-vitest` wanneer een Vitest-suite dit nodig heeft
...
`defineFlintVitestConfig`-helper installeert de productie
`forgeWebScriptPlugin` met behoud van de Vite-plug-ins en instellingen voor consumenten.
Zie [Testen in Mission Platform](../../../../../../docs/locales/nl/testing.md#flint-tests) voor de
...
projecten die `.flint` of de virtuele artefactquery's importeren, moeten de
subpad voor declaratie alleen van het type
`@mission-platform/flint-vitest/flint` naar hun
TypeScript `types`-lijst of een ingangspunt voor het testtype waarnaar wordt verwezen.

De gedeelde harnasbevestigingen onder
`packages/flint-vitest/fixtures/` is het pakketoverschrijdende corpus voor
...
| --------------- | ----------------------------------------------------------------------------------------------------------- |
| `FLINT-LEX-*`   | ongeldige tekens/escapes, onbewerkte tekenreeksregelterminators of niet-afgesloten tekenreeksen/opmerkingen |
| `FLINT-PARSE-*` | ongeldige syntaxis van module, declaratie, instructie of expressie                                          |
| `FLINT-TYPE-*`  | ongeldig primitief type, naam, operator, argument of return                                                 |
| `FLINT-ABI-*`   | dubbele namen, geweigerde mogelijkheden, export of import                                                   |
...
`packages/flint/core/src/fixtures/bootstrap.ts` is het uitvoerbare bestand
...
`packages/flint-vitest/fixtures` bestrijkt dezelfde grens:
`valid/collections.flint` oefent verzamelingsliterals, indexering, contextueel
lege vectoren, `length()` en geldige ontsnapte tekenreeksen;
`valid/aggregates.flint` oefent functiewaarden uit, gekwalificeerde `Result::Ok` en
`Result::Error`-constructors en arm-local match-bindingen; en
`diagnostics/collections.flint` oefent ongeldige incassoaanroepen en aggregaties uit
...
periode; Flint herinterpreteert of vervangt ze niet.
...
Flint is het productiedoel voor de neutrale
...
`packages/flint/core/src/fixtures/codec-migration.ts` is de eerste
...
nep voor conformiteitstests terwijl het armatuur het Flint bewijst
...
| Bestaand streepjescodegedrag                                                          | `@mission-platform/barcode`                                   | Pakket-lokale Flint-grafieken bieden de getypte barcode-façade.                                                                           |
| Nieuw browserveilig computergebruik voor algemeen gebruik met expliciete hosteffecten | Webscript smeden plus `@mission-platform/vite-plugin-flint`   | `.flint`-bron-, manifest-, getypte lader en standaard-deny-by-default-mogelijkheden met versieversie.                                     |
...
| Kaderneutrale UI/componentcompilatie                                                  | Componentcompiler smeden                                      | Flint is geen vervanging voor `FrameworkOutputPlugin` of componentdoelen.                                                                 |

Gebruik de Flint Vite-plug-in alleen voor `.flint`-vermeldingen. Gebruik de
...
expliciet geleverd aan Flint-modules.
...
Het VM-bytecodecontract is backend-onafhankelijk. A `FlintVmModule`
...
`FlintSelfHostedStageReport` in plaats van als compleet te worden gepresenteerd
zelf-hosting.

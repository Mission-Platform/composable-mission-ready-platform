...
源格式为 UTF-8 文本，扩展名为 `.flint`。源文件是一个
...
- `linkProfile: "dynamic"`：保留显式源模块边界。
  `FlintDynamicLinkCache` 用于在运行时解析解码器模块，
...
扫描仪消耗有界区域，发出稳定的 `FLINT-LEX-*` 诊断信息，并且
...
诊断 `FLINT-LEX-003` 和解析器恢复仍然可用于其余部分
来源。
...
共享包 `@mission-platform/flint-regex` 拥有稳定的 `$`
...
`FLINT-PARSE-052`；不可变结构、标记枚举、接口和函数
...
没有嵌套的 `module` 声明。每个 `.flint` 文件都是一个模块及其
稳定名称源自其规范化文件 ID。例如，
项目 `/workspace/app` 中的 `src/time.flint` 具有模块 ID `src/time`。嵌套
...
下的状态内存装置
`packages/flint/core/src/fixtures/stateful-memory.flint` 是一致性
...
调用者继续传递和接收 `FlintBytes`，并且 `memory` 是
...
`@mission-platform/flint-wasm`。后端之前验证其字节
归还它们；错误会抑制可执行输出。 Vite 适配器和 LSP 使用
外观，不需要依赖 Node CLI。

对于文件系统工作流程，安装 `@mission-platform/flint-cli` 和
使用其独立的 `flint` 二进制文件：
...
当 Vitest 套件需要时，请使用 `@mission-platform/flint-vitest`
...
`defineFlintVitestConfig` 帮助程序安装生产
`forgeWebScriptPlugin`，同时保留消费者 Vite 插件和设置。
请参阅[任务平台测试](../../../../../../docs/locales/zh/testing.md#flint-tests)
配置和夹具示例。
...
导入 `.flint` 或其虚拟工件查询的项目应添加
仅类型声明子路径
`@mission-platform/flint-vitest/flint` 发送给他们
TypeScript `types` 列表或引用的测试类型入口点。

下面的共享线束夹具
`packages/flint-vitest/fixtures/` 是跨包语料库
...
| 代码家族        | 意义                                                     |
| --------------- | -------------------------------------------------------- |
| `FLINT-LEX-*`   | 无效字符/转义符、原始字符串行终止符或未终止的字符串/注释 |
| `FLINT-PARSE-*` | 无效的模块、声明、语句或表达式语法                       |
| `FLINT-TYPE-*`  | 无效的基本类型、名称、运算符、参数或返回                 |
| `FLINT-ABI-*`   | 重复名称、被拒绝的功能、导出或导入                       |
...
`packages/flint/core/src/fixtures/bootstrap.ts` 是可执行文件
...
共享源语料库位于
`packages/flint-vitest/fixtures` 涵盖相同的边界：
`valid/collections.flint` 练习集合文字、索引、上下文
空向量、`length()` 和有效的转义字符串；
`valid/aggregates.flint` 练习函数值，合格 `Result::Ok` 和
`Result::Error` 构造函数和 Arm 本地匹配绑定；和
`diagnostics/collections.flint` 执行无效的收集调用和聚合
构造函数/绑定诊断。收集夹具也编译好了
...
`packages/flint/core/src/fixtures/codec-migration.ts` 是第一个
...
| 具有显式主机效应的新型通用浏览器安全计算               | Forge Web 脚本加上 `@mission-platform/vite-plugin-flint`      | 版本化 `.flint` 源、清单、类型化加载程序和默认拒绝功能。                  |
...
仅对 `.flint` 条目使用 Forge Web 脚本 Vite 插件。使用
...
释放；这个边界是故意的并且暴露为
`FlintSelfHostedStageReport` 而不是完整呈现
自托管。

...
process.stdin.on('data', (chunk) => {
  ...
  while (newline !== -1) {
    ...
    } else if (request.command === 'stackTrace') reply({ stackFrames: [{ id: 10, name: 'main', line: 2, column: 1, source: { path: 'main.flint' } }], totalFrames: 1 });
    ...
    else if (request.command === 'continue') { reply({}); send({ type: 'continued', threadId: 1 }); send({ type: 'stopped', reason: 'step', threadId: 1, source: { path: 'main.flint' }, line: 3, column: 1 }); }
...
describe('Forge Web Script DAP server', () => {
  it('launches a separate runtime, stops on a source breakpoint, inspects state, steps, and terminates', async () => {
    ...
    const program = path.join(cwd, 'main.flint');
  ...
  it('surfaces invalid program path as a launch failure', async () => {
    ...
    const cwd = await mkdtemp(path.join(tmpdir(), 'fws-dap-'));
    const program = path.join(cwd, 'missing.flint');
  ...
  it('returns actionable launch failures for missing runtime and program paths', async () => {
    ...
    const parser = new DapFrameParser();
    const messages: DapMessage[] = [];
    const server = createFlintDapServer({ input, output });
    server.start();
    sendRequest(input, 1, 'launch', { program: 'missing.flint', cwd: process.cwd() });
    expect(await nextMessage(output, parser, messages)).toMatchObject({
      request_seq: 1,
      success: false,
      message: "Launch requires 'runtimePath' or FLINT_RUNTIME.",
    });
    server.dispose();
  });
});

...
import {
  ...
  type FlintDapLaunchArguments,
  type FlintRuntimeCommand,
  type FlintRuntimeMessage,
  type FlintRuntimeProcess,
  type FlintRuntimeRequest,
  type FlintRuntimeResponse,
  type FlintRuntimeSpawner,
} from './protocol.js';
...
export interface FlintDapServerOptions {
  ...
  readonly spawnRuntime?: FlintRuntimeSpawner;
...
export function createFlintDapServer(options: FlintDapServerOptions): FlintDapServer {
  ...
  let runtime: FlintRuntimeProcess | undefined;
  let started = false;
  let disposed = false;
  ...
  const attachRuntime = (process_: FlintRuntimeProcess): void => {
    ...
    process_.on('close', (code) => {
      rejectPending(new Error('Flint runtime exited before completing the request.'));
  ...
  const sendRuntimeRequest = (command: FlintRuntimeCommand, arguments_: unknown): Promise<FlintRuntimeResponse> => {
    ...
    const request: FlintRuntimeRequest = {
      type: 'request',
      requestId,
      command,
  ...
  const handleRequest = async (request: DapRequest): Promise<void> => {
    try {
      ...
      if (request.command === 'launch') {
        ...
        if (runtime !== undefined) throw new Error('A Flint debug session is already running.');
        attachRuntime(
        ...
        if (!result.success) throw new Error(result.message ?? 'The Flint runtime rejected launch.');
  ...
  const runtimePathValue =
    typeof arguments_.runtimePath === 'string' && arguments_.runtimePath.trim().length > 0
      ...
      : // The environment fallback is intentionally explicit and only supplies the runtime executable.
        ...
        process.env.FLINT_RUNTIME;
  if (runtimePathValue === undefined) throw new Error("Launch requires 'runtimePath' or FLINT_RUNTIME.");
  if (

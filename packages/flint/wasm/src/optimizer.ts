...
function validateAndAnnotateSwitches(
  ...
  return statements.map((statement) => {
    if (statement.kind === 'switch') {
      ...
      const strategy: FlintWasmSwitchStrategy =
...
function directCalls(module: FlintWasmModule): number {
  ...
  let count = 0;
  const visit = (expression: FlintWasmExpression): void => {
  ...
  const visitStatements = (statements: readonly FlintWasmStatement[]): void => {
    for (const statement of statements) {
      switch (statement.kind) {
        case 'let':
        case 'assignment': {

...
import { lowerFlintWasmFunctionToSsa } from './cfg.js';

import type { FlintWasmFunction, FlintWasmSourceSpan } from './contracts.js';
...
function functionWith(body: FlintWasmFunction['body']): FlintWasmFunction {
  return {
    kind: 'function',
    name: 'ssa',
    exported: false,
...
describe('Forge Web Script structured CFG/SSA lowering', () => {
  it('creates a phi value and join block for divergent scalar definitions', () => {
    const plan = lowerFlintWasmFunctionToSsa(
      functionWith([
  ...
  it('creates a loop-header phi and a backedge binding for loop-carried values', () => {
    const loop: FlintWasmFunction['body'][number] = {
      kind: 'while',
      condition: {
        kind: 'binary',
        operator: '<',

...
import { compileRegex, type CompiledRegex } from '@mission-platform/flint-regex';
import {
...
const wasmTypes: Readonly<Record<FlintWasmPrimitiveType, readonly WasmValueType[]>> = {
  bool: [0x7f],
  bytes: [0x7f, 0x7f],
  f32: [0x7d],
  f64: [0x7c],
...
function returnIterResult(
  ...
  const valueExpression: FlintWasmExpression =
...
function buildYieldStateMachine(
  ...
  const state: FlintWasmExpression = { kind: 'identifier', name: '__state', span };
  let statements: FlintWasmStatement[] = [returnIterResult(0, 1, span)];
...
function lowerIteratorModule(module: FlintWasmModule): {
  ...
  const functions: FlintWasmFunction[] = [];
  ...
  const iteratorExports: FlintWasmIteratorExport[] = descriptors.map((descriptor) => ({
    name: descriptor.id,
...
function emitWasm(
  ...
  const collectionContracts = hasCollectionRuntime
    ? COLLECTION_RUNTIME_OPERATION_ORDER.map((operation) =>
        buildFlintWasmCollectionRuntimeBodies().find((contract) => contract.operation === operation),
  ...
  const collectionTypeIndexes = collectionContracts.map(({ parameters, results }) =>
    getTypeIndex(parameters as readonly FlintWasmPrimitiveType[], (results[0] ?? 'unit') as FlintWasmPrimitiveType),
  );
  ...
  for (const declaration of module.functions) {
    ...
    const locations = new Map<FlintWasmStatement, ValueLocation>();
    const iteratorBindingLocations = new Map<FlintWasmStatement, ValueLocation>();
    const iteratorSourceLocations = new Map<FlintWasmStatement, ValueLocation>();
    const expressionLocations = new Map<FlintWasmExpression, ValueLocation>();
    ...
    function emitExpression(expression: FlintWasmExpression, visible: ReadonlyMap<string, ValueLocation>): void {
      if (expression.kind === 'literal') {
        ...
        if (temporary === undefined || layout === undefined)
          throw new Error(`FLINT-RECORD-001: Record layout for "${expression.type}" is unavailable.`);
        body.push(
          0x41,
        ...
        if (expression.standardLibrary !== undefined) {
          ...
          if (expression.standardLibrary === 'memory-load-u32') {
            ...
            if (address === undefined) throw new Error('FLINT-MEMORY-001: memory_load_u32 requires an address.');
          ...
          if (expression.standardLibrary === 'memory-store-u32') {
            ...
            if (address === undefined || value === undefined)
              throw new Error('FLINT-MEMORY-002: memory_store_u32 requires an address and value.');
            emitExpression(address, visible);
            emitExpression(value, visible);
          ...
          if (expression.standardLibrary === 'memory-store-f64') {
            ...
            const value = expression.arguments[1];
            if (address === undefined || value === undefined)
              throw new Error('FLINT-MEMORY-004: memory_store_f64 requires an address and value.');
            emitExpression(address, visible);
            emitExpression(value, visible);
          ...
          if (expression.standardLibrary === 'f64-from-u32') {
            ...
            if (value === undefined) throw new Error('FLINT-NUMERIC-001: f64_from_u32 requires a value.');
            emitExpression(value, visible);
            body.push(0xb8);
            return;
          }
          if (expression.standardLibrary === 'memory-copy') {
            ...
            if (destination === undefined || source === undefined || size === undefined)
              throw new Error('FLINT-MEMORY-005: memory_copy requires dest, src, and size.');
          ...
          if (expression.standardLibrary === 'memory-fill') {
            ...
            if (destination === undefined || value === undefined || size === undefined)
              throw new Error('FLINT-MEMORY-006: memory_fill requires dest, value, and size.');
            emitExpression(destination, visible);
            emitExpression(value, visible);
            emitExpression(size, visible);
          ...
          if (expression.standardLibrary === 'simd-v128-load') {
            const address = expression.arguments[0];
            if (address === undefined) throw new Error('FLINT-SIMD-001: simd_v128_load requires address.');
            emitExpression(address, visible);
          ...
          if (expression.standardLibrary === 'simd-v128-store') {
            ...
            const value = expression.arguments[1];
            if (address === undefined || value === undefined)
              throw new Error('FLINT-SIMD-002: simd_v128_store requires address and value.');
            emitExpression(address, visible);
            emitExpression(value, visible);
          ...
          if (expression.standardLibrary === 'simd-i8x16-splat') {
            const value = expression.arguments[0];
            if (value === undefined) throw new Error('FLINT-SIMD-003: simd_i8x16_splat requires value.');
            emitExpression(value, visible);
            body.push(0xfd, 0x0f);
            return;
          }
          if (expression.standardLibrary === 'simd-i8x16-eq') {
            ...
            if (left === undefined || right === undefined)
              throw new Error('FLINT-SIMD-004: simd_i8x16_eq requires two operands.');
          ...
          if (expression.standardLibrary === 'simd-i8x16-lt-u') {
            ...
            const right = expression.arguments[1];
            if (left === undefined || right === undefined)
              throw new Error('FLINT-SIMD-005: simd_i8x16_lt_u requires two operands.');
            emitExpression(left, visible);
            emitExpression(right, visible);
            body.push(0xfd, 0x26);
            return;
          ...
          if (expression.standardLibrary === 'simd-i8x16-bitmask') {
            const value = expression.arguments[0];
            if (value === undefined) throw new Error('FLINT-SIMD-006: simd_i8x16_bitmask requires value.');
            emitExpression(value, visible);
            body.push(0xfd, 0x64);
            return;
          }
          if (expression.standardLibrary === 'simd-i32x4-splat') {
            const value = expression.arguments[0];
            if (value === undefined) throw new Error('FLINT-SIMD-007: simd_i32x4_splat requires value.');
            emitExpression(value, visible);
            body.push(0xfd, 0x11);
            return;
          }
          if (expression.standardLibrary === 'simd-i32x4-add') {
            ...
            const right = expression.arguments[1];
            if (left === undefined || right === undefined)
              throw new Error('FLINT-SIMD-008: simd_i32x4_add requires two operands.');
            emitExpression(left, visible);
            emitExpression(right, visible);
          ...
          if (pattern?.kind !== 'literal' || pattern.type !== 'string' || typeof pattern.value !== 'string')
            throw new Error('FLINT-REGEX-004: Regex patterns must be compile-time string literals.');
          const table = getRegexTable(pattern.value);
          const input = expression.arguments[1];
          if (input === undefined) throw new Error('FLINT-REGEX-005: Regex operation is missing its input string.');
    ...
    const emitSwitch = (
      ...
      const values = statement.cases.map((arm) => {
        ...
        if (value === undefined || !Number.isInteger(value))
          throw new Error(`FLINT-DISPATCH-001: Invalid switch case "${String(arm.value)}".`);
        return value;
      });
      if (new Set(values).size !== values.length) throw new Error('FLINT-DISPATCH-002: Duplicate switch case value.');
...
function validateReturnPaths(module: FlintWasmModule, fileName: string): readonly FlintWasmDiagnostic[] {
  return module.functions.flatMap((declaration) => {
    ...
    if (declaration.result.name === 'unit' || !lowerFlintWasmFunctionToSsa(declaration).exitReachable) return [];
    return [
      {
        ...backendDiagnostic(
          fileName,
...
function emitVariant(
  ...
  const stageDiagnostics: FlintWasmDiagnostic[] = stage.diagnostics.map((diagnostic) => ({
    ...backendDiagnostic(fileName, diagnostic.message, diagnostic.span),
    code: diagnostic.code,
  }));
  const wat = renderFlintWasmWat(stage.module, { ...metadata, targetFeatures });
  const diagnostics: FlintWasmDiagnostic[] = [
    ...stageDiagnostics,
  ...
  try {
    ...
    if (!WebAssembly.validate(wasm.buffer as ArrayBuffer)) {
      ...
      diagnostics.push(
        backendDiagnostic(
          fileName,
          `The Flint backend emitted invalid WebAssembly.${validationDetail}`,
          stage.module.span,
        ),
      );

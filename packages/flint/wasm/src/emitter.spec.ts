...
import type { FlintWasmExpression, FlintWasmModule, FlintWasmSourceSpan } from './contracts.js';

const span: FlintWasmSourceSpan = { start: 0, end: 1, line: 1, column: 1, endLine: 1, endColumn: 2 };

function moduleWith(functions: FlintWasmModule['functions']): FlintWasmModule {
  return {
    kind: 'module',
    name: 'backend-spec',
    imports: [],
...
function wasmBytes(result: { readonly wasm?: Uint8Array }): Uint8Array {
  return result.wasm ?? new Uint8Array();
}

function wasmModule(result: { readonly wasm?: Uint8Array }): WebAssembly.Module {
  return new WebAssembly.Module(wasmBytes(result));
}
...
describe('Forge Web Script WASM backend', () => {
  it('emits valid deterministic bytes, WAT, and UTF-8 pointer-length literals', () => {
    ...
    const first = compileFlintWasm(input);
    const second = compileFlintWasm(input);
    expect(first.diagnostics).toHaveLength(0);
    expect(first.wasm).toBeDefined();
    expect(WebAssembly.validate(wasmBytes(first).buffer as ArrayBuffer)).toBe(true);
  ...
  it('exports a working realloc ABI function', () => {
    ...
    const exports = new WebAssembly.Instance(wasmModule(result), {}).exports as Record<string, Function>;
  ...
  it('lowers sparse switch dispatch through equality guards without a padded br_table', () => {
    ...
    expect(WebAssembly.validate(wasmBytes(result).buffer as ArrayBuffer)).toBe(true);
    const exports = new WebAssembly.Instance(wasmModule(result), {}).exports as Record<string, Function>;
  ...
  it('emits balanced block/end structure for non-empty and empty switch statements', () => {
    ...
    const nonEmptyResult = compileFlintWasm({
      ir: nonEmptyModule,
    ...
    const emptyResult = compileFlintWasm({
      ir: emptyModule,
  ...
  it('executes vector literal growth, indexed access, and bounds checking through runtime helpers', () => {
    ...
    expect(result.diagnostics).toEqual([]);
    const exports = new WebAssembly.Instance(wasmModule(result), {}).exports as Record<string, Function>;
    expect(countInstruction(wasmBytes(result), [0xfc, 0x0a, 0x00, 0x00])).toBeGreaterThanOrEqual(1);
  ...
  it('inlines bounds-checked byte access for string pointer-length tuples', () => {
    ...
    const result = compileFlintWasm({ ir: module, optimizedIr: module, abi: {}, links: {}, metadata });
    expect(result.diagnostics).toEqual([]);
    expect(countInstruction(wasmBytes(result), [0x2d, 0x00, 0x00])).toBeGreaterThan(5);
    expect(WebAssembly.validate(wasmBytes(result).buffer as ArrayBuffer)).toBe(true);
    const exports = new WebAssembly.Instance(wasmModule(result), {}).exports as Record<string, Function>;
    const memory = exports.memory as WebAssembly.Memory;
    const allocate = exports.fws_alloc as (size: number) => number;
  ...
  it('validates target profiles and emits shared-memory modules only when enabled', () => {
    ...
    expect(denied.diagnostics.map(({ code }) => code)).toContain('FLINT-FEATURE-001');

    const enabled = compileFlintWasm({
      ir: module,
      optimizedIr: module,
      abi: {},
      links: {},
    ...
    expect(WebAssembly.validate(wasmBytes(enabled).buffer as ArrayBuffer)).toBe(true);
  });
  ...
  it('returns deterministic optimized and unoptimized debug artifacts', () => {
    ...
    const makeModule = (value: number): FlintWasmModule =>
      moduleWith([
        {
          kind: 'function',
          name: 'answer',
  ...
  it('short-circuits logical expressions with result-producing Wasm control flow', () => {
    const divisionByZero: FlintWasmExpression = {
      kind: 'binary',
      operator: '/',
      left: index32(1),
      right: index32(0),
  ...
  it('executes compiler-owned regex matching without capability imports', () => {
    ...
    expect(result.wasm).toBeDefined();
    expect(WebAssembly.validate(wasmBytes(result).buffer as ArrayBuffer)).toBe(true);
    const exports = new WebAssembly.Instance(wasmModule(result), {}).exports;
  ...
  it('returns capture start and end offsets from compiled bytecode', () => {
    ...
    const result = compileFlintWasm({ ir: module, optimizedIr: module, abi: {}, links: {}, metadata });
    expect(result.diagnostics).toEqual([]);
    expect(countInstruction(wasmBytes(result), [0xfc, 0x0a, 0x00, 0x00])).toBeGreaterThanOrEqual(2);
    const exports = new WebAssembly.Instance(wasmModule(result), {}).exports;
  ...
  it('suppresses executable output when a non-unit path is missing a return', () => {
    ...
    expect(result.diagnostics.map(({ code }) => code)).toEqual(['FLINT-CFG-001']);
  });

  it('executes deterministic string and byte helpers inside standalone WASM', () => {
    const functions: FlintWasmModule['functions'] = [
      {
        kind: 'function',
        name: 'length',
        exported: true,
  ...
  it('uses SIMD for long string prefix checks while preserving scalar tails', () => {
    ...
    expect(result.diagnostics).toEqual([]);
    expect(countInstruction(wasmBytes(result), [0xfd, 0x00, 0x00, 0x00])).toBeGreaterThanOrEqual(2);
    const exports = new WebAssembly.Instance(wasmModule(result), {}).exports as Record<string, Function>;
  ...
  it('inlines pointer-length string length without a runtime call', () => {
    ...
    const result = compileFlintWasm({ ir: module, optimizedIr: module, abi: {}, links: {}, metadata });
    expect(result.diagnostics).toEqual([]);
    expect(countInstruction(wasmBytes(result), [0x10, 0x02])).toBe(0);
    const instance = new WebAssembly.Instance(wasmModule(result), {}).exports as Record<string, Function>;
  ...
  it('traps string helpers on invalid pointer-length and slice ranges', () => {
    ...
    const invalid = (
      ...
      arguments_: FlintWasmExpression[],
    ) => ({
  ...
  it('lowers iterator functions to factory/next state machines without crashing', () => {
    ...
    expect(result.diagnostics.some(({ code }) => code === 'FLINT-EMIT-001')).toBe(false);
    expect(result.wasm).toBeDefined();
    expect(WebAssembly.validate(wasmBytes(result).buffer as ArrayBuffer)).toBe(true);
    ...
    const exports = new WebAssembly.Instance(wasmModule(result), {}).exports as Record<string, CallableFunction>;
  ...
  it('elides bounds check for proven-safe indexed reads, but retains it for required/writes', () => {
    ...
    // eslint-disable-next-line unicorn/consistent-function-scoping -- This fixture is specific to this test case.
    const compile = (module: FlintWasmModule) =>
      compileFlintWasm({
        ir: module,
        optimizedIr: module,
        abi: {},
    ...
    expect(wasmBytes(resultSafe).length).toBeLessThan(wasmBytes(resultRequired).length);
    ...
    const instance = new WebAssembly.Instance(wasmModule(resultWrite), {});
  ...
  it('emits bulk memory instructions for memory.copy and memory.fill and executes correctly', () => {
    ...
    const instance = new WebAssembly.Instance(wasmModule(result), {});

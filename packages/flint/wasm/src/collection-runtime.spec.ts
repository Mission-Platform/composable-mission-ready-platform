...
function instantiateArrayIteratorRuntime(): {
  ...
  const runtimeBodies = buildFlintWasmCollectionRuntimeWasmBodies(allocatorIndex);
...
describe('Forge Web Script Wasm collection runtime contracts', () => {
  it('exposes deterministic collection, ECS, and signal runtime bodies', () => {
    const first = buildFlintWasmCollectionRuntimeBodies();
    const second = buildFlintWasmCollectionRuntimeBodies();

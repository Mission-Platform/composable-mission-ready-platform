...
interface IndexedReference {
  ...
  readonly uri: string;
  readonly range: FlintLocation['range'];
}
...
export class FlintWorkspaceSemanticIndex implements FlintWorkspaceIndex {
  ...
  readonly #host: FlintWorkspaceHost | undefined;
  ...
  public async refresh(uri?: string): Promise<void> {
    ...
    const entries = uniqueUris([...documents.keys(), ...listed].filter((entry) => entry.endsWith('.flint')));
    ...
    const graphKey = hashFlintModuleGraph(result.graph);
    ...
    for (const module of result.graph.modules) {
      ...
      const document: FlintDocument = open ?? {
        uri: documentUri,
        fileName: module.fileName,
        text: module.source,
        version: 0,
      };
      const importTypeEnvironment = resolveFlintImportTypeEnvironment(module, result.graph);
      const importEnvironmentHash = JSON.stringify(importTypeEnvironment);
      const optionsHash = JSON.stringify(options);
      const existing = this.#records.get(document.uri);
      const analysis: FlintAnalysis =
        existing !== undefined &&
        ...
        (open === undefined || existing.analysis.version === open.version)
          ? existing.analysis
          : analyzeFlint(document, options, { importTypeEnvironment });
      nextRecords.set(
        document.uri,
  ...
  public symbolsFor(uri: string): readonly FlintSymbol[] {
    this.#ensureIndexed();
    const record = this.#findRecord(uri);
  ...
  public workspaceSymbols(query?: string): readonly { readonly symbol: FlintSymbol; readonly uri: string }[] {
    ...
    const results: { readonly symbol: FlintSymbol; readonly uri: string }[] = [];
  ...
  #createRecord(
    ...
    graphModule?: FlintResolvedModule,
...
export function createFlintWorkspaceIndex(
  ...
  return new FlintWorkspaceSemanticIndex(callbacks, host);
}

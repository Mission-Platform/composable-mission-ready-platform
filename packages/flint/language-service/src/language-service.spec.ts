import { lexFlint } from '@mission-platform/flint';
import { describe, expect, it } from 'vitest';

import { createFlintLanguageService, offsetAtPosition, positionAtOffset, tokenizeFlint } from '.';
...
function document(text: string, version = 1) {
  return { uri: 'file:///workspace/arithmetic.flint', fileName: 'arithmetic.flint', text, version };
}
...
export fn entry(value: i32) -> i32 { return helper(value); }`;
  ...
  return values[2];
    ...
    expect(analysis.analysis?.findings).toEqual(
      expect.arrayContaining([expect.objectContaining({ code: 'FLINT-ANALYSIS-MEMORY-002' })]),
    );
    service.dispose();
  });
...
export fn entry() -> i32 { return hidden(); }`;
    ...
    expect(strict.diagnostics).toEqual([
      expect.objectContaining({ code: 'FLINT-ABI-003', phase: 'abi', severity: 'error' }),
    ]);
  ...
  it.each(['interpret', 'jit', 'aot'] as const)('reports bounded FWS lex parity in %s mode', (selfHostedVmMode) => {
    ...
    const analysisPromise = service.refreshWorkspace('file:///workspace/arithmetic.flint');
    return analysisPromise.then(() => {
      const analysis = service.diagnose('file:///workspace/arithmetic.flint');
      expect(analysis.valid).toBe(true);
      expect(analysis.diagnostics).toEqual([]);
  ...
  it('reuses cached analyses, ignores stale versions, and invalidates changed documents', () => {
    const service = createFlintLanguageService();
    service.openDocument(document(arithmetic));
    const first = service.diagnose('file:///workspace/arithmetic.flint');
    expect(service.diagnose('file:///workspace/arithmetic.flint')).toBe(first);
    service.updateDocument(document('', 0));
    expect(service.diagnose('file:///workspace/arithmetic.flint')).toBe(first);
    service.updateDocument(document('export fn changed() -> i32 { return 1; }', 2));
    const second = service.diagnose('file:///workspace/arithmetic.flint');
    expect(second).not.toBe(first);
    expect(second.version).toBe(2);
    service.dispose();
    expect(() => service.diagnose('file:///workspace/arithmetic.flint')).toThrow('disposed');
  });
...
export fn current() -> i64 { return now(); }`;
    ...
    expect(service.diagnose(document(source).uri).diagnostics.map((item) => item.code)).toContain('FLINT-ABI-002');
...
export fn caller() -> i32 { return add(1); }`;
  ...
  it('keeps completion and hover safe for incomplete documents', () => {
    ...
    const tokenKinds = tokenizeFlint('// note\nlet x: i32 = "unterminated');
  ...
  it('classifies grammar tokens across trivia and preserves UTF-16 ranges', () => {
    ...
    const tokens = tokenizeFlint(source);
...
export fn invalid() -> i32 { while true { return 1; } }`;
    const iteratorDocument = {
      uri: 'file:///workspace/iterator.flint',
      fileName: 'iterator.flint',
      text: source,
      version: 1,
    };
    ...
    expect(analysis.symbols.find((symbol) => symbol.name === 'Iterator<i32>')).toMatchObject({
      detail: 'Flint type Iterator<i32>',
    });
    ...
    expect(lexFlint(source, iteratorDocument.fileName).diagnostics).toEqual([]);
  });
...
`;
    ...
    expect(service.hover(editorDocument.uri, positionAtOffset(source, source.indexOf('Pair<T:') + 1))).toMatchObject({
      contents: ['Flint type Pair<T: Equatable>'],
    });
...
export fn caller(value: i32) -> i32 { return add(value); }`;
  ...
  it('resolves qualified cross-module definitions, references, and safe workspace renames', async () => {
    ...
    const definition = service.definition('file:///workspace/main.flint', positionAtOffset(mainSource, callOffset + 1));
    expect(definition).toEqual([
      expect.objectContaining({
        uri: 'file:///workspace/helper.flint',
        range: expect.objectContaining({
    ...
    expect(service.declaration('file:///workspace/main.flint', positionAtOffset(mainSource, callOffset + 1))).toEqual(
      definition,
    );

    const referencesFromDeclaration = service.references(
      'file:///workspace/helper.flint',
  ...
  it('checks imported signatures and refreshes importer diagnostics when dependencies change', async () => {
    const helperUri = 'file:///workspace/helper.flint';
    ...
    expect(service.diagnose(mainUri).diagnostics).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ code: expect.stringMatching(/FLINT-(ABI|TYPE)/u) })]),
    );
    ...
    service.updateDocument({ uri: helperUri, fileName: 'helper.flint', text: incompatibleHelper, version: 2 });
    await service.refreshWorkspace(helperUri);

    expect(service.diagnose(mainUri).diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: 'FLINT-TYPE-005', message: expect.stringContaining('string') }),
      ]),
    );
    expect(service.inlayHints(mainUri)).toEqual(
  ...
  it('uses imported signatures for inlay hints and preserves export filtering across alias changes', async () => {
    ...
    service.updateDocument({ uri: mainUri, fileName: 'main.flint', text: incompatibleMain, version: 2 });
    await service.refreshWorkspace(mainUri);
    expect(service.diagnose(mainUri).diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: 'FLINT-TYPE-005', message: expect.stringContaining("expected 'i32'") }),
        expect.objectContaining({
          code: 'FLINT-TYPE-005',
    ...
    const aliasedMain = 'import "./helper.flint" as h;\nexport fn entry(value: i32) -> i32 { return h.run(value); }';
    service.updateDocument({ uri: mainUri, fileName: 'main.flint', text: aliasedMain, version: 3 });
    await service.refreshWorkspace(mainUri);
    expect(service.diagnose(mainUri).diagnostics).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ code: expect.stringMatching(/^FLINT-(ABI|TYPE)/u) })]),
    );
    ...
    service.updateDocument({ uri: mainUri, fileName: 'main.flint', text: privateMain, version: 4 });
    await service.refreshWorkspace(mainUri);
    expect(service.diagnose(mainUri).diagnostics).toEqual(
      expect.arrayContaining([expect.objectContaining({ code: 'FLINT-ABI-004' })]),
    );
  ...
  it('does not load unrelated listed files during a URI-scoped refresh', async () => {
    const activeUri = 'file:///workspace/main.flint';
    const unrelatedUri = 'file:///workspace/generated.flint';
    const files = new Map<string, string>([
...
export fn entry(value: i32) -> i32 { return run(value); }`;
  ...
  it('keeps malformed documents query-safe and refreshes changed workspace modules', async () => {
    const files = new Map<string, string>([
      ['file:///workspace/helper.flint', 'export fn run(value: i32) -> i32 { return value + 1; }'],
      [
        'file:///workspace/main.flint',
        'import "./helper.flint" as helper;\nexport fn entry(value: i32) -> i32 { return helper.run(value); }',
      ],
    ...
    service.openDocument({
      uri: 'file:///workspace/broken.flint',
      fileName: 'broken.flint',
      text: malformed,
      version: 1,
    });
    await service.refreshWorkspace('file:///workspace/broken.flint');
    expect(() => service.diagnose('file:///workspace/broken.flint')).not.toThrow();
    expect(() =>
      service.definition('file:///workspace/broken.flint', positionAtOffset(malformed, malformed.length - 1)),
    ).not.toThrow();
    expect(
      service.definition('file:///workspace/broken.flint', positionAtOffset(malformed, malformed.length - 1)),
    ).toEqual([]);
    expect(
      service.rename('file:///workspace/broken.flint', positionAtOffset(malformed, malformed.length - 1), 'fixed'),
    ).toBeUndefined();
    ...
    files.set('file:///workspace/helper.flint', updatedHelper);
    service.updateDocument({
      uri: 'file:///workspace/helper.flint',
      fileName: 'helper.flint',
      text: updatedHelper,
      version: 2,
    });
    notify?.({ uri: 'file:///workspace/helper.flint', kind: 'changed' });
    await service.refreshWorkspace('file:///workspace/helper.flint');
    ...
    expect(
      service
        .complete('file:///workspace/main.flint', positionAtOffset(main, main.indexOf('helper.run') + 'helper.'.length))
...
export fn second(value: Foo) -> Foo { return value; }`;
  ...
  it('drops stale parsed declarations when an open document becomes malformed', async () => {
    ...
    service.updateDocument({ uri, fileName: 'arithmetic.flint', text: malformed, version: 2 });
  ...
  it('recomputes source-import aliases after an open document changes', async () => {
    const files = new Map<string, string>([
      ['file:///workspace/one.flint', 'export fn one() -> i32 { return 1; }'],
      ['file:///workspace/two.flint', 'export fn two(value: i32) -> i32 { return value; }'],
    ...
    const mainUri = 'file:///workspace/main.flint';
    const updated = 'import "./two.flint" as dep;\nexport fn entry(value: i32) -> i32 { return dep.two(value); }';
    service.updateDocument({ uri: mainUri, fileName: 'main.flint', text: updated, version: 2 });
    await service.refreshWorkspace(mainUri);
    const target = service.definition(mainUri, positionAtOffset(updated, updated.lastIndexOf('two') + 1));
    expect(target).toEqual([expect.objectContaining({ uri: 'file:///workspace/two.flint' })]);

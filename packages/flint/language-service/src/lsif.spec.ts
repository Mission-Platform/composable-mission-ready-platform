...
import { createFlintLanguageService, createFlintLsif, serializeFlintLsif, type FlintDocument } from '.';

function makeWorkspace(): {
  readonly documents: readonly FlintDocument[];
  readonly service: ReturnType<typeof createFlintLanguageService>;
} {
  const documents: readonly FlintDocument[] = [
    {
      uri: 'file:///workspace/main.flint',
      fileName: 'main.flint',
      version: 1,
      text: `import "./helper.flint" as helper;
/** Calls the helper. */
...
describe('Forge Web Script LSIF export', () => {
  it('emits a structurally connected graph for workspace semantic facts', async () => {
    ...
    const graph = createFlintLsif({ service, documents, projectRoot: 'file:///workspace/../workspace' });
    ...
    expect(
      graph.vertices.find((vertex) => vertex.label === 'document' && vertex.uri === 'file:///workspace/main.flint'),
    ).toMatchObject({
      languageId: 'flint',
      moduleId: 'main',
    });
  ...
  it('is byte-for-byte deterministic independent of input document order', async () => {
    ...
    const firstGraph = createFlintLsif({ service: first.service, documents: first.documents });
    const secondGraph = createFlintLsif({
      service: second.service,
      documents: [...second.documents].toReversed(),
    });
    expect(serializeFlintLsif(firstGraph)).toBe(serializeFlintLsif(secondGraph));
    expect(JSON.parse(`[${serializeFlintLsif(firstGraph).replaceAll('\n', ',')}]`)).toHaveLength(

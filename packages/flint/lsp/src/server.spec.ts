...
import { createFlintLspServer } from './server.js';

import type { InitializeParams } from 'vscode-languageserver/node';

const uri = 'file:///workspace/example.flint';
...
describe('Flint LSP server', () => {
  ...
  it('publishes protocol diagnostics and clears them on close', async () => {
    ...
    expect(published.at(-1)).toMatchObject({
      uri,
      diagnostics: expect.arrayContaining([expect.objectContaining({ code: 'FLINT-PARSE-017', source: 'flint' })]),
    });
  ...
  it('accepts private helpers in default protocol diagnostics', async () => {
    ...
    expect(published.at(-1)?.diagnostics).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ code: 'FLINT-ABI-003' })]),
    );
    await server.shutdown();
  });
...
export fn entry(value: i32) -> i32 {
  return helper(value);
    ...
    expect(
      server.definition({ textDocument: { uri: 'file:///workspace/missing.flint' }, position: helperPosition }),
    ).toEqual([]);
    expect(
      server.rename({
        textDocument: { uri: 'file:///workspace/missing.flint' },

...
describe('Flint stdio protocol', () => {
  it('handles initialize, open diagnostics, shutdown, and exit over framed messages', async () => {
    ...
    expect(diagnostics).toMatchObject({
      ...
      params: {
        diagnostics: expect.arrayContaining([expect.objectContaining({ source: 'flint', code: 'FLINT-PARSE-017' })]),
      },
    });
    expect(diagnostics.params?.diagnostics).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ source: 'flint', code: 'FLINT-ABI-003' })]),
    );
    ...
    expect(() =>
      server.completion({ textDocument: { uri: 'file:///workspace/main.flint' }, position: { line: 0, character: 0 } }),
    ).toThrow(/disposed/u);
    connection.dispose();
  });
});

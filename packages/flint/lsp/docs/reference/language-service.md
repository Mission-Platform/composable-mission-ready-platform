# Flint language tooling

Flint (`.flint`) has an editor-neutral language service, a stdio
Language Server Protocol (LSP) server, and a browser-facing Monaco adapter.
All three use the executable Flint v1 contract from
`@mission-platform/flint`, so diagnostics, source ranges, symbols,
...
AssemblyScript integrations. See [Flint v1](../../../flint/docs/reference/language.md)
...
forms, are specified in [the FWS language reference](../../../flint/docs/reference/language.md).
...
The server is published as `@mission-platform/flint-lsp` and
exposes the executable `flint-lsp`. It speaks standard LSP over
...
`flint-lsp` for installed packages. A checkout can use `node` and
...
`extensions/flint-vscode` release output with **Extensions: Install from VSIX**,
then reload VS Code. Opening a `.flint` file activates the extension. The
...
The extension contributes the `fws` language id, `.flint` filename association,
...
`flint.nodePath` defaults to `node` and must resolve to Node 24 or
newer. Leave `flint.serverPath` empty to use the packaged server;
...
for LSP tracing; startup failures are written to the **Flint
...
and runtime dependencies under `extensions/flint-vscode/server`. `package`
produces `extensions/flint-vscode/fws-vscode-0.1.0.vsix`; development sources
...
recognize `.flint` files.

The plugin maps `*.flint` to language id `fws` and starts one shared stdio
server for the project. IntelliJ configuration is provided exclusively by
**Settings | Tools | Flint**; there is no project-script or Flora
configuration path. Configure:

- **Node.js executable** — Node 24 or newer; defaults to `node`.
- **Language server command/path** — defaults to `flint-lsp` and
  ...
  `node_modules/@mission-platform/flint-lsp/dist/main.js` is also
  supported.
...
- **VS Code packaged server missing:** rebuild with
  ...
  `extensions/flint-vscode/server/dist/main.js` exists, or set
  `flint.serverPath` to a valid built entrypoint. Inspect the
  **Flint Language Server** output channel with tracing enabled.
- **IntelliJ server command not found:** install
  `@mission-platform/flint-lsp` in the opened project, ensure its
...
- **No diagnostics or completion:** verify the file is named `.flint`, the
...
`validateFlint`. A capability import that is not allowed by the
workspace produces the stable ABI diagnostic `FLINT-ABI-002`; export-related
requirements use the corresponding `FLINT-ABI-003` contract. Capability names
...
`FLINT-ABI-003`. Private functions remain available to same-module symbols,
...
`@mission-platform/flint` continues to require `export fn` for every
...
The LSP server publishes `source: "flint"`. The phase and hint are
also included in the diagnostic `data` object. Typical stable code families
are:

| Code family     | Phase        | Meaning                                                                                         |
| --------------- | ------------ | ----------------------------------------------------------------------------------------------- |
| `FLINT-LEX-*`   | `lex`        | Invalid characters/escapes, raw string line terminators, or unterminated strings/block comments |
| `FLINT-PARSE-*` | `parse`      | Invalid module, declaration, statement, or expression syntax                                    |
| `FLINT-TYPE-*`  | `type-check` | Invalid types, names, operators, arguments, or returns                                          |
| `FLINT-ABI-*`   | `abi`        | Duplicate names, denied capabilities, exports, or imports                                       |

Malformed input is still tokenized and analyzed where parser recovery allows
it. For example, malformed source may produce `FLINT-PARSE-017` while retaining
...
and trailing backslashes produce lexical diagnostics (`FLINT-LEX-004` or
`FLINT-LEX-005`). Lexer and diagnostic spans are bounded by the source length;
...
`packages/content/content/content/src/monaco/flint.ts`. `ForgeMonacoEditor` loads
the adapter lazily when `language="flint"`; Monaco remains a type-only import in
...
Set `flint={false}` to disable the automatic integration. Otherwise,
the component registers the `fws` language and `.flint` extension, uses Monaco's
...
`registerFlintLanguage` is safe to call when `fws` is already
...
| --------------- | ----------------------------------------- | ----------------------------------------------------------------------------- | ------------------ |
| Node LSP client | `RootBoundedFlintWorkspaceHost`           | Canonicalized configured filesystem roots; outside reads are rejected         | stdio LSP          |
| Monaco/browser  | Application-supplied `FlintWorkspaceHost` | The host decides which URIs/files/options to expose; no filesystem assumption | In-process adapter |
...
files; a failure limited to those existing files is not a Flint
language-tooling regression. The authoritative language fixture expectations
remain in `../../../flint/src/fixtures/bootstrap.ts` and the
[language reference](../../../flint/docs/reference/language.md).

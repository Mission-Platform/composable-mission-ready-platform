# Herramientas del lenguaje Flint
...
Flint (`.flint`) tiene un servicio de lenguaje neutral para el editor, un stdio
Servidor Language Server Protocol (LSP) y un adaptador Monaco orientado al navegador.
Los tres utilizan el contrato ejecutable Flint v1 de
`@mission-platform/flint`, por lo que diagnósticos, rangos de fuentes, símbolos,
...
Integraciones de AssemblyScript. Ver [Forjar script web v1](../../../../../flint/docs/locales/es/reference/language.md)
...
formularios, se especifican en [la referencia del lenguaje FWS](../../../../../flint/docs/locales/es/reference/language.md).
...
El servidor está publicado como `@mission-platform/flint-lsp` y
expone el ejecutable `flint-lsp`. Habla LSP estándar sobre
...
`flint-lsp` para paquetes instalados. Un pago puede utilizar `node` y
...
`extensions/flint-vscode` salida de lanzamiento con **Extensiones: Instalar desde VSIX**,
luego recarga VS Code. Al abrir un archivo `.flint` se activa la extensión. el
...
La extensión aporta la identificación de idioma `fws`, la asociación de nombre de archivo `.flint`,
...
`flint.nodePath` tiene como valor predeterminado `node` y debe resolverse como Node 24 o
más nuevo. Deje `flint.serverPath` vacío para utilizar el servidor empaquetado;
...
para rastreo de LSP; los fallos de inicio se escriben en **Flint
...
y dependencias de tiempo de ejecución en `extensions/flint-vscode/server`. `package`
produce `extensions/flint-vscode/fws-vscode-0.1.0.vsix`; fuentes de desarrollo
...
reconocer archivos `.flint`.

El complemento asigna `*.flint` al ID de idioma `fws` e inicia un stdio compartido
...
- **Ruta/comando del servidor de idioma**: el valor predeterminado es `flint-lsp` y
  ...
  `node_modules/@mission-platform/flint-lsp/dist/main.js` también es
  apoyado.
...
- **Falta el servidor empaquetado VS Code:** reconstruir con
  ...
  `extensions/flint-vscode/server/dist/main.js` existe o está configurado
  `flint.serverPath` a un punto de entrada construido válido. inspeccionar el
  **Flint Language Server** canal de salida con seguimiento habilitado.
- **Comando del servidor IntelliJ no encontrado:** instalar
  `@mission-platform/flint-lsp` en el proyecto abierto, asegúrese de que
...
- **Sin diagnóstico ni finalización:** verifique que el archivo se llame `.flint`, el
...
`validateFlint`. Una importación de capacidad que no está permitida por el
el espacio de trabajo produce el diagnóstico ABI estable `FLINT-ABI-002`; relacionado con la exportación
requisitos utilice el contrato `FLINT-ABI-003` correspondiente. Nombres de capacidad
...
`FLINT-ABI-003`. Las funciones privadas permanecen disponibles para los símbolos del mismo módulo,
...
`@mission-platform/flint` continúa requiriendo `export fn` para cada
...
El servidor LSP publica `source: "flint"`. La fase y la pista son
...
| ------------------ | ------------ | ------------------------------------------------------------------------------------------------------------------------ |
| `FLINT-LEX-*`      | `lex`        | Caracteres/escapes no válidos, terminadores de línea de cadena sin terminar o cadenas/comentarios de bloque sin terminar |
| `FLINT-PARSE-*`    | `parse`      | Sintaxis de módulo, declaración, sentencia o expresión no válida                                                         |
| `FLINT-TYPE-*`     | `type-check` | Tipos, nombres, operadores, argumentos o devoluciones no válidos                                                         |
| `FLINT-ABI-*`      | `abi`        | Nombres duplicados, capacidades denegadas, exportaciones o importaciones                                                 |

La entrada con formato incorrecto todavía se tokeniza y analiza cuando la recuperación del analizador lo permite
eso. Por ejemplo, una fuente con formato incorrecto puede producir `FLINT-PARSE-017` manteniendo
...
y las barras invertidas al final producen diagnósticos léxicos (`FLINT-LEX-004` o
`FLINT-LEX-005`). Los intervalos de diagnóstico y Lexer están limitados por la longitud de la fuente;
...
`packages/content/content/content/src/monaco/flint.ts`. `ForgeMonacoEditor` cargas
el adaptador perezosamente cuando `language="flint"`; Mónaco sigue siendo una importación tipográfica en
...
Configure `flint={false}` para deshabilitar la integración automática. De lo contrario,
el componente registra el idioma `fws` y la extensión `.flint`, utiliza el idioma de Mónaco
...
`registerFlintLanguage` es seguro llamar cuando `fws` ya está
...
| ---------------- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------- | -------------------- |
| Node Cliente LSP | `RootBoundedFlintWorkspaceHost`                      | Raíces del sistema de archivos configurado canonicalizado; se rechazan las lecturas externas    | estándar LSP         |
| Mónaco/navegador | `FlintWorkspaceHost` proporcionado por la aplicación | El host decide qué URI/archivos/opciones exponer; sin suposiciones sobre el sistema de archivos | Adaptador en proceso |
...
permanecen en `../../../flint/src/fixtures/bootstrap.ts` y el
[referencia del idioma](../../../../../flint/docs/locales/es/reference/language.md).

# Strumenti per il linguaggio Flint
...
Flint (`.flint`) dispone di un servizio linguistico indipendente dall'editor, un stdio
Server Language Server Protocol (LSP) e un adattatore Monaco rivolto al browser.
Tutti e tre utilizzano il contratto eseguibile Flint v1 da
`@mission-platform/flint`, quindi diagnostica, intervalli di origine, simboli,
...
Integrazioni AssemblyScript. Vedere [Forgia WebScript v1](../../../../../flint/docs/locales/it/reference/language.md)
...
moduli, sono specificati in [il riferimento del linguaggio FWS](../../../../../flint/docs/locales/it/reference/language.md).
...
Il server è pubblicato come `@mission-platform/flint-lsp` e
espone l'eseguibile `flint-lsp`. Parla LSP standard
...
`flint-lsp` per i pacchetti installati. Un checkout può utilizzare `node` e
...
`extensions/flint-vscode` output di rilascio con **Estensioni: installazione da VSIX**,
quindi ricaricare VS Code. L'apertura di un file `.flint` attiva l'estensione. Il
...
L'estensione contribuisce con l'id della lingua `fws`, l'associazione del nome file `.flint`,
...
Per impostazione predefinita `flint.nodePath` è `node` e deve risolversi in Node 24 o
più recente. Lasciare vuoto `flint.serverPath` per utilizzare il server in pacchetto;
...
per il tracciamento LSP; gli errori di avvio vengono scritti nel **Flint
...
e dipendenze di runtime in `extensions/flint-vscode/server`. `package`
produce `extensions/flint-vscode/fws-vscode-0.1.0.vsix`; fonti di sviluppo
...
riconoscere i file `.flint`.

Il plugin mappa `*.flint` sull'ID lingua `fws` e avvia uno stdio condiviso
...
- **Comando/percorso server lingua**: il valore predefinito è `flint-lsp` e
  ...
  Anche `node_modules/@mission-platform/flint-lsp/dist/main.js` lo è
  supportato.
...
- **Server in pacchetto VS Code mancante:** ricostruisci con
  ...
  `extensions/flint-vscode/server/dist/main.js` esiste o è impostato
  `flint.serverPath` a un punto di ingresso creato valido. Ispezionare il
  Canale di output **Flint Language Server** con traccia abilitata.
- **Comando del server IntelliJ non trovato:** installa
  `@mission-platform/flint-lsp` nel progetto aperto, assicurati che sia
...
- **Nessuna diagnostica o completamento:** verificare che il file sia denominato `.flint`, il
...
`validateFlint`. Un'importazione di capacità non consentita da
l'area di lavoro produce la diagnostica ABI stabile `FLINT-ABI-002`; legati alle esportazioni
i requisiti utilizzano il corrispondente contratto `FLINT-ABI-003`. Nomi di capacità
...
`FLINT-ABI-003`. Le funzioni private rimangono disponibili per i simboli dello stesso modulo,
...
`@mission-platform/flint` continua a richiedere `export fn` per ogni
...
Il server LSP pubblica `source: "flint"`. La fase e il suggerimento sono
incluso anche nell'oggetto diagnostico `data`. Tipiche famiglie di codici stabili
sono:

| Famiglia di codici | Fase         | Significato                                                                                                                 |
| ------------------ | ------------ | --------------------------------------------------------------------------------------------------------------------------- |
| `FLINT-LEX-*`      | `lex`        | Caratteri/escape non validi, terminatori di riga di stringhe non elaborate o stringhe/commenti di blocco senza terminazione |
| `FLINT-PARSE-*`    | `parse`      | Sintassi di modulo, dichiarazione, istruzione o espressione non valida                                                      |
| `FLINT-TYPE-*`     | `type-check` | Tipi, nomi, operatori, argomenti o risultati non validi                                                                     |
| `FLINT-ABI-*`      | `abi`        | Nomi duplicati, funzionalità, esportazioni o importazioni negate                                                            |

L'input non valido viene ancora tokenizzato e analizzato laddove consentito dal ripristino del parser
esso. Ad esempio, un'origine non valida può produrre `FLINT-PARSE-017` pur conservando
...
e le barre rovesciate finali producono diagnosi lessicali (`FLINT-LEX-004` o
`FLINT-LEX-005`). Gli intervalli lexer e diagnostici sono limitati dalla lunghezza dell'origine;
...
`packages/content/content/content/src/monaco/flint.ts`. `ForgeMonacoEditor` viene caricato
l'adattatore pigramente quando `language="flint"`; Monaco rimane un'importazione di solo tipo
...
Impostare `flint={false}` per disabilitare l'integrazione automatica. Altrimenti,
il componente registra la lingua `fws` e l'estensione `.flint`, utilizza Monaco
...
È sicuro chiamare `registerFlintLanguage` quando `fws` è già presente
...
| --------------- | ----------------------------------------- | ----------------------------------------------------------------------------------- | --------------------- |
| Node Client LSP | `RootBoundedFlintWorkspaceHost`           | Root del filesystem configurato canonicamente; le letture esterne vengono rifiutate | stdio LSP             |
| Monaco/browser  | Applicazione fornita `FlintWorkspaceHost` | L'host decide quali URI/file/opzioni esporre; nessun presupposto sul filesystem     | Adattatore in-process |
...
rimangono in `../../../flint/src/fixtures/bootstrap.ts` e il
[riferimento linguistico](../../../../../flint/docs/locales/it/reference/language.md).

...
Forge 웹 스크립트(`.flint`)에는 편집자 중립 언어 서비스인 stdio가 있습니다.
LSP(Language Server Protocol) 서버 및 브라우저 연결 Monaco 어댑터.
세 가지 모두에서 실행 가능한 Flint v1 계약을 사용합니다.
`@mission-platform/flint`, 따라서 진단, 소스 범위, 기호,
...
AssemblyScript 통합. 보다 [포지 웹 스크립트 v1](../../../../../flint/docs/locales/ko/reference/language.md)
언어 및 ABI 참조용입니다.
...
양식은 다음에 지정되어 있습니다. [FWS 언어 참조](../../../../../flint/docs/locales/ko/reference/language.md).
...
서버는 `@mission-platform/flint-lsp`으로 게시되고
실행 파일 `flint-lsp`을 노출합니다. 표준 LSP를 통해 말합니다.
...
설치된 패키지의 경우 `flint-lsp`입니다. 체크아웃에서는 `node`을 사용할 수 있으며
대신 빌드된 진입점:
...
**확장: VSIX에서 설치**가 포함된 `extensions/flint-vscode` 릴리스 출력,
그런 다음 VS Code를 다시 로드하세요. `.flint` 파일을 열면 확장이 활성화됩니다. 는
기본 시작 경로는 VSIX에 번들로 제공되는 서버이며 확장은
stdio를 통해 구성된 Node 실행 파일로 시작합니다.

확장은 `fws` 언어 ID, `.flint` 파일 이름 연결,
...
`flint.nodePath`의 기본값은 `node`이며 Node으로 확인되어야 합니다. 24 또는
최신. 패키지된 서버를 사용하려면 `flint.serverPath`를 비워 두세요.
...
`extensions/flint-vscode/server` 아래의 런타임 종속성. `package`
`extensions/flint-vscode/fws-vscode-0.1.0.vsix`를 생성합니다. 개발 소스
...
- **VS Code 패키지 서버 누락:** 다음으로 재구축
  ...
  `extensions/flint-vscode/server/dist/main.js`가 존재하거나 설정되었습니다.
  `flint.serverPath`을 유효한 빌드 진입점으로 설정합니다. 검사
  **Flint Language Server** 추적이 활성화된 출력 채널.
- **IntelliJ 서버 명령을 찾을 수 없습니다:** 설치
  열린 프로젝트에서 `@mission-platform/flint-lsp`가 있는지 확인하세요.
  `node_modules/.bin`가 있거나 명시적 명령/경로를 구성하십시오. 는
  플러그인은 검색된 프로젝트 루트와 제안된 설치 경로를 보고합니다.
- **진단 또는 완료 없음:** 파일 이름이 `.flint`인지 확인하세요.
...
`validateFlint`. 에서 허용되지 않는 기능 가져오기
작업 공간은 안정적인 ABI 진단 `FLINT-ABI-002`을 생성합니다. 수출관련
요구 사항은 해당 `FLINT-ABI-003` 계약을 사용합니다. 기능 이름
...
`FLINT-ABI-003`. 동일한 모듈 기호에는 전용 기능을 계속 사용할 수 있습니다.
...
`@mission-platform/flint`에는 계속해서 `export fn`가 필요합니다.
...
LSP 서버는 `source: "flint"`을 게시합니다. 단계와 힌트는 다음과 같습니다.
진단 `data` 개체에도 포함됩니다. 일반적인 안정 코드 계열
다음과 같습니다:

| 코드 계열       | 단계         | 의미                                                                              |
| --------------- | ------------ | --------------------------------------------------------------------------------- |
| `FLINT-LEX-*`   | `lex`        | 잘못된 문자/이스케이프, 원시 문자열 줄 종결자 또는 종료되지 않은 문자열/블록 주석 |
| `FLINT-PARSE-*` | `parse`      | 잘못된 모듈, 선언, 문 또는 식 구문                                                |
| `FLINT-TYPE-*`  | `type-check` | 잘못된 유형, 이름, 연산자, 인수 또는 반환                                         |
| `FLINT-ABI-*`   | `abi`        | 중복된 이름, 거부된 기능, 내보내기 또는 가져오기                                  |

파서 복구가 허용되는 경우 잘못된 입력은 여전히 토큰화 및 분석됩니다.
그것. 예를 들어 잘못된 소스는 `FLINT-PARSE-017`을 생성할 수 있지만
...
후행 백슬래시는 어휘 진단(`FLINT-LEX-004` 또는
`FLINT-LEX-005`). 렉서 및 진단 범위는 소스 길이로 제한됩니다.
클라이언트는 이를 UTF-16 LSP 범위로 직접 안전하게 변환할 수 있습니다.

## 모나코 어댑터 내장

브라우저 어댑터는 `@mission-platform/content`에 의해 내보내지고 다음 위치에 있습니다.
`packages/content/content/content/src/monaco/flint.ts`. `ForgeMonacoEditor` 로드
`language="flint"`일 때 어댑터가 게으르게; 모나코는 여전히 유형만 수입하고 있습니다.
...
자동 통합을 비활성화하려면 `flint={false}`을 설정하십시오. 그렇지 않으면,
구성 요소는 `fws` 언어 및 `.flint` 확장을 등록하고 Monaco의
...
`fws`이 이미 있을 때 `registerFlintLanguage`을 호출해도 안전합니다.
...
| ------------------- | -------------------------------------- | ----------------------------------------------------------------- | ------------------ |
| Node LSP 클라이언트 | `RootBoundedFlintWorkspaceHost`        | 정규화된 구성된 파일 시스템 루트 외부 읽기가 거부됩니다           | 스튜디오 LSP       |
| 모나코/브라우저     | 애플리케이션 제공 `FlintWorkspaceHost` | 호스트는 노출할 URI/파일/옵션을 결정합니다. 파일 시스템 가정 없음 | 프로세스 내 어댑터 |
...
`../../../flint/src/fixtures/bootstrap.ts`에 남아 있고
[언어 참조](../../../../../flint/docs/locales/ko/reference/language.md).

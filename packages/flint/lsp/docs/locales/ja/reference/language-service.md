...
Flint (`.flint`) には、エディター中立の言語サービス、stdio があります。
Language Server Protocol (LSP) サーバー、およびブラウザー向け Monaco アダプター。
3 つすべてで、実行可能な Flint v1 コントラクトを使用します。
`@mission-platform/flint`、つまり診断、ソース範囲、シンボル、
...
AssemblyScript の統合。見る [Forge Web スクリプト v1](../../../../../flint/docs/locales/ja/reference/language.md)
...
形式は、で指定されます [FWS 言語リファレンス](../../../../../flint/docs/locales/ja/reference/language.md)。
...
サーバーは `@mission-platform/flint-lsp` として公開されており、
実行可能ファイル `flint-lsp` を公開します。標準の LSP を話します
...
インストールされているパッケージの場合は `flint-lsp`。チェックアウトでは `node` と
代わりに構築されたエントリポイント:
...
**拡張機能: VSIX からインストール** を使用した `extensions/flint-vscode` リリース出力、
次に、VS Code をリロードします。 `.flint` ファイルを開くと、拡張機能が有効になります。の
デフォルトの起動パスは VSIX にバンドルされているサーバーであり、拡張子は
stdio 経由で構成された Node 実行可能ファイルを使用して起動します。

この拡張子は、`fws` 言語 ID、`.flint` ファイル名の関連付け、
...
`flint.nodePath` のデフォルトは `node` であり、Node 24 に解決される必要があります。
新しい。パッケージ化されたサーバーを使用するには、`flint.serverPath` を空のままにしておきます。
...
LSP トレース用。起動の失敗は **Flint に書き込まれます
...
`extensions/flint-vscode/server` の下のランタイム依存関係。 `package`
`extensions/flint-vscode/fws-vscode-0.1.0.vsix` を生成します。開発ソース
...
プラグインは、`*.flint` を言語 ID `fws` にマップし、1 つの共有 stdio を開始します。
...
- **VS Code パッケージ サーバーが見つかりません:** を使用して再構築します
  ...
  `extensions/flint-vscode/server/dist/main.js` が存在する、または設定されている
  `flint.serverPath` を有効なビルド済みエントリポイントに設定します。を検査します。
  トレースが有効になっている **Flint Language Server** 出力チャネル。
- **IntelliJ サーバー コマンドが見つかりません:** インストール
  開いたプロジェクトの `@mission-platform/flint-lsp` を確認してください。
  `node_modules/.bin` が存在するか、明示的なコマンド/パスを構成します。の
  プラグインは、検索されたプロジェクト ルートと推奨されるインストール パスを報告します。
- **診断も完了もありません:** ファイルの名前が `.flint` であることを確認します。
...
`validateFlint`。によって許可されていない機能のインポート
ワークスペースは安定した ABI 診断 `FLINT-ABI-002` を生成します。輸出関連
要件では、対応する `FLINT-ABI-003` コントラクトを使用します。機能名
...
`FLINT-ABI-003`。プライベート関数は同じモジュールのシンボルで引き続き使用できます。
...
`@mission-platform/flint` では、引き続きすべての場合に `export fn` が必要です
...
LSP サーバーは `source: "flint"` を発行します。フェーズとヒントは、
診断 `data` オブジェクトにも含まれています。典型的な安定したコードファミリー
は:

| コードファミリー | フェーズ     | 意味                                                                                      |
| ---------------- | ------------ | ----------------------------------------------------------------------------------------- |
| `FLINT-LEX-*`    | `lex`        | 無効な文字/エスケープ、生の文字列行終端文字、または終了していない文字列/ブロック コメント |
| `FLINT-PARSE-*`  | `parse`      | 無効なモジュール、宣言、ステートメント、または式の構文です。                              |
| `FLINT-TYPE-*`   | `type-check` | 無効な型、名前、演算子、引数、または戻り値                                                |
| `FLINT-ABI-*`    | `abi`        | 重複した名前、拒否された機能、エクスポートまたはインポート                                |

不正な入力は引き続きトークン化され、パーサーの回復が可能な場合には分析されます。
それ。たとえば、不正な形式のソースによって次のようなエラーが発生する可能性があります。 `FLINT-PARSE-017` 保持しながら
...
末尾のバックスラッシュは語彙診断を生成します (`FLINT-LEX-004` または
`FLINT-LEX-005`)。レクサーおよび診断スパンは、ソースの長さによって制限されます。
クライアントは、それらを安全に UTF-16 LSP 範囲に直接変換できます。

## Monaco アダプターの組み込み

ブラウザ アダプタは `@mission-platform/content` によってエクスポートされ、次の場所に存在します。
`packages/content/content/content/src/monaco/flint.ts`。 `ForgeMonacoEditor` ロード
`language="flint"` の場合、アダプターは遅延します。モナコは依然として型のみの輸入品です
...
`flint={false}` を設定して自動統合を無効にします。それ以外の場合は、
コンポーネントは `fws` 言語と `.flint` 拡張子を登録し、Monaco の言語を使用します。
...
`fws` がすでに存在する場合、`registerFlintLanguage` は安全に呼び出すことができます。
...
| --------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------- |
| Node LSP クライアント | `RootBoundedFlintWorkspaceHost`             | 正規化された構成済みファイルシステムのルート。外部読み取りは拒否されます。 stdio LSP             |
| モナコ/ブラウザ       | アプリケーション提供の `FlintWorkspaceHost` | どの URI/ファイル/オプションを公開するかはホストが決定します。ファイルシステムを前提としていない | インプロセスアダプター |
...
ファイル。これらの既存のファイルに限定された障害は Flint ではありません
言語ツールの回帰。権威ある言語フィクスチャーの期待
`../../../flint/src/fixtures/bootstrap.ts` に残り、
[言語リファレンス](../../../../../flint/docs/locales/ja/reference/language.md)。

...
Flint (`.flint`) verfügt über einen editorneutralen Sprachdienst, einen stdio
Language Server Protocol (LSP)-Server und ein browserseitiger Monaco-Adapter.
Alle drei nutzen den ausführbaren Flint v1-Vertrag von
`@mission-platform/flint`, also Diagnose, Quellbereiche, Symbole,
...
AssemblyScript-Integrationen. Sehen [Flint v1](../../../../../flint/docs/locales/de/reference/language.md)
...
Formen, sind in angegeben [die FWS-Sprachreferenz](../../../../../flint/docs/locales/de/reference/language.md).
...
Der Server wird als `@mission-platform/flint-lsp` und veröffentlicht
Macht die ausführbare Datei `flint-lsp` verfügbar. Es spricht Standard-LSP über
...
`flint-lsp` für installierte Pakete. Eine Kasse kann `node` und verwenden
...
`extensions/flint-vscode` Release-Ausgabe mit **Erweiterungen: Von VSIX installieren**,
Laden Sie dann den VS-Code neu. Durch Öffnen einer `.flint`-Datei wird die Erweiterung aktiviert. Die
...
Die Erweiterung trägt die Sprach-ID `fws` und die Dateinamenzuordnung `.flint` bei.
...
`flint.nodePath` ist standardmäßig `node` und muss in Node 24 oder aufgelöst werden
neuer. Lassen Sie `flint.serverPath` leer, um den Paketserver zu verwenden.
...
und Laufzeitabhängigkeiten unter `extensions/flint-vscode/server`. `package`
erzeugt `extensions/flint-vscode/fws-vscode-0.1.0.vsix`; Entwicklungsquellen
...
erkennt `.flint`-Dateien.

Das Plugin ordnet `*.flint` der Sprach-ID `fws` zu und startet ein gemeinsames stdio
...
- **Befehl/Pfad des Sprachservers** – standardmäßig `flint-lsp` und
  ...
  `node_modules/@mission-platform/flint-lsp/dist/main.js` ist auch
  unterstützt.
...
- **VS-Code-Paketserver fehlt:** neu erstellen mit
  ...
  `extensions/flint-vscode/server/dist/main.js` ist vorhanden oder festgelegt
  `flint.serverPath` zu einem gültigen erstellten Einstiegspunkt. Überprüfen Sie die
  **Flint Language Server**-Ausgabekanal mit aktivierter Ablaufverfolgung.
- **IntelliJ-Serverbefehl nicht gefunden:** install
  Stellen Sie sicher, dass `@mission-platform/flint-lsp` im geöffneten Projekt vorhanden ist
...
- **Keine Diagnose oder Abschluss:** Überprüfen Sie, ob die Datei den Namen `.flint` trägt
...
`validateFlint`. Ein Fähigkeitsimport, der nicht zulässig ist
Arbeitsbereich erzeugt die stabile ABI-Diagnose `FLINT-ABI-002`; exportbezogen
Anforderungen verwenden den entsprechenden `FLINT-ABI-003`-Vertrag. Fähigkeitsnamen
...
`FLINT-ABI-003`. Private Funktionen bleiben für Symbole desselben Moduls verfügbar.
...
`@mission-platform/flint` erfordert weiterhin `export fn` für alle
...
Der LSP-Server veröffentlicht `source: "flint"`. Die Phase und der Hinweis sind
auch im Diagnoseobjekt `data` enthalten. Typische stabile Codefamilien
sind:

| Codefamilie     | Phase        | Bedeutung                                                                                                                          |
| --------------- | ------------ | ---------------------------------------------------------------------------------------------------------------------------------- |
| `FLINT-LEX-*`   | `lex`        | Ungültige Zeichen/Escapezeichen, rohe Zeichenfolgenzeilen-Abschlusszeichen oder nicht abgeschlossene Zeichenfolgen/Blockkommentare |
| `FLINT-PARSE-*` | `parse`      | Ungültige Modul-, Deklarations-, Anweisungs- oder Ausdruckssyntax                                                                  |
| `FLINT-TYPE-*`  | `type-check` | Ungültige Typen, Namen, Operatoren, Argumente oder Rückgaben                                                                       |
| `FLINT-ABI-*`   | `abi`        | Doppelte Namen, verweigerte Funktionen, Exporte oder Importe                                                                       |

Fehlerhafte Eingaben werden weiterhin tokenisiert und analysiert, sofern die Parser-Wiederherstellung dies zulässt
es. Beispielsweise kann eine fehlerhafte Quelle beim Beibehalten `FLINT-PARSE-017` erzeugen
...
und abschließende Backslashes erzeugen eine lexikalische Diagnose (`FLINT-LEX-004` oder
`FLINT-LEX-005`). Lexer- und Diagnosespannen sind durch die Quelllänge begrenzt;
...
`packages/content/content/content/src/monaco/flint.ts`. `ForgeMonacoEditor` wird geladen
der Adapter träge, wenn `language="flint"`; Monaco bleibt ein reiner Typimport
...
Legen Sie `flint={false}` fest, um die automatische Integration zu deaktivieren. Ansonsten,
Die Komponente registriert die Sprache `fws` und die Erweiterung `.flint` und verwendet die von Monaco
...
`registerFlintLanguage` kann sicher aufgerufen werden, wenn `fws` bereits vorhanden ist
...
| --------------- | ------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------ |
| Node LSP-Client | `RootBoundedFlintWorkspaceHost`                         | Kanonisierte konfigurierte Dateisystem-Roots; Externe Lesevorgänge werden abgelehnt                   | stdio LSP          |
| Monaco/Browser  | Von der Anwendung bereitgestelltes `FlintWorkspaceHost` | Der Host entscheidet, welche URIs/Dateien/Optionen verfügbar gemacht werden; keine Dateisystemannahme | In-Process-Adapter |
...
bleiben in `../../../flint/src/fixtures/bootstrap.ts` und die
[Sprachreferenz](../../../../../flint/docs/locales/de/reference/language.md).

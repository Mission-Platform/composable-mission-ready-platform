...
Flint (`.flint`) heeft een editor-neutrale taalservice, een stdio
Language Server Protocol (LSP)-server en een browsergerichte Monaco-adapter.
Alle drie gebruiken ze het uitvoerbare Flint v1-contract van
`@mission-platform/flint`, dus diagnostiek, bronbereiken, symbolen,
...
AssemblyScript-integraties. Zien [Forge WebScript v1](../../../../../flint/docs/locales/nl/reference/language.md)
...
formulieren, zijn gespecificeerd in [de FWS-taalreferentie](../../../../../flint/docs/locales/nl/reference/language.md).
...
De server wordt gepubliceerd als `@mission-platform/flint-lsp` en
toont het uitvoerbare bestand `flint-lsp`. Het spreekt standaard LSP over
...
`flint-lsp` voor geïnstalleerde pakketten. Een kassa kan `node` en
...
`extensions/flint-vscode` geeft uitvoer vrij met **Extensies: installeren vanaf VSIX**,
Laad vervolgens VS Code opnieuw. Het openen van een `.flint`-bestand activeert de extensie. De
...
De extensie draagt de `fws` taal-ID, `.flint` bestandsnaamassociatie bij,
...
`flint.nodePath` is standaard `node` en moet worden omgezet in Node 24 of
nieuwer. Laat `flint.serverPath` leeg om de pakketserver te gebruiken;
...
voor LSP-tracering; opstartfouten worden naar het **Flint geschreven
...
en runtime-afhankelijkheden onder `extensions/flint-vscode/server`. `package`
produceert `extensions/flint-vscode/fws-vscode-0.1.0.vsix`; ontwikkelingsbronnen
...
herkent `.flint`-bestanden.

De plug-in wijst `*.flint` toe aan taal-ID `fws` en start een gedeelde stdio
...
- **Taalserveropdracht/pad** — standaard ingesteld op `flint-lsp` en
  ...
  `node_modules/@mission-platform/flint-lsp/dist/main.js` is dat ook
  ondersteund.
...
- **VS Code-verpakte server ontbreekt:** opnieuw opgebouwd met
  ...
  `extensions/flint-vscode/server/dist/main.js` bestaat of is ingesteld
  `flint.serverPath` naar een geldig gebouwd toegangspunt. Inspecteer de
  **Flint Language Server**-uitvoerkanaal met tracering ingeschakeld.
- **IntelliJ-serveropdracht niet gevonden:** installeren
  `@mission-platform/flint-lsp` in het geopende project, zorg ervoor dat dit het geval is
...
- **Geen diagnose of voltooiing:** controleer of het bestand de naam `.flint` heeft, de
...
`validateFlint`. Een mogelijkhedenimport die niet is toegestaan door de
werkruimte produceert de stabiele ABI-diagnostiek `FLINT-ABI-002`; exportgerelateerd
vereisten gebruiken het overeenkomstige `FLINT-ABI-003`-contract. Namen van mogelijkheden
...
`FLINT-ABI-003`. Privéfuncties blijven beschikbaar voor symbolen van dezelfde module,
...
`@mission-platform/flint` blijft `export fn` vereisen voor elke
...
De LSP-server publiceert `source: "flint"`. De fase en hint zijn
ook opgenomen in het diagnostische `data`-object. Typische stabiele codefamilies
zijn:

| Codeer familie  | Fase         | Betekenis                                                                                                       |
| --------------- | ------------ | --------------------------------------------------------------------------------------------------------------- |
| `FLINT-LEX-*`   | `lex`        | Ongeldige tekens/escapes, onbewerkte tekenreeksregelbeëindigers of niet-afgesloten tekenreeksen/blokopmerkingen |
| `FLINT-PARSE-*` | `parse`      | Ongeldige syntaxis van module, declaratie, instructie of expressie                                              |
| `FLINT-TYPE-*`  | `type-check` | Ongeldige typen, namen, operators, argumenten of retourneert                                                    |
| `FLINT-ABI-*`   | `abi`        | Dubbele namen, geweigerde mogelijkheden, export of import                                                       |

Misvormde invoer wordt nog steeds getokeniseerd en geanalyseerd waar parserherstel dit mogelijk maakt
het. Een verkeerd opgemaakte bron kan bijvoorbeeld `FLINT-PARSE-017` produceren terwijl deze behouden blijft
...
en achterliggende backslashes produceren lexicale diagnostiek (`FLINT-LEX-004` of
`FLINT-LEX-005`). Lexer- en diagnostische overspanningen worden begrensd door de bronlengte;
...
`packages/content/content/content/src/monaco/flint.ts`. `ForgeMonacoEditor` geladen
de adapter lui wanneer `language="flint"`; Monaco blijft een type-only import
...
Stel `flint={false}` in om de automatische integratie uit te schakelen. Anders,
de component registreert de `fws`-taal en de `.flint`-extensie, gebruikt Monaco's
...
`registerFlintLanguage` kan veilig worden aangeroepen als `fws` dat al is
...
| --------------- | ------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ---------------- |
| Node LSP-client | `RootBoundedFlintWorkspaceHost`                   | Gecanoniseerde geconfigureerde bestandssysteemwortels; externe lezingen worden afgewezen             | stdio LSP        |
| Monaco/browser  | Door de applicatie geleverde `FlintWorkspaceHost` | De host beslist welke URI's/bestanden/opties beschikbaar worden gesteld; geen bestandssysteemaanname | In-procesadapter |
...
bestanden; een fout die beperkt is tot die bestaande bestanden is geen Flint
taal-tooling regressie. De gezaghebbende taal is een vast onderdeel van de verwachtingen
blijven in `../../../flint/src/fixtures/bootstrap.ts` en de
[taalreferentie](../../../../../flint/docs/locales/nl/reference/language.md).

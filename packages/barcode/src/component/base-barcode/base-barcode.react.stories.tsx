import { Barcode } from '@mission-platform/barcode/react';

import type { Meta, StoryObj } from '@storybook/react-vite';

/**
 * `Barcode` is the **React** build of the write-once `BaseBarcode` in
 * `@mission-platform/barcode`. The payload is encoded entirely on the client by
 * the WebAssembly `@mission-platform/barcode` encoder and drawn as a crisp SVG;
 * the `error` emit becomes the `onError` callback prop. Authored once in the
...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `Barcode` — authored once in the neutral JSX dialect and shipped to both React (this story, via `@mission-platform/barcode/react`) and Vue 3 (`@mission-platform/barcode/vue`). The payload is encoded entirely on the client by the WebAssembly `@mission-platform/barcode` encoder and drawn as a crisp SVG. Opt into a save/copy toolbar via `showActions` (or the individual `show*Button` props). Styling comes from the co-located `base-barcode.module.scss`.',
      },
    },
  },
  argTypes: {

...
import styles from './icon-split.module.scss';
...
/**
 * `IconSplit` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-split.module.scss`.
 */
export function IconSplit(properties: Readonly<IconSplitProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div classNames={styles['base-icon-split']}>
      <svg

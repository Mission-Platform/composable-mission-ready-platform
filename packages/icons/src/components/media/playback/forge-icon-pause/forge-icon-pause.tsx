...
import styles from './forge-icon-pause.module.scss';
 ...
 */
export function ForgeIconPause(properties: Readonly<IconPauseProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-pause']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-pause')} />
      </svg>
    </div>
  );
}

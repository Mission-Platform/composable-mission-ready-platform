...
import styles from './forge-icon-image.module.scss';
 ...
 */
export function ForgeIconImage(properties: Readonly<IconImageProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-image']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-image')} />
      </svg>
    </div>
  );
}

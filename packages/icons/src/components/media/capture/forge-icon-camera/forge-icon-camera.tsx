...
import styles from './forge-icon-camera.module.scss';
 ...
 */
export function ForgeIconCamera(properties: Readonly<IconCameraProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-camera']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-camera')} />
      </svg>
    </div>
  );
}

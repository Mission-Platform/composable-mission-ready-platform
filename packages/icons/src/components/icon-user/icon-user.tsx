...
import styles from './icon-user.module.scss';
...
/**
 * `IconUser` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-user.module.scss`.
 */
export function IconUser(properties: Readonly<IconUserProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div classNames={styles['base-icon-user']}>
      <svg

...
import IconDownload from './icon.vue';

describe('IconDownload', () => {
  it('renders without errors', () => {
    const wrapper = mount(IconDownload);
    expect(wrapper.exists()).toBe(true);
  });

  it('applies size prop', () => {
    const wrapper = mount(IconDownload, { props: { size: 32 } });
  ...
  it('applies ariaLabel when provided', () => {
    const wrapper = mount(IconDownload, { props: { ariaLabel: 'Test label' } });
    const svg = wrapper.find('svg');
  ...
  it('hides from a11y when ariaLabel is undefined', () => {
    const wrapper = mount(IconDownload, { props: { ariaLabel: undefined } });
    const svg = wrapper.find('svg');

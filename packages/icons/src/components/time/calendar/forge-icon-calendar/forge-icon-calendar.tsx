...
import styles from './forge-icon-calendar.module.scss';
 ...
 */
export function ForgeIconCalendar(properties: Readonly<IconCalendarProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-calendar']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-calendar')} />
      </svg>
    </div>
  );
}

...
import styles from './forge-icon-clock.module.scss';
 ...
 */
export function ForgeIconClock(properties: Readonly<IconClockProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-clock']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-clock')} />
      </svg>
    </div>
  );
}

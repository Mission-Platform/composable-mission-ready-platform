...
describe('IconWarning', () => {
  it('renders without errors', () => {
    const wrapper = mount(IconWarning);
    expect(wrapper.exists()).toBe(true);
  });

  it('applies numeric size as px', () => {
    const wrapper = mount(IconWarning, { props: { size: 32 } });
  ...
  it('applies named size token', () => {
    const wrapper = mount(IconWarning, { props: { size: 'lg' } });
    expect(wrapper.find('svg').attributes('width')).toContain('mp-size-icon-lg');
  });

  it('applies ariaLabel when provided', () => {
    const wrapper = mount(IconWarning, { props: { ariaLabel: 'Test label' } });
    const svg = wrapper.find('svg');
  ...
  it('hides from a11y when ariaLabel is undefined', () => {
    const wrapper = mount(IconWarning, { props: { ariaLabel: undefined } });
    const svg = wrapper.find('svg');

...
import styles from './icon-align-right.module.scss';

export interface IconAlignRightProperties extends MpProperties {
...
/**
 * `IconAlignRight` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-align-right.module.scss`.
 */
export function IconAlignRight(properties: Readonly<IconAlignRightProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div classNames={styles['base-icon-align-right']}>
      <svg
      ...
      >
        <path d="M3 6H21M9 12H21M6 18H21" />
      </svg>
    </div>
  );
}

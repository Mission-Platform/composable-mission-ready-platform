...
describe('IconSettings', () => {
  it('renders without errors', () => {
    const wrapper = mount(IconSettings);
    expect(wrapper.exists()).toBe(true);
  });

  it('applies size prop', () => {
    const wrapper = mount(IconSettings, { props: { size: 32 } });
  ...
  it('applies ariaLabel when provided', () => {
    const wrapper = mount(IconSettings, { props: { ariaLabel: 'Test label' } });
    const svg = wrapper.find('svg');
  ...
  it('hides from a11y when ariaLabel is undefined', () => {
    const wrapper = mount(IconSettings, { props: { ariaLabel: undefined } });
    const svg = wrapper.find('svg');

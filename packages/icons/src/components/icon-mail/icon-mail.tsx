...
import styles from './icon-mail.module.scss';
...
/**
 * `IconMail` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-mail.module.scss`.
 */
export function IconMail(properties: Readonly<IconMailProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['base-icon-mail']}>
      <svg

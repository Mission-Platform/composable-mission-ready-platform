...
import styles from './icon-geodesic.module.scss';
...
/**
 * `IconGeodesic` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-geodesic.module.scss`.
 */
export function IconGeodesic(properties: Readonly<IconGeodesicProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div classNames={styles['base-icon-geodesic']}>
      <svg

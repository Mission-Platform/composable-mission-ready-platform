...
import styles from './icon-table-row-remove.module.scss';
...
/**
 * `IconTableRowRemove` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-table-row-remove.module.scss`.
 */
export function IconTableRowRemove(properties: Readonly<IconTableRowRemoveProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div classNames={styles['base-icon-table-row-remove']}>
      <svg

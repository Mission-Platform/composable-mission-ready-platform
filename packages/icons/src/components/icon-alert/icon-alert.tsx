...
import styles from './icon-alert.module.scss';
...
/**
 * `IconAlert` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-alert.module.scss`.
 */
export function IconAlert(properties: Readonly<IconAlertProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div classNames={styles['base-icon-alert']}>
      <svg

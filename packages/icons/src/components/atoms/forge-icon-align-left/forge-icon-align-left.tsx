...
import styles from './forge-icon-align-left.module.scss';

export interface IconAlignLeftProperties extends MpProperties {
 ...
 */
export function ForgeIconAlignLeft(properties: Readonly<IconAlignLeftProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-align-left']}>
      <svg
      ...
      >
        <path d="M3 6H21M3 12H15M3 18H18" />
      </svg>
    </div>
  );
}

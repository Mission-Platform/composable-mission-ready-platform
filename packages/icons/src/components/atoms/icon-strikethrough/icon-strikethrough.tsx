...
import styles from './icon-strikethrough.module.scss';
...
/**
 * `IconStrikethrough` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-strikethrough.module.scss`.
 */
export function IconStrikethrough(properties: Readonly<IconStrikethroughProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['base-icon-strikethrough']}>
      <svg

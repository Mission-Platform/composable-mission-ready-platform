...
import styles from './forge-icon-refresh.module.scss';

export interface IconRefreshProperties extends MpProperties {
 ...
 */
export function ForgeIconRefresh(properties: Readonly<IconRefreshProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-refresh']}>
      <svg
      ...
      >
        ...
        <path d="M3.51 9A9 9 0 0 1 15 3.05M21 12A9 9 0 0 1 9 20.94" />
      </svg>
    </div>
  );
}

...
import styles from './forge-icon-draw-line.module.scss';

export interface IconDrawLineProperties extends MpProperties {
 ...
 */
export function ForgeIconDrawLine(properties: Readonly<IconDrawLineProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-draw-line']}>
      <svg
      ...
      >
        <polyline points="3,19 9,9 15,14 21,5" />
        <circle
          fill={color}
          cx={3}
          cy={19}

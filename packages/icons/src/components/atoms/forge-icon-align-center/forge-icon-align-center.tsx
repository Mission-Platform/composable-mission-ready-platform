...
import styles from './forge-icon-align-center.module.scss';

export interface IconAlignCenterProperties extends MpProperties {
 ...
 */
export function ForgeIconAlignCenter(properties: Readonly<IconAlignCenterProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-align-center']}>
      <svg
      ...
      >
        <path d="M3 6H21M6 12H18M4 18H20" />
      </svg>
    </div>
  );
}

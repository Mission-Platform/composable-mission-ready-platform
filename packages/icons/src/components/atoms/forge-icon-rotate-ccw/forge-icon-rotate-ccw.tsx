...
import styles from './forge-icon-rotate-ccw.module.scss';

export interface IconRotateCcwProperties extends MpProperties {
 ...
 */
export function ForgeIconRotateCcw(properties: Readonly<IconRotateCcwProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-rotate-ccw']}>
      <svg
      ...
      >
        <path d="M3 2v6h6" />
        <path d="M3 8A9 9 0 1 1 4.64 14.64" />
      </svg>
    </div>
  );
}

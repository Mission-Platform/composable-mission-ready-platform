...
import styles from './forge-icon-edit.module.scss';

export interface IconEditProperties extends MpProperties {
 ...
 */
export function ForgeIconEdit(properties: Readonly<IconEditProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-edit']}>
      <svg
      ...
      >
        ...
        <path d="M18.5 2.5A2.121 2.121 0 0 1 21 5L12 14L8 15L9 11Z" />
      </svg>
    </div>
  );
}

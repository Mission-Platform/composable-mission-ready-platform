...
import styles from './forge-icon-cloud.module.scss';

export interface IconCloudProperties extends MpProperties {
 ...
 */
export function ForgeIconCloud(properties: Readonly<IconCloudProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-cloud']}>
      <svg
      ...
      >
        <path d="M17.5 19a4.5 4.5 0 1 0-1.06-8.875 6 6 0 1 0-10.43 5.34A4 4 0 0 0 7 19h10.5Z" />
      </svg>
    </div>
  );
}

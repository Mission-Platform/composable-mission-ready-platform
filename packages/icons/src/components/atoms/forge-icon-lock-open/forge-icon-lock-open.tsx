...
import styles from './forge-icon-lock-open.module.scss';

export interface IconLockOpenProperties extends MpProperties {
 ...
 */
export function ForgeIconLockOpen(properties: Readonly<IconLockOpenProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-lock-open']}>
      <svg

...
import styles from './forge-icon-rotate-cw.module.scss';

export interface IconRotateCwProperties extends MpProperties {
 ...
 */
export function ForgeIconRotateCw(properties: Readonly<IconRotateCwProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-rotate-cw']}>
      <svg
      ...
      >
        <path d="M21 2v6h-6" />
        <path d="M21 8A9 9 0 1 0 19.36 14.64" />
      </svg>
    </div>
  );
}

...
import styles from './forge-icon-close.module.scss';
 ...
 */
export function ForgeIconClose(properties: Readonly<IconCloseProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-close']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-close')} />
      </svg>
    </div>
  );
}

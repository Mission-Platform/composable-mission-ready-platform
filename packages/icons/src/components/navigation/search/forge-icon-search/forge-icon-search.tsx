...
import styles from './forge-icon-search.module.scss';
 ...
 */
export function ForgeIconSearch(properties: Readonly<IconSearchProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-search']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-search')} />
      </svg>
    </div>
  );
}

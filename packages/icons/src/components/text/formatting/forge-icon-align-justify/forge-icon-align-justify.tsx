...
import styles from './forge-icon-align-justify.module.scss';
 ...
 */
export function ForgeIconAlignJustify(properties: Readonly<IconAlignJustifyProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-align-justify']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-align-justify')} />
      </svg>
    </div>
  );
}

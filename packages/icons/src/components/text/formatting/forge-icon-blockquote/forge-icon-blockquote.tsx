...
import styles from './forge-icon-blockquote.module.scss';
 ...
 */
export function ForgeIconBlockquote(properties: Readonly<IconBlockquoteProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-blockquote']}>
      <svg
        aria-hidden={!properties.ariaLabel}
        aria-label={properties.ariaLabel ?? 'Blockquote'}
        height={sizeValue}
        stroke={color}
        width={sizeValue}
        fill="none"
      ...
      >
        <use href={useIconHref('icon-blockquote')} />
      </svg>
    </div>
  );
}

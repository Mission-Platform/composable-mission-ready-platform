...
import styles from './forge-icon-bullet-list.module.scss';
 ...
 */
export function ForgeIconBulletList(properties: Readonly<IconBulletListProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-bullet-list']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-bullet-list')} />
      </svg>
    </div>
  );
}

...
import styles from './forge-icon-heading.module.scss';
 ...
 */
export function ForgeIconHeading(properties: Readonly<IconHeadingProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-heading']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-heading')} />
      </svg>
    </div>
  );
}

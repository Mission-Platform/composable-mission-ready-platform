...
import styles from './forge-icon-heading-five.module.scss';
 ...
 */
export function ForgeIconHeadingFive(properties: Readonly<IconHeadingFiveProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-heading-five']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-heading-five')} />
      </svg>
    </div>
  );
}

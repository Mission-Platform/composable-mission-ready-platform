...
import styles from './forge-icon-alert-critical.module.scss';
 ...
 */
export function ForgeIconAlertCritical(properties: Readonly<IconAlertCriticalProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-alert-critical']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-alert-critical')} />
      </svg>
    </div>
  );
}

...
import styles from './forge-icon-error.module.scss';
 ...
 */
export function ForgeIconError(properties: Readonly<IconErrorProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-error']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-error')} />
      </svg>
    </div>
  );
}

...
import styles from './forge-icon-notice.module.scss';
 ...
 */
export function ForgeIconNotice(properties: Readonly<IconNoticeProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-notice']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-notice')} />
      </svg>
    </div>
  );
}

...
import styles from './forge-icon-warning.module.scss';
 ...
 */
export function ForgeIconWarning(properties: Readonly<IconWarningProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-warning']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-warning')} />
      </svg>
    </div>
  );
}

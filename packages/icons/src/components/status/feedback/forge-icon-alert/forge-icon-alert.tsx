...
import styles from './forge-icon-alert.module.scss';
 ...
 */
export function ForgeIconAlert(properties: Readonly<IconAlertProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-alert']}>
      <svg
        aria-hidden={!properties.ariaLabel}
        aria-label={properties.ariaLabel ?? 'Alert'}
        height={sizeValue}
        stroke={color}
        width={sizeValue}
        fill="none"
      ...
      >
        <use href={useIconHref('icon-alert')} />
      </svg>
    </div>
  );
}

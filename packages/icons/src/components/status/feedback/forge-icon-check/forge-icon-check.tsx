...
import styles from './forge-icon-check.module.scss';
 ...
 */
export function ForgeIconCheck(properties: Readonly<IconCheckProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-check']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-check')} />
      </svg>
    </div>
  );
}

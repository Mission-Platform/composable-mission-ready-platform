...
import styles from './forge-icon-alert-neutral.module.scss';
 ...
 */
export function ForgeIconAlertNeutral(properties: Readonly<IconAlertNeutralProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-alert-neutral']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-alert-neutral')} />
      </svg>
    </div>
  );
}

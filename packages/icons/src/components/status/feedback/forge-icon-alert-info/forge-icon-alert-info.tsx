...
import styles from './forge-icon-alert-info.module.scss';
 ...
 */
export function ForgeIconAlertInfo(properties: Readonly<IconAlertInfoProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-alert-info']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-alert-info')} />
      </svg>
    </div>
  );
}

...
import IconTableColumnAdd from './Icon.vue'

describe('IconTableColumnAdd', () => {
  it('renders without errors', () => {
    const wrapper = mount(IconTableColumnAdd)
    expect(wrapper.exists()).toBe(true)
  })

  it('applies numeric size as px', () => {
    const wrapper = mount(IconTableColumnAdd, { props: { size: 32 } })
  ...
  it('applies named size token', () => {
    const wrapper = mount(IconTableColumnAdd, { props: { size: 'lg' } })
    expect(wrapper.find('svg').attributes('width')).toContain('mp-size-icon-lg')
  })

  it('applies ariaLabel when provided', () => {
    const wrapper = mount(IconTableColumnAdd, { props: { ariaLabel: 'Test label' } })
    const svg = wrapper.find('svg')
  ...
  it('hides from a11y when ariaLabel is undefined', () => {
    const wrapper = mount(IconTableColumnAdd, { props: { ariaLabel: undefined } })
    const svg = wrapper.find('svg')

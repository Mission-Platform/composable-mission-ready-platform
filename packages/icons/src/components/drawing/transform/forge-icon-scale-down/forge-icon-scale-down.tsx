...
import styles from './forge-icon-scale-down.module.scss';
 ...
 */
export function ForgeIconScaleDown(properties: Readonly<IconScaleDownProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-scale-down']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-scale-down')} />
      </svg>
    </div>
  );
}

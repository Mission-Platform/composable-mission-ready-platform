...
import styles from './forge-icon-draw-circle.module.scss';
 ...
 */
export function ForgeIconDrawCircle(properties: Readonly<IconDrawCircleProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-draw-circle']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-draw-circle')} />
      </svg>
    </div>
  );
}

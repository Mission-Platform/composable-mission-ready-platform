...
import styles from './forge-icon-draw-triangle.module.scss';
 ...
 */
export function ForgeIconDrawTriangle(properties: Readonly<IconDrawTriangleProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-draw-triangle']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-draw-triangle')} />
      </svg>
    </div>
  );
}

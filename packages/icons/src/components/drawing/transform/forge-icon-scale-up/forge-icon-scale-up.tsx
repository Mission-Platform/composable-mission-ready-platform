...
import styles from './forge-icon-scale-up.module.scss';
 ...
 */
export function ForgeIconScaleUp(properties: Readonly<IconScaleUpProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-scale-up']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-scale-up')} />
      </svg>
    </div>
  );
}

...
import styles from './forge-icon-pencil.module.scss';
 ...
 */
export function ForgeIconPencil(properties: Readonly<IconPencilProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-pencil']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-pencil')} />
      </svg>
    </div>
  );
}

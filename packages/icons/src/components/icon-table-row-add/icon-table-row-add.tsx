...
import styles from './icon-table-row-add.module.scss';
...
/**
 * `IconTableRowAdd` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-table-row-add.module.scss`.
 */
export function IconTableRowAdd(properties: Readonly<IconTableRowAddProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div classNames={styles['base-icon-table-row-add']}>
      <svg

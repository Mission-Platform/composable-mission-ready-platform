...
import styles from './forge-icon-map-pin.module.scss';
 ...
 */
export function ForgeIconMapPin(properties: Readonly<IconMapPinProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-map-pin']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-map-pin')} />
      </svg>
    </div>
  );
}

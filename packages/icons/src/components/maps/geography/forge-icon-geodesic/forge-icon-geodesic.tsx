...
import styles from './forge-icon-geodesic.module.scss';
 ...
 */
export function ForgeIconGeodesic(properties: Readonly<IconGeodesicProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-geodesic']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-geodesic')} />
      </svg>
    </div>
  );
}

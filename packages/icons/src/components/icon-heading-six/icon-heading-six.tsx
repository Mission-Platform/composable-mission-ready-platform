...
import styles from './icon-heading-six.module.scss';
...
/**
 * `IconHeadingSix` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-heading-six.module.scss`.
 */
export function IconHeadingSix(properties: Readonly<IconHeadingSixProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div classNames={styles['base-icon-heading-six']}>
      <svg

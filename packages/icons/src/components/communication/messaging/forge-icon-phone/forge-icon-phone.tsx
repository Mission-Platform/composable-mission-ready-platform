...
import styles from './forge-icon-phone.module.scss';
 ...
 */
export function ForgeIconPhone(properties: Readonly<IconPhoneProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-phone']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-phone')} />
      </svg>
    </div>
  );
}

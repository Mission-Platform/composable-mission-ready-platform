...
import styles from './forge-icon-send.module.scss';
 ...
 */
export function ForgeIconSend(properties: Readonly<IconSendProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-send']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-send')} />
      </svg>
    </div>
  );
}

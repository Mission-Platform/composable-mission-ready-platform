...
import styles from './forge-icon-bell.module.scss';
 ...
 */
export function ForgeIconBell(properties: Readonly<IconBellProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-bell']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-bell')} />
      </svg>
    </div>
  );
}

...
import styles from './icon-heading-four.module.scss';
...
/**
 * `IconHeadingFour` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-heading-four.module.scss`.
 */
export function IconHeadingFour(properties: Readonly<IconHeadingFourProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div classNames={styles['base-icon-heading-four']}>
      <svg

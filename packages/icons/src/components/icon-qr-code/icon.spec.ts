...
import IconQrCode from './icon.vue';

describe('IconQrCode', () => {
  it('renders without errors', () => {
    const wrapper = mount(IconQrCode);
    expect(wrapper.exists()).toBe(true);
  });

  it('applies size prop', () => {
    const wrapper = mount(IconQrCode, { props: { size: 32 } });
  ...
  it('applies ariaLabel when provided', () => {
    const wrapper = mount(IconQrCode, { props: { ariaLabel: 'Test label' } });
    const svg = wrapper.find('svg');
  ...
  it('hides from a11y when ariaLabel is undefined', () => {
    const wrapper = mount(IconQrCode, { props: { ariaLabel: undefined } });
    const svg = wrapper.find('svg');

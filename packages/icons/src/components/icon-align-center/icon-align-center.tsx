...
import styles from './icon-align-center.module.scss';
...
/**
 * `IconAlignCenter` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-align-center.module.scss`.
 */
export function IconAlignCenter(properties: Readonly<IconAlignCenterProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div classNames={styles['base-icon-align-center']}>
      <svg

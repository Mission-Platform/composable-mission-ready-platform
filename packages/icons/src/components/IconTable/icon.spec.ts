...
import IconTable from './Icon.vue';

describe('IconTable', () => {
  it('renders without errors', () => {
    const wrapper = mount(IconTable);
    expect(wrapper.exists()).toBe(true);
  });

  it('applies numeric size as px', () => {
    const wrapper = mount(IconTable, { props: { size: 32 } });
  ...
  it('hides from a11y when ariaLabel is undefined', () => {
    const wrapper = mount(IconTable, { props: { ariaLabel: undefined } });
    const svg = wrapper.find('svg');

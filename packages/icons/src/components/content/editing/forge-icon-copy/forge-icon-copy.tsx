...
import styles from './forge-icon-copy.module.scss';
 ...
 */
export function ForgeIconCopy(properties: Readonly<IconCopyProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-copy']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-copy')} />
      </svg>
    </div>
  );
}

...
import styles from './forge-icon-waypoint.module.scss';
...
export function ForgeIconWaypoint(properties: Readonly<IconWaypointProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-waypoint']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-waypoint')} />
      </svg>
    </div>
  );
}

...
import styles from './icon-underline.module.scss';
...
/**
 * `IconUnderline` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-underline.module.scss`.
 */
export function IconUnderline(properties: Readonly<IconUnderlineProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['base-icon-underline']}>
      <svg

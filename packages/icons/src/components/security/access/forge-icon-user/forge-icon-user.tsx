...
import styles from './forge-icon-user.module.scss';
 ...
 */
export function ForgeIconUser(properties: Readonly<IconUserProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-user']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-user')} />
      </svg>
    </div>
  );
}

...
import styles from './icon-clock.module.scss';
...
/**
 * `IconClock` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-clock.module.scss`.
 */
export function IconClock(properties: Readonly<IconClockProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div classNames={styles['base-icon-clock']}>
      <svg

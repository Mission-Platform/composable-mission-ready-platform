...
import styles from './forge-icon-table.module.scss';
 ...
 */
export function ForgeIconTable(properties: Readonly<IconTableProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-table']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-table')} />
      </svg>
    </div>
  );
}

...
import styles from './forge-icon-table-row-remove.module.scss';
 ...
 */
export function ForgeIconTableRowRemove(properties: Readonly<IconTableRowRemoveProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-table-row-remove']}>
      <svg
        aria-hidden={!properties.ariaLabel}
        aria-label={properties.ariaLabel ?? 'Remove Table Row'}
        height={sizeValue}
        stroke={color}
        width={sizeValue}
        fill="none"
      ...
      >
        <use href={useIconHref('icon-table-row-remove')} />
      </svg>
    </div>
  );
}

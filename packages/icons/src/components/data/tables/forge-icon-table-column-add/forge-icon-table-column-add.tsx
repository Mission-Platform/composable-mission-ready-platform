...
import styles from './forge-icon-table-column-add.module.scss';
 ...
 */
export function ForgeIconTableColumnAdd(properties: Readonly<IconTableColumnAddProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-table-column-add']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-table-column-add')} />
      </svg>
    </div>
  );
}

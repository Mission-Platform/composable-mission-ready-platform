...
import styles from './forge-icon-puzzle.module.scss';
 ...
 */
export function ForgeIconPuzzle(properties: Readonly<IconPuzzleProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-puzzle']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-puzzle')} />
      </svg>
    </div>
  );
}

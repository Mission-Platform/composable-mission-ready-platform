...
import styles from './forge-icon-settings.module.scss';
 ...
 */
export function ForgeIconSettings(properties: Readonly<IconSettingsProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-settings']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-settings')} />
      </svg>
    </div>
  );
}

...
import styles from './forge-icon-heart.module.scss';
 ...
 */
export function ForgeIconHeart(properties: Readonly<IconHeartProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div className={styles['forge-icon-heart']}>
      <svg
      ...
      >
        <use href={useIconHref('icon-heart')} />
      </svg>
    </div>
  );
}

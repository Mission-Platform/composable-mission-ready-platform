...
import styles from './icon-debug.module.scss';
...
/**
 * `IconDebug` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-debug.module.scss`.
 */
export function IconDebug(properties: Readonly<IconDebugProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div classNames={styles['base-icon-debug']}>
      <svg

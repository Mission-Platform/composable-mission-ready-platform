...
import styles from './icon-draw-circle.module.scss';
...
/**
 * `IconDrawCircle` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-draw-circle.module.scss`.
 */
export function IconDrawCircle(properties: Readonly<IconDrawCircleProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div classNames={styles['base-icon-draw-circle']}>
      <svg

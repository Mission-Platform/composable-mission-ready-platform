...
import styles from './icon-globe.module.scss';

export interface IconGlobeProperties extends MpProperties {
...
/**
 * `IconGlobe` — an icon authored once in the neutral JSX dialect.
 *
 ...
 * co-located CSS Module `icon-globe.module.scss`.
 */
export function IconGlobe(properties: Readonly<IconGlobeProperties>): MpElement {
  const size = properties.size ?? 'md';
  const sizeValue = typeof size === 'number' ? size : mapTokenToPixels(size);
  const color = properties.color ?? 'currentColor';

  return (
    <div classNames={styles['base-icon-globe']}>
      <svg

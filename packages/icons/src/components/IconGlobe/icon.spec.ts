...
describe('IconGlobe', () => {
  it('renders without errors', () => {
    const wrapper = mount(IconGlobe);
    expect(wrapper.exists()).toBe(true);
  });

  it('applies size prop', () => {
    const wrapper = mount(IconGlobe, { props: { size: 32 } });
  ...
  it('applies ariaLabel when provided', () => {
    const wrapper = mount(IconGlobe, { props: { ariaLabel: 'Test label' } });
    const svg = wrapper.find('svg');
  ...
  it('hides from a11y when ariaLabel is undefined', () => {
    const wrapper = mount(IconGlobe, { props: { ariaLabel: undefined } });
    const svg = wrapper.find('svg');

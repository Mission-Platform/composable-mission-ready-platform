...
export interface ForgeWebScriptWasmSsaPlan {
  ...
  readonly branchOutputs: ReadonlyMap<ForgeWebScriptWasmStatement, readonly (ForgeWebScriptWasmSsaBindings | undefined)[]>;
...
class BlockBuilder {
  private nextId = 0;
  private readonly mutable = new Map<number, { kind: ForgeWebScriptWasmSsaBlock['kind']; predecessors: Set<number>; successors: Set<number> }>();
...
function sameValue(values: readonly (ForgeWebScriptWasmSsaValue | undefined)[]): ForgeWebScriptWasmSsaValue | undefined {
...
function analyzeCfg(
  ...
  for (const statement of statements) {
    ...
    if (statement.kind === 'if') {
      ...
      const alternateExits = statement.alternate === undefined
        ? [alternate]
        : analyzeCfg(statement.alternate, builder, [alternate]);
    ...
    if (statement.kind === 'while' || statement.kind === 'for') {
      ...
      const bodyStatements = statement.kind === 'for'
...
export function lowerForgeWebScriptWasmFunctionToSsa(
  ...
  const analyze = (items: readonly ForgeWebScriptWasmStatement[], initial: Map<string, ForgeWebScriptWasmSsaValue>): AnalysisResult => {
    let bindings = new Map(initial);
    for (const statement of items) {
      entryBindings.set(statement, new Map(bindings));
      if (statement.kind === 'let') {
        const value = createValue(statement.name, statement.type.name, 'definition', statement.type.reference, statement.type.length);
        ...
        const alternate = statement.alternate === undefined
        ...
        const assigned = assignedNames(statement.kind === 'for'
          ? [...statement.body, statement.update].filter((value): value is ForgeWebScriptWasmStatement => value !== undefined)
          : statement.body);

...
// (bundler target) glue for the `matrix-code-encode` Rust crate.
//
...
import * as bindings from './wasm/matrix-code-encode_bg.js';
import wasmBase64 from './wasm/matrix-code-encode_bg.wasm';

import type * as WasmApi from './wasm/matrix-code-encode.js';
...
const instance = new WebAssembly.Instance(new WebAssembly.Module(toBytes(wasmBase64)), {
  './matrix-code-encode_bg.js': bindings as unknown as WebAssembly.ModuleImports,
});
 ...
 * buffer (row-major, `1` = dark). Returns `undefined` when the symbology is
 * unknown or the payload is invalid. The wasm module is already instantiated, so

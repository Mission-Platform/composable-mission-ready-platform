 ...
 * the leaf `ForgeMonacoEditor` is replaced by a faithful stand-in that preserves
 ...
 * whole surrounding graph — `ForgeWysiwygEditor` → `ForgeSchemaFormDialog` →
 * `ForgeSchemaForm` → `ForgeSelect` — is the real compiled build, which is where a
...
vi.mock('@mission-platform/components', async (importOriginal) => {
  ...
  // A faithful stand-in for `ForgeMonacoEditor`: a controlled textarea that (a)
  ...
  const ForgeMonacoEditorStub = defineComponent({
    name: 'ForgeMonacoEditorStub',
    props: {
  ...
  return { ...original, ForgeMonacoEditor: ForgeMonacoEditorStub };
});
...
describe('ForgeWysiwygEditor code-block dialog (compiled Vue build)', () => {
  afterEach(() => {
  ...
  it('does not lock up when the code-block language is changed and code is typed', async () => {
    ...
    const app = createApp({
      render: () => h(ForgeWysiwygEditor as never, { modelValue: '<p>hello</p>' }),
    });
    ...
    // Change the language via the language picker. `ForgeSelect` keeps a hidden

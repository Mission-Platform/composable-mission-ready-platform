...
export interface WysiwygEditorProperties extends MpProperties {
  /**
   * The editor contents as an HTML string (controlled).
  ...
  /** Fired with the next HTML (the controlled `v-model` update). */
  onUpdateModelValue?: (value: string) => void;
  /** Fired with the next HTML whenever the content changes. */
...
/**
 * `BaseWysiwygEditor` — a framework-agnostic WYSIWYG rich-text editor authored
 ...
 * pattern used by `BaseMonacoEditor`). Formatting is applied through the
 ...
 * **dropdown** ({@link BaseWysiwygToolbar}), and the live word/character counter
 * is its own customisable {@link BaseWysiwygStatusBar}.
 *
 ...
 *   attributes; the editor then portals a real `BaseCodeBlock` into each
 ...
 *   outlines it and shows a floating {@link BaseWysiwygBlockControls} bar to move
 *   the block up/down and change its alignment/justification.
 *
 * An optional HTML **source view** swaps the surface for a `BaseMonacoEditor`
 * with `spellCheck` — bringing Hunspell + Harper proofreading to the raw markup.
 */
export function BaseWysiwygEditor(properties: Readonly<WysiwygEditorProperties>): MpElement {
  const {
    modelValue = '',
    placeholder = '',
    readonly = false,
  ...
  // opened.
  const [codeDialogLanguage, setCodeDialogLanguage] = useState<string>('plaintext');
  ...
  const emitContent = (): void => {
    ...
    properties.onUpdateModelValue?.(html);
    properties.onChange?.(html);
  ...
  const refreshActiveState = (): void => {
    ...
    setActiveCommands(TOGGLE_COMMANDS.filter((command) => isCommandActive(documentReference, command)));
  ...
  const handleCodeDialogSubmit = (values: FormValues, isValid: boolean): void => {
    if (!isValid) {
      return;
    }
    ...
    const language = typeof values.language === 'string' && values.language.length > 0 ? values.language : 'plaintext';
    const host = surfaceReference.current;
    const documentReference = host?.ownerDocument;
    host?.focus();
  ...
  // Keep the code-block dialog's live language in sync with the picker. The
  // form owns its values internally, so we only observe the language field and
  // update local state when it actually changes — guarding against a re-render
  // per keystroke in the `code` field.
  const handleCodeDialogUpdate = (values: FormValues): void => {
    const next = typeof values.language === 'string' && values.language.length > 0 ? values.language : 'plaintext';
    if (next !== codeDialogLanguage) {
      setCodeDialogLanguage(next);
    }
  };

  // The JSON Schema driving the code-block dialog: a language picker plus the
  // Monaco-backed `code` editor field, whose syntax follows the picked
  // `language`. Memoised on the selected language so the underlying validator
  // is only rebuilt when the language changes (not on every render).
  const codeDialogSchema = useMemo<SchemaFormDefinition>(
    () => ({
      type: 'object',
      properties: {
        language: {
          type: 'string',
          title: labels.codeBlockLanguageLabel,
          enum: [...CODE_BLOCK_LANGUAGES],
          default: 'plaintext',
        },
        code: {
          type: 'string',
          title: labels.codeBlockCodeLabel,
          ui: { widget: 'code', language: codeDialogLanguage, rows: 12 },
        },
      },
      required: ['code'],
    }),
    [codeDialogLanguage, labels.codeBlockLanguageLabel, labels.codeBlockCodeLabel],
  );
  ...
  useEffect(() => {
    const host = surfaceReference.current;
    if (host && !sourceMode && serializeSurface(host) !== modelValue) {
      host.innerHTML = modelValue;
      rescanCodeBlocks();
    }
  ...
  return (
    <div className={[styles['wysiwyg'], { [styles['wysiwyg--disabled']]: disabled }]}>
        ...
        {sourceMode
          ...
          : codeBlocks.map((block) => (
              ...
              >
                <BaseCodeBlock
                  code={block.code}
                  language={block.language as CodeBlockLanguage}
                />
              </Teleport>
            ))}
      ...
      {readonly || !codeDialogOpen ? undefined : (
        <BaseSchemaFormDialog
          key={codeDialogKey}
          cancelLabel={labels.codeBlockCancel}
          modelValue={{ language: codeDialogLanguage, code: '' }}
          open={codeDialogOpen}
          schema={codeDialogSchema}
          submitLabel={labels.codeBlockInsert}
          title={labels.codeBlockDialogTitle}
          onClose={closeCodeDialog}
          onSubmit={handleCodeDialogSubmit}
          onUpdateModelValue={handleCodeDialogUpdate}

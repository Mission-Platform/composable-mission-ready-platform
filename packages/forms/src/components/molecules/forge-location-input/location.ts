...
// representations the `ForgeLocationInput` (and the schema-form `location`
// widget) understand:
//
...
function parseSexagesimal(text: string, axis: CoordinateAxis): number | undefined {
  ...
  const hemisphereLetter = hemMatch?.[0]?.toUpperCase();
  const negative = hemisphereLetter ? hemisphereLetter === 'S' || hemisphereLetter === 'W' : Number(deg) < 0;

...
import { FormBuilder } from '@mission-platform/forms/vue';

import type { SchemaFormDefinition } from './base-form-builder';
import type { Meta, StoryObj } from '@storybook/vue3-vite';

/**
 * `FormBuilder` is the Vue 3 build of the write-once `BaseFormBuilder` in this
...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `FormBuilder` — authored once in the neutral JSX dialect and shipped to both Vue 3 (this story, via `@mission-platform/forms/vue`) and React (`@mission-platform/forms/react`). Like the Vue original it is a visual authoring surface for JSON-Schema forms: a field **palette**, a tabbed centre (**Editor** canvas, a wizard-only **Steps** tab, a live **Preview**, and a **Schema** JSON tab), and an **inspector**. The whole field tree is emitted as a `SchemaFormDefinition` (built through the shared `@mission-platform/forms-core`) via `v-model`, ready to feed straight into `SchemaForm`. `@dnd-kit/vue` is replaced with native HTML5 drag-and-drop; the palette/canvas/properties/condition/steps sub-components are inlined. Styling comes from the co-located `base-form-builder.module.scss`.',
      },
    },
  },
  argTypes: {

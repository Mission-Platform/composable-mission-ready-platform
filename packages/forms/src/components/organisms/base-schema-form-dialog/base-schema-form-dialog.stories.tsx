...
import { SchemaFormDialog } from '@mission-platform/forms';

import type { SchemaFormDefinition } from '../base-schema-form';
import type { Meta, StoryObj } from '@mission-platform/storybook-framework';

/**
 * `SchemaFormDialog` is the write-once `BaseSchemaFormDialog` in
 ...
 */
const meta = {
  title: 'Organisms/Forms/BaseSchemaFormDialog',
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `SchemaFormDialog` — authored once in the neutral JSX dialect and shipped to all supported frameworks. It hosts a `SchemaForm` inside `BaseModal`; the primary action is the form\'s `type="submit"`, so submitting validates through the shared `@mission-platform/forms-core` engine and fires `onSubmit(values, isValid)`. Open state is controlled via `open` + `onUpdateOpen`/`onClose`. The `code` field renders a Monaco editor.',
      },
    },
  },
  args: {

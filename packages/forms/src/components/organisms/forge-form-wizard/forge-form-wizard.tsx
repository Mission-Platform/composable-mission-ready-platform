    ...
    <div className={[styles['forge-form-wizard'], sizeStyles[`forge-size--${size}`]]}>
      ...
      >
        {visibleSteps.map((step, index) => (
          <li
            key={step.id}
            className={styles['forge-form-wizard__step']}
          >
            <button
              aria-current={index === current ? 'step' : undefined}
              className={[
                styles['forge-form-wizard__step-btn'],
                {
                  [styles['forge-form-wizard__step-btn--active']]: index === current,
                  [styles['forge-form-wizard__step-btn--done']]: index < current,
                  [styles['forge-form-wizard__step-btn--error']]: !!step.error,
                },
              ]}
            ...
            >
              <span
                aria-hidden="true"
                className={styles['forge-form-wizard__step-index']}
              >
                {index + 1}
              </span>
              <span className={styles['forge-form-wizard__step-text']}>
                <ForgeTypography
                  as="span"
                  color="inherit"
                  variant="label"
      ...
      <div className={styles['forge-form-wizard__footer']}>
        <Slot name="footer" />
        <div className={styles['forge-form-wizard__nav']}>
          <button
            className={[styles['forge-form-wizard__button'], styles['forge-form-wizard__button--secondary']]}
            disabled={isFirst}
            type="button"
            onClick={previous}
          >
          ...
          <button
            className={[styles['forge-form-wizard__button'], styles['forge-form-wizard__button--primary']]}
            disabled={!canAdvance}
            type="button"
            onClick={next}
          >

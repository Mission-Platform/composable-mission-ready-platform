...
/**
 * `SchemaForm` is the Vue 3 build of the write-once `BaseSchemaForm` in this
...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `SchemaForm` — authored once in the neutral JSX dialect and shipped to both Vue 3 (this story, via `@mission-platform/forms/vue`) and React (`@mission-platform/forms/react`). Like the Vue original it is driven entirely by a **JSON Schema**: both the rendered fields and the validation rules are derived from it through the shared `@mission-platform/forms-core` package (so the two frameworks validate identically). A single object renders a one-step form; a top-level array renders a multi-step wizard. Fields support nested field sets, `ui.visibleWhen` conditional visibility, and Ajv validation surfaced as per-field errors. The dynamic `<component :is>` control resolution becomes a static `switch`; generated messages use the built-in English fallback (the neutral dialect has no i18n). Styling comes from the co-located `base-schema-form.module.scss`.',
      },
    },
  },
  render: (arguments_) => ({

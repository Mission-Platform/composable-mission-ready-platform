...
import { ForgeRangeInput, type RangeValue } from './forge-range-input';

/**
 * Exercises the **neutral** `ForgeRangeInput` authored in this package, rendering
 ...
 */
const ReactRangeInput = toReactComponent(ForgeRangeInput, 'RangeInput');
const VueRangeInput = toVueComponent(ForgeRangeInput, 'RangeInput');

describe('ForgeRangeInput authors the same component for React and Vue', () => {
  it('renders two slider thumbs reflecting the ordered bounds on both frameworks', async () => {
    ...
    for (const html of [react, vue]) {
      ...
      expect(html).toContain('forge-range-input__fill');
    }
  });

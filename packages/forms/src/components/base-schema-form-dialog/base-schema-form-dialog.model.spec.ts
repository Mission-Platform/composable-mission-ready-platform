 ...
 * `BaseSchemaForm` **by reference** therefore emitted `undefined`, silently
...
describe('BaseSchemaFormDialog forwards update:modelValue on the compiled Vue build', () => {
  afterEach(() => {
  ...
  it('re-emits the host update:modelValue when an inner field changes', async () => {
    ...
    const { BaseSchemaFormDialog } = (await import('@mission-platform/forms/vue')) as unknown as {
      BaseSchemaFormDialog: unknown;
    };
    ...
    const app = createApp({
      render: () =>
        h(BaseSchemaFormDialog as never, {
          open: true,

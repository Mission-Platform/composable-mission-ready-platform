...
```typescript
export type ForgeWebScriptTrapCode = | 'CapabilityDenied' | 'HostError' | 'InvalidAbi' | 'InvalidOwnership' | 'MemoryExhausted' | 'MemoryOutOfBounds' | 'GuestTrap' | 'BorrowViolation' | 'RegionExpired' | 'UseAfterRelease' | 'DoubleRelease';
```

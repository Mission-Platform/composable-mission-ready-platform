...
frames), and for keeping the scan pipeline inside one statically linked Flint/WebAssembly artifact.

> **Current implementation:** The scanner is shipped as a statically linked
> Flint graph under `src/fws`, with a dynamic source-module profile
...
> **ZXing port status (2026-09-12):** The scanner-owned Flint graph now contains
...
> points, metadata, and timestamp, while the current Flint wire format only
...
> independently, but combined QR linkage is blocked by Flint
> `FLINT-WASM-001`; the reduced 2D paths do not claim full ZXing parity.
...
Phase 1 replaced that with a single Flint `scan_and_decode` call (see §1); the
historical motivation below is kept as rationale, while the current source of
truth is the Flint graph and its Vitest conformance suite.
...
### Target architecture — one Flint call, image in, payload out

> **Status: implemented.** The scanner exports `scan_and_decode`, links the
> decoder Flint graphs directly, and the JS façade decodes through that single
...
`scan_and_decode(width, height, luma) -> Option<ScanOutcome>` runs the whole pipeline inside `src/fws/scanner.flint` and
returns the **decoded payload** directly (`value` is empty when a symbol is located but undecodable). The JS façade
(`scanner/index.ts`) is a thin marshalling layer over scanner-owned decoder Flint sources;
...
- **Done:** `packages/integrations/code-scanner/README.md` was updated — the stale "1D barcode decoder is still a scaffold, so
...
### Step 6 — wire the new formats into the JS façade + build the Flint artifact _(done)_
...
- **The scanner Flint artifact is built** from `src/fws/scanner.flint` by the Flint Vite plugin. The static profile
...
- **The Flint graph and façade suites** (`src/fws/scanner-graph.spec.ts` and
...
**Result:** `vitest` is green against the Flint artifact and the `tsc` build

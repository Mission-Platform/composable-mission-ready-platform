 ...
 */
const meta = {
  title: 'Molecules/Data Display/ForgeQrCode',
  component: ForgeQrCode,
  tags: ['autodocs'],
  parameters: {
    docs: {
      description: {
        component:
          'Cross-framework `ForgeQrCode` — authored once in the neutral JSX dialect and shipped to all supported frameworks. The payload is encoded entirely on the client by the WebAssembly `@mission-platform/qr-code` encoder and drawn as a crisp SVG. The `variant` prop switches between a standard square QR Code (`qr`), a compact square Micro QR Code (`micro`), and a wide Rectangular Micro QR / rMQR Code (`rmqr`). The original `computed` render becomes the neutral `useMemo`, and the `error` emit becomes the `onError` callback prop. Opt into a save/copy toolbar via `showActions` — `true` for every button, or an object such as `{ download: true }` to pick them individually. Styling comes from the co-located `forge-qr-code.module.scss`.',
      },
    },
  },
  argTypes: {
...
} satisfies Meta<typeof ForgeQrCode>;

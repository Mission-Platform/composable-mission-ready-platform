...
describe('ZXing foundation source contract', () => {
  const source = readFileSync(resolve(fwsDirectory, 'foundation.flint'), 'utf8');
  const scannerSource = readFileSync(resolve(fwsDirectory, 'scanner.flint'), 'utf8');
  ...
  it('does not use allocation or capability imports in the foundation', () => {
    expect(source).not.toMatch(/import\s+"(?!\.\/common\.flint)[^"]+"/);
  ...
  it('keeps the foundation reachable from the static scanner entry graph', () => {
    expect(scannerSource).toContain('import "./foundation.flint" as foundation;');

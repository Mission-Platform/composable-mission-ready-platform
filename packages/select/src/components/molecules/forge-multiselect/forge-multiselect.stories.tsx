...
/**
 * `ForgeMultiselect` is the write-once `ForgeMultiselect` component of `@mission-platform/select`. Selected values render as removable `Tag` chips
...
export const Expanded: Story = {
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(canvas.getByRole('combobox', { name: 'Toppings' }));
...
export const FocusVisible: Story = {
  play: async ({ canvasElement }) => {
    const combobox = within(canvasElement).getByRole('combobox', { name: 'Toppings' });
    await userEvent.tab();

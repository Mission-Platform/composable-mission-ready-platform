...
> 英語の原典: [docs/component-token-reference.md](../../component-token-reference.md)
> 言語: 日本語 (ja)

これは、Forge で作成されたコンポーネントの正規インベントリと Figma ハンドオフです。意図的に独立しています
生成されたフレームワーク アダプター: 同じエントリが適用されます Vue, React, Solid, Svelte、Web コンポーネント。
...
`color-scheme`, `light-dark()`, `[data-theme]`、 そして `.theme-*` サブツリーピン。コンシューマと Storybook は、
下の葉 `component` で `overrides.tokens.json`;オーバーライドは、生成されたトークン スタイルシートの後に適用されます。
...
コンポーネントによってサポートされる状態のみが以下にリストされています。 `expanded` 開示/選択サーフェスに使用されます。 `selected`
選択肢/タブ/ナビゲーション用、および `invalid` フォームの検証用。未使用の状態変数は必要ありません。
...
|アーティファクト |カウント |意味 |
| --------------------- | ----: | ------------------------------------------------------------------------------------ |
|コンポーネント TSX ソース |   249 |非ストーリー Forge および電子メール コンポーネント ソース |
...
- **Inherited-visual** — 独立したスタイルのホストをレンダリングしません。その姿は子から、親から、 `currentColor`、
  サードパーティのホスト/キャンバス、または構成されたコンポーネントのコントラクト。
- **動作のみ** — レンダリングまたはビューポートの動作を制御し、独自の視覚的な決定は行いません。

以下の各箇条書きは 1 つのインベントリ エントリです。ストーリーがマークされていない限り `story: missing`、コンポーネントには一致するものがあります
`<component>.stories.tsx` ソースの横にあります。パッケージ/レベルの見出しは、安定したソース パスのプレフィックスを提供します。
...
| `forge-typography`       |ビジュアル | `component.typography`                          | `as`、タイポグラフィーのバリエーション、色。デフォルト/リンク/無効 |
...
### 生物とテンプレート — `packages/components/src/components/{organisms,templates}/`
...
| `forge-menubar`            |ビジュアル | `component.navigation.menubar`                                  |項目、境界線、サイズ;デフォルト/ホバー/フォーカス表示/展開/無効 |
...
### フォーム — `packages/forms/src/components/`

すべてのフォームエントリは共有を使用します `component.field` 以下の契約に加えて、ラベル/ヘルパー/エラーの役割。ネイティブ
コントロールの状態は、コントロールがサポートする場合にのみ表現されます。

|レベル |コンポーネント (カンマ区切りの名前ごとに 1 つのエントリ) |分類・契約 |共有の外観プロパティと状態 |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
|原子 | `forge-checkbox`, `forge-input`, `forge-radio`, `forge-range-input`, `forge-rating`, `forge-slider`, `forge-switch`, `forge-textarea`                                                                                                                                                                                                                                     |ビジュアル / `component.checkable` チェックボックス/ラジオ/評価/スライダー/スイッチ用。 `component.input` 入力/範囲入力/テキストエリア用 | `size`、ラベル/値の小道具。サポートされている場合は、default/hover/active/focus-visible/disabled/invalid/selected |
|分子 | `forge-calendar`, `forge-color-input`, `forge-date-input`, `forge-date-range-input`, `forge-field-set`, `forge-file-input`, `forge-location-input`, `forge-multiselect`, `forge-number-stepper`, `forge-otp-input`, `forge-phone-input`, `forge-radio-group`, `forge-search-input`, `forge-segment-control`, `forge-select`, `forge-time-input`, `forge-time-range-input` |ビジュアル / `component.input`, `component.select`, `component.checkable`、 または `component.field` 構成された制御に従って | `size`, `disabled`、検証および選択の小道具。デフォルト/フォーカス表示/無効/展開/選択/無効 |
|生物 | `forge-date-time-range-input`, `forge-form-builder`, `forge-form-wizard`, `forge-schema-form-dialog`, `forge-schema-form`                                                                                                                                                                                                                                                 |ビジュアル / `component.field` + composed input/select/overlay contracts                                                      |スキーマ、ステップ、検証。 default/focus-visible/disabled/expanded/selected/invalid                          |

### アイコン — `packages/icons/src/components/`

All 106 icon entries are **inherited-visual**.グリフの使用 `currentColor`;サイズはコンシューマーによって制御されるか、マップされます。
`component.icon.size`。グリフごとの変数は受け取りません。それぞれに同じ場所にあるストーリーがあり、同じ内容に従います
親がその状態を公開するデフォルト/選択/無効な色の役割。
...
|コンテンツ/編集 | `forge-icon-copy`, `forge-icon-edit`, `forge-icon-eye`, `forge-icon-eye-off`, `forge-icon-redo`, `forge-icon-trash`, `forge-icon-undo`                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `typography/atoms`           | `forge-typography`                                                                                                                                 |ビジュアル | `component.typography`                                       |バリエーション、カラー、 `as`;デフォルト/リンク/無効 |
| `vcard`                      | `forge-icalendar`                                                                                                                                  |動作のみ |なし |カレンダーデータをシリアル化します。ビジュアルホストがありません |
| `vcard`                      | `forge-vcard`                                                                                                                                      |動作のみ |なし |連絡先データをシリアル化します。ビジュアルホストがありません |

## 電子メールのコンポーネント

`@mission-platform/email-components` TSX ソースが Forge で作成されているため、含まれています。電子メールクライアントはそうではありません
実行時カスタム プロパティを消費します。レンダラーは同じセマンティック ロールをインライン値に解決します。以下のすべてのエントリ
視覚的で用途が広い `component.email`、 と `component.button`, `component.typography`、 または `component.media` 記載のある箇所。
...
|分子 | `email-card`, `email-column`, `email-list`, `email-row`, `email-social-links` | `component.email`;リンクがインタラクティブなデフォルト/選択済み |
...
`content/molecules/forge-markdown/markdown-block`、 そして `content/molecules/forge-markdown/markdown-inline`;彼らの
視覚的な状態はその親ストーリーによって実行され、上では継承された視覚として文書化されています。

...
يجب أن يتواجد كل متجر في الدليل الفرعي المسمى الخاص به ضمن `src/stores/`، مصحوبًا بملف اختبار مشترك في الموقع و
برميل محلي.
...
1. **الحالة الخاصة**: احتفظ بالحالة ضمن نطاق الوحدة النمطية (قيم TypeScript البسيطة).
2. **الوصول إلى اللقطة**: قم بتوفير وظيفة `getSnapshot()` لاسترداد الحالة الحالية.
3. **الاشتراك**: قم بتوفير وظيفة `subscribe(listener)` التي تضيف رد اتصال إلى القائمة وترجع رسالة إلغاء الاشتراك
   وظيفة.
...
3. **أمان SSR**: حماية الوصول إلى واجهات برمجة تطبيقات المتصفح (على سبيل المثال، `localStorage`) بحيث يمكن تهيئة المتجر في Node.js
   بيئة.
4. **الاختبار الإلزامي**: يجب أن يكون لدى كل متجر ملف `.spec.ts` في موقع مشترك.

...
מדריך זה מספק פתרונות לבעיות נפוצות שנתקלות בהן במהלך הפיתוח, הבנייה והפריסה בתוך המשימה
...
- **אופטימיזציה של תמונות**: השתמש בפורמטים של WebP/AVIF וספק `srcset` לתמונות רספונסיביות.
- **טעינת משאבים מראש**: השתמש ב-`<link rel="preload">` עבור תמונת LCP או גופנים קריטיים.
- **צמצם את עבודת השרשור הראשי**: דחה JavaScript לא חיוני באמצעות `async` או `defer`.
...
3. חפש את "אלמנטים DOM מנותקים".

**פתרונות**:

- **ניקוי בחומרים קומפוזיציים**: נקה תמיד טיימרים והסר מאזיני אירועים ב-`onUnmounted`.
- **ניהול חנות**: ודא שהמצב התגובתי בפניה או בחנויות אחרות מנוקה כאשר אין צורך יותר.
- **השלכה מ-observables**: אם אתה משתמש ב-RxJS, ודא שכל המנויים בוטלו.

## בעיות בבנייה ובסביבת עבודה

### שגיאות מטמון של Turborepo

**בעיה**: שינויים אינם באים לידי ביטוי ב-build, או שה-build נכשל עם חפצים מיושנים.
...
**בעיה**: TypeScript או Vite לא יכולים למצוא חבילה המוגדרת בסביבת העבודה.

**פתרונות**:

1. ודא שהחבילה רשומה ב-`package.json` של סביבת העבודה הצורכת.
...
**בעיה**: Build נכשל ב-CI עם שגיאות TypeScript שלא מופיעות ב-IDE שלך.
...
- ודא שאתה משתמש בנתיב המוחלט לקובץ הבינארי node ובסקריפט בתצורת הלקוח שלך.
...
**סיבה**: פונקציית אסינכרון גרמה שגיאה שלא נתפסה. **תיקון**: עטוף תמיד שיחות אסינכרון בבלוקים `try/catch`.

...
2. **חשב את ה-hash של החפץ** באמצעות המאמת: `flint_verify_artifact(artifact)` מחזיר את ה-`contentHash`.
3. **השווה גיבובים**: אם הם תואמים, החפץ לא השתנה בטעות או בזדון מאז שהערך הצפוי תועד.
4. **אמת את המניפסט**: השתמש ב-`flint_inspect_manifest` כדי לבדוק יבוא, ייצוא, מטא נתונים ותאימות למדיניות באופן עצמאי.

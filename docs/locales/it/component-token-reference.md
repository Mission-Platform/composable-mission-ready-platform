...
> Fonte inglese: [docs/component-token-reference.md](../../component-token-reference.md)
> Lingua: Italiano (it)
...
gli adattatori framework generati: vale la stessa voce Vue, React, Solid, Sveltee componenti Web.
...
`color-scheme`, `light-dark()`, `[data-theme]`, E `.theme-*` perni del sottoalbero. I consumatori e Storybook possono sovrascrivere qualsiasi
foglia sottostante `component` In `overrides.tokens.json`; viene applicata una sostituzione dopo il foglio di stile del token generato.
...
Di seguito sono elencati solo gli stati supportati da un componente. `expanded` viene utilizzato per rivelare/selezionare superfici, `selected`
...
Ogni punto elenco riportato di seguito rappresenta una voce di inventario. A meno che una storia non sia segnata `story: missing`, il componente ha una corrispondenza
`<component>.stories.tsx` accanto alla fonte. Un'intestazione di pacchetto/livello fornisce il prefisso del percorso di origine stabile.

## `@mission-platform/components`

### Atomi — `packages/components/src/components/atoms/`
...
### Molecole — `packages/components/src/components/molecules/`
...
### Organismi e modelli — `packages/components/src/components/{organisms,templates}/`
...
### Moduli — `packages/forms/src/components/`

Tutte le voci del modulo utilizzano il file shared `component.field` ruoli di etichetta/aiuto/errore in aggiunta al contratto riportato di seguito. Nativo
...
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| atomi | `forge-checkbox`, `forge-input`, `forge-radio`, `forge-range-input`, `forge-rating`, `forge-slider`, `forge-switch`, `forge-textarea`                                                                                                                                                                                                                                     | visivo / `component.checkable` per casella di controllo/radio/valutazione/cursore/interruttore; `component.input` for input/range-input/textarea | `size`, oggetti di scena etichetta/valore; predefinito/al passaggio del mouse/attivo/focus-visibile/disabilitato/non valido/selezionato dove supportato |
| molecole | `forge-calendar`, `forge-color-input`, `forge-date-input`, `forge-date-range-input`, `forge-field-set`, `forge-file-input`, `forge-location-input`, `forge-multiselect`, `forge-number-stepper`, `forge-otp-input`, `forge-phone-input`, `forge-radio-group`, `forge-search-input`, `forge-segment-control`, `forge-select`, `forge-time-input`, `forge-time-range-input` | visivo / `component.input`, `component.select`, `component.checkable`, O `component.field` secondo il controllo composto | `size`, `disabled`, strumenti di validazione e selezione; predefinito/focus-visibile/disabilitato/espanso/selezionato/non valido |
| organismi | `forge-date-time-range-input`, `forge-form-builder`, `forge-form-wizard`, `forge-schema-form-dialog`, `forge-schema-form`                                                                                                                                                                                                                                                 | visivo / `component.field` + contratti di input/selezione/overlay composti | schema, passaggi, validazione; predefinito/focus-visibile/disabilitato/espanso/selezionato/non valido |

### Icone — `packages/icons/src/components/`

Tutte le 106 voci di icone sono **visive ereditate**. Uso dei glifi `currentColor`; le loro dimensioni sono controllate dal consumatore o mappate
...
| `resource-planner/organisms` | `forge-resource-planner`                                                                                                                           | visivo | `component.resource-planner`                                 | risorse, gamma, selezione; predefinito/passa al passaggio del mouse/selezionato/focus-visibile/conflitto/non disponibile |
...
| `select/molecules`           | `forge-multiselect`, `forge-select`                                                                                                                | visivo | `component.select` + `component.input` + `component.field`   | dimensioni, opzioni, modello, validazione; predefinito/passaggio del mouse/focus-visibile/disabilitato/espanso/selezionato/non valido |
...
aiutanti ricorsivi `components/organisms/forge-tree-view/forge-tree-view-item`,
...
Viene caricata l'anteprima condivisa di Storybook `@mission-platform/tokens/scss/tokens`, il plug-in di override di Storybook e il file
...
per testare le sostituzioni del consumatore, modifica `apps/storybook/design-tokens/overrides.tokens.json` Sotto `component` utilizzando a
`{ "light": "...", "dark": "..." }` valore. Lo schema di sostituzione è

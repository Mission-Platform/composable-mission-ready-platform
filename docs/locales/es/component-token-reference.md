...
> Fuente en inglés: [docs/component-token-reference.md](../../component-token-reference.md)
> Idioma: Español (es)
...
los adaptadores de marco generados: la misma entrada se aplica a Vue, React, Solid, Sveltey componentes web.
...
`color-scheme`, `light-dark()`, `[data-theme]`, y `.theme-*` pines del subárbol. Los consumidores y Storybook pueden anular cualquier
hoja debajo `component` en `overrides.tokens.json`; se aplica una anulación después de la hoja de estilo del token generado.
...
A continuación solo se enumeran los estados admitidos por un componente. `expanded` se utiliza para revelar/superficies seleccionadas, `selected`
para opciones/pestañas/navegación, y `invalid` para validación de formularios; no se requieren variables de estado no utilizadas.
...
Cada viñeta a continuación es una entrada de inventario. A menos que una historia esté marcada `story: missing`, el componente tiene una coincidencia
...
Todas las entradas del formulario utilizan el compartido `component.field` roles de etiqueta/ayudante/error además del contrato a continuación. Nativo
...
Las 106 entradas de íconos son **heredadas-visuales**. Uso de glifos `currentColor`; su tamaño está controlado por el consumidor o se asigna a
...
es visual y utiliza `component.email`, con `component.button`, `component.typography`, o `component.media` donde se indique.
...
Hay 246 historias coubicadas para 249 fuentes componentes. Las únicas fuentes sin historias independientes son las
...
Se carga la vista previa compartida del Storybook. `@mission-platform/tokens/scss/tokens`, el complemento de anulación de Storybook y el
`theme` global. Para inspeccionar el contrato, configure el tema global en claro u oscuro y use los controles de las historias componentes;
para probar las anulaciones del consumidor, edite `apps/storybook/design-tokens/overrides.tokens.json` bajo `component` usando un
`{ "light": "...", "dark": "..." }` valor. El esquema de anulación es

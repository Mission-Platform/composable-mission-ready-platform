...
> 英文原文: [docs/component-token-reference.md](../../component-token-reference.md)
> 语言: 简体中文 (zh)

这是 Forge 创作的组件的规范清单和 Figma 交接。它是有意独立于
生成的框架适配器：相同的条目适用于 Vue, React, Solid, Svelte和 Web 组件。
...
下面的叶子 `component` 在 `overrides.tokens.json`;在生成的令牌样式表之后应用覆盖。
...
| `border` / `focus-ring`                      |笔画和键盘指示 | `default`, `hover`, `focus-visible`, `active`, `disabled`, `selected`, `invalid`       |
| `padding` / `gap` / `radius` / `shadow`      |几何和高程|默认或特定尺寸 |
| `opacity` / `transition`                     |去加重和运动 | `disabled`, `loading`, `hover`, `active`                                               |

下面仅列出了组件支持的状态。 `expanded` 用于披露/选择表面， `selected`
用于选择/选项卡/导航，以及 `invalid` 用于表单验证；不需要未使用的状态变量。
...
- **继承视觉** - 不渲染独立样式的主机；它的出现来自于孩子、父母， `currentColor`,
  第三方主机/画布，或组合组件的合约。
- **仅行为** - 控制渲染或视口行为，并且不做出自己的视觉决定。

下面的每一项都是一个库存条目。除非故事被标记 `story: missing`，该组件有一个匹配的
`<component>.stories.tsx` 在来源旁边。包/级别标题提供稳定的源路径前缀。

## `@mission-platform/components`

### 原子—— `packages/components/src/components/atoms/`
...
| `forge-quote`            |视觉| `component.typography` + `component.surface`    |引文、变体；默认 |
...
| `forge-language-switcher` |继承视觉 | `component.navigation` + 子选择合同 |语言环境；默认/扩展/选定|
...
| `forge-toast`             |视觉| `component.overlay` + `component.feedback`     |状态、持续时间；默认/加载 |
| `forge-tooltip`           |视觉| `component.overlay`                            |开放、安置；默认/扩展|
| `forge-window-popout`     |视觉| `component.overlay.window-popout`              |开放，尺寸；默认/悬停/焦点可见/选定 |

### 生物体和模板 — `packages/components/src/components/{organisms,templates}/`
...
| `forge-virtual-table`      |视觉| `component.data.virtual-table` + `component.data.table`          |列、大小、行高、高度、过扫描、条纹/边框、排序；默认/悬停/焦点可见 |
...
| `content/molecules`      | `forge-markdown`               |视觉| `component.typography` + `component.code`    |尺寸、链接；默认/无效 |
| `content/molecules`      | `markdown-block`               |继承视觉 | `component.typography` + 子合同 |令牌、大小；继承|
| `content/molecules`      | `markdown-inline`              |继承视觉 | `component.typography`                       |令牌、链接；继承/悬停/选择|
| `content/molecules`      | `forge-wysiwyg-block-controls` |视觉| `component.editor.block-controls` + `component.button` |块选择；默认/悬停/焦点可见/选定 |
| `content/molecules`      | `forge-wysiwyg-block-menu`     |视觉| `component.editor.block-menu` + `component.overlay`   |打开;默认/扩展/选定 |
| `content/molecules`      | `forge-wysiwyg-status-bar`     |视觉| `component.editor.status-bar`                         |地位;默认/无效/加载 |
| `content/molecules`      | `forge-wysiwyg-toolbar`        |视觉| `component.editor.toolbar` + `component.button`       |命令；默认/禁用 |
| `content/organisms`      | `forge-monaco-editor`          |视觉| `component.editor.monaco` + `component.code`          |语言，只读；默认/禁用/无效 |
| `content/organisms`      | `forge-wysiwyg-editor`         |视觉| `component.editor.wysiwyg` + `component.code`        |可编辑，无效；默认/焦点可见/无效/禁用 |
| `float/molecules`        | `forge-alert-banner`           |视觉| `component.feedback` + `component.overlay`   |地位，可解雇；默认/焦点可见 |
| `float/molecules`        | `forge-dropdown`               |视觉| `component.overlay` + `component.navigation` |打开;默认/扩展/选定 |
| `float/molecules`        | `forge-popover`                |视觉| `component.overlay`                          |打开;默认/扩展 |
| `float/molecules`        | `forge-toast`                  |视觉| `component.overlay` + `component.feedback`   |地位;默认/加载 |
...
### 表格 — `packages/forms/src/components/`

所有表单条目均使用共享 `component.field` 除了下面的合同之外，标签/帮助者/错误角色。本地人
仅在控件支持的情况下才表示控件状态。

|水平|组件（每个逗号分隔名称一个条目）|分类/合同|共享外观道具和状态 |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
|原子| `forge-checkbox`, `forge-input`, `forge-radio`, `forge-range-input`, `forge-rating`, `forge-slider`, `forge-switch`, `forge-textarea`                                                                                                                                                                                                                                     |视觉的 / `component.checkable` 用于复选框/收音机/评级/滑块/开关； `component.input` 用于输入/范围输入/文本区域 | `size`，标签/值道具；默认/悬停/活动/焦点可见/禁用/无效/在支持的情况下选择 |
|分子| `forge-calendar`, `forge-color-input`, `forge-date-input`, `forge-date-range-input`, `forge-field-set`, `forge-file-input`, `forge-location-input`, `forge-multiselect`, `forge-number-stepper`, `forge-otp-input`, `forge-phone-input`, `forge-radio-group`, `forge-search-input`, `forge-segment-control`, `forge-select`, `forge-time-input`, `forge-time-range-input` |视觉的 / `component.input`, `component.select`, `component.checkable`， 或者 `component.field` 按组合控制| `size`, `disabled`、验证和选择道具；默认/焦点可见/禁用/扩展/选定/无效 |
|生物| `forge-date-time-range-input`, `forge-form-builder`, `forge-form-wizard`, `forge-schema-form-dialog`, `forge-schema-form`                                                                                                                                                                                                                                                 |视觉的 / `component.field` + 组合输入/选择/覆盖合约 |模式、步骤、验证；默认/焦点可见/禁用/扩展/选定/无效 |

### 图标 — `packages/icons/src/components/`
...
| `select/molecules`           | `forge-language-switcher`                                                                                                                          |继承视觉 | `component.select` + `component.navigation`                  |语言环境；默认/扩展/选定|
...
| `theme/atoms`                | `forge-theme-toggle`                                                                                                                               |视觉| `component.button` + `component.icon`                        |模式;默认/悬停/活动/选定 |
...
`@mission-platform/email-components` 包含在内是因为其 TSX 源代码是 Forge 创作的。电子邮件客户端没有
使用运行时自定义属性：渲染器将相同的语义角色解析为内联值。下面的每个条目
是视觉和使用的 `component.email`， 和 `component.button`, `component.typography`， 或者 `component.media` 凡注明。
...
共享故事书预览加载 `@mission-platform/tokens/scss/tokens`、 Storybook 覆盖插件和
`theme` 全球的。要检查合约，请将全局主题设置为浅色或深色，并使用组件故事的控件；
要测试消费者覆盖，编辑 `apps/storybook/design-tokens/overrides.tokens.json` 在下面 `component` 使用
`{ "light": "...", "dark": "..." }` 价值。覆盖架构是

...
| 出口                 | 类型      | 描述                                                                                                                 |
| :----------------- | :------ | :----------------------------------------------------------------------------------------------------------------- |
| `toVueComponent`   | 适配器     | Converts a Forge component to a Vue 3 component from `@mission-platform/forge-adapters/vue`.       |
| `toReactComponent` | 适配器     | Converts a Forge component to a React component from `@mission-platform/forge-adapters/react`.     |
...
| `ContentComponent`        | Type     | 已订购 `ContentField`s、插槽和 `interactive` 旗帜。                                                       |
| `ContentFieldKind`        | Type     | `text`, `richtext`, `number`, `boolean`, `option`, `asset`, `link`, `children`. |
| `CmsOutputPlugin`         | Type     | 目标合约：一个绑定框架插件加上四个发射器。                                                                           |

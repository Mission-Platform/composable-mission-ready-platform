...
- **이미지 최적화**: WebP/AVIF 형식을 사용하고 반응형 이미지를 위해 `srcset`을 제공합니다.
- **리소스 사전 로드**: LCP 이미지 또는 중요 글꼴에는 `<link rel="preload">`을 사용합니다.
- **메인 스레드 작업 최소화**: `async` 또는 `defer`을 사용하여 필수적이지 않은 JavaScript를 연기합니다.

### 메모리 누수

**문제**: 애플리케이션은 시간이 지남에 따라 점점 더 많은 양의 메모리를 소비하여 결국 충돌이 발생합니다.
...
- **컴포저블 정리**: `onUnmounted`에서 항상 타이머를 지우고 이벤트 리스너를 제거합니다.
- **스토어 관리**: 더 이상 필요하지 않을 때 Pinia 또는 다른 스토어의 반응 상태가 지워지도록 보장합니다.
...
**문제**: TypeScript 또는 Vite은 작업공간에 정의된 패키지를 찾을 수 없습니다.

**해결책**:

1. 패키지가 소비 작업 영역의 `package.json`에 나열되어 있는지 확인합니다.
...
**문제**: IDE에 표시되지 않는 TypeScript 오류로 인해 CI에서 빌드가 실패합니다.
...
1. MCP 서버가 `pnpm exec turbo run build --filter @mission-platform/mcp-*`으로 구축되었는지 확인합니다.
2. 서버가 수동으로 시작되는지 확인하십시오: `node mcp/developer/dist/index.js`.

**해결책**:

- node 바이너리에 대한 절대 경로와 클라이언트 구성의 스크립트를 사용하고 있는지 확인하세요.
...
**원인**: 비동기 함수에서 포착되지 않은 오류가 발생했습니다. **수정**: 항상 비동기 호출을 `try/catch` 블록으로 래핑합니다.

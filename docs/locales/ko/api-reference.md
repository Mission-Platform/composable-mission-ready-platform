...
2. 검증기를 사용하여 **아티팩트 해시를 계산**합니다. `flint_verify_artifact(artifact)`은 `contentHash`을 반환합니다.
3. **해시 비교**: 일치하는 경우 예상 값이 기록된 이후 아티팩트가 실수로 또는 악의적으로 변경되지 않은 것입니다.
4. **매니페스트 확인**: `flint_inspect_manifest`를 사용하여 기능 가져오기, 내보내기, 메타데이터 및 정책 준수를 독립적으로 확인합니다.

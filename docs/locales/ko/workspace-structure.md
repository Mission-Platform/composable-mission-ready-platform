...
Mission Platform은 pnpm 작업 공간과 Turborepo를 사용하여 다중 패키지 환경을 관리합니다. 저장소가 정리되어 있습니다.
기능적 계층으로:
...
응용 프로그램은 `packages/` 디렉터리의 기능을 구성하는 배포 가능한 단위입니다. 그들은 대개 비공개입니다
레지스트리에 게시되지 않았습니다.

- **`docs/`**: Markdown 코퍼스에 대한 Vite + Vue 설명서 사이트입니다.
- **`my-care-notes/`**: 대표적인 케어 노트 애플리케이션입니다.
...
- **`@mission-platform/router`** 및 **`@mission-platform/i18n`**: 프레임워크 중립적 라우팅 및 지역화.
...
- **`eslint-config/`**, **`prettier-config/`** 및 **`stylelint-config/`**: 린팅 및 서식 지정 규칙.
- **`typescript-config/`**: Node, DOM, 라이브러리 및 프레임워크 소비자를 위한 기본 `tsconfig.json` 파일입니다.
...
- **`forge-spa/`**: `ASSETS` 바인딩 SPA 대체를 사용하여 정적 자산을 제공합니다.

배포 가능한 애플리케이션 작업자는 `apps/website/wrangler.jsonc`으로 구성됩니다.
`apps/my-care-notes/wrangler.jsonc` 및 `apps/service-monitor/wrangler.jsonc`. 는
`api-proxy` 및 `forge-spa` 패키지는 독립형 Wrangler 배포가 아닌 번들 종속성입니다.
...
`src/` 내의 모든 디렉터리에는 `index.ts`(배럴 파일)이 포함되어야 합니다.

- 하위 디렉토리는 로컬 `index.ts`을 통해 내부 기호를 내보냅니다.
- 루트 `src/index.ts`은 전체 작업공간 멤버에 대한 공개 진입점 역할을 합니다.
...
Mission Platform은 내부 종속성을 위해 `workspace:*` 프로토콜을 사용합니다. 이렇게 하면 패키지가 항상
개발 중 다른 작업공간 멤버의 로컬 버전.

### PNPM 카탈로그

저장소는 **pnpm 카탈로그**(`pnpm-workspace.yaml`에 정의됨)를 활용하여 전체 종속성 버전을 중앙 집중화합니다.
모노레포. 이는 버전 드리프트를 방지하고 유지 관리를 단순화합니다.

### 작업 실행

교차 작업공간 작업은 Turborepo를 사용하여 루트 `package.json`을 통해 실행됩니다.

- `pnpm build`: 올바른 종속성 순서로 모든 작업공간을 빌드합니다.
- `pnpm test`: `test` 태스크를 사용하여 모든 작업공간에 대한 테스트 스위트를 실행합니다. `pnpm exec turbo run test --affected`을 사용하세요.
  변경된 작업공간 CI 범위.
- `pnpm lint`: 작업공간 전체에서 ESLint을 실행합니다.
- `pnpm lint:style`: 앱 및 패키지 스타일에 대해 Stylelint를 실행합니다.
- `pnpm format`: Prettier으로 포맷을 확인하세요.

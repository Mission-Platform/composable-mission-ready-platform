...
This document describes the testing strategy and tooling for the Mission Platform monorepo. It serves as both a **How-to guide** for common testing tasks and a **Technical reference** for the underlying configuration.
...
### Browser Testing
Mission Platform utilizes Vitest's Browser Mode for tests that require a real DOM environment or cross-browser verification.
...
Most workspaces use the `defineVitestConfig` utility from `@mission-platform/vite-config`. This provides a standardized environment:

//! Shared MaxiCode primitives used by the encode/decode crates and the scanner's
//! MaxiCode locator.
//!
//! A MaxiCode symbol is a fixed 30-column × 33-row grid of hexagonal modules
//! (odd rows offset by half a module). Data is carried by 144 six-bit codewords;
//! [`BITNR`] maps each grid cell `(x, y)` to the bit index it carries (a negative
//! entry marks a fixed/structural module that carries no data). This crate holds
//! that map, the symbol dimensions, and the GF(64) Reed-Solomon corrector shared
//! by the reader and writer.

pub mod reed_solomon;

pub use reed_solomon::{correct, error_correction};

/// Number of module columns in a MaxiCode symbol.
pub const WIDTH: usize = 30;
/// Number of module rows in a MaxiCode symbol.
pub const HEIGHT: usize = 33;
/// Total data codewords carried by a symbol (before mode-specific splitting).
pub const TOTAL_CODEWORDS: usize = 144;

/// Per-cell bit index map (port of ZXing `BitMatrixParser.BITNR`), row-major
/// `HEIGHT × WIDTH`. `BITNR[y][x]` is the codeword bit number carried by cell
/// `(x, y)` (bit `b` lives in `codewords[b / 6]`, MSB-first: mask
/// `1 << (5 - b % 6)`). A negative entry is a fixed/structural module (finder,
/// orientation) carrying no data.
#[rustfmt::skip]
pub const BITNR: [[i16; WIDTH]; HEIGHT] = [
...
/// Read the 144 six-bit codewords out of a `HEIGHT × WIDTH` module grid (row
/// major, `true` = dark). Port of ZXing `BitMatrixParser.readCodewords`.
pub fn read_codewords(modules: &[bool]) -> [u8; TOTAL_CODEWORDS] {
    let mut result = [0u8; TOTAL_CODEWORDS];
    if modules.len() != WIDTH * HEIGHT {
        return result;
    }
    for (y, row) in BITNR.iter().enumerate() {
        for (x, &bit) in row.iter().enumerate() {
            if bit >= 0 && modules[y * WIDTH + x] {
...
/// Lay 144 six-bit codewords into a `HEIGHT × WIDTH` module grid (row major,
/// `true` = dark), the inverse of [`read_codewords`]. Structural (negative-index)
/// cells are left light; callers that need the exact finder/orientation modules
/// overlay them separately, but for a codeword round-trip only the data cells
/// matter (the reader ignores structural cells).
pub fn place_codewords(codewords: &[u8]) -> Vec<bool> {
    let mut modules = vec![false; WIDTH * HEIGHT];
    for (y, row) in BITNR.iter().enumerate() {
        for (x, &bit) in row.iter().enumerate() {
            if bit >= 0 {
                let bit = bit as usize;
                let cw = codewords.get(bit / 6).copied().unwrap_or(0);
                if (cw >> (5 - (bit % 6))) & 1 != 0 {
...
mod tests {
    ...
    #[test]
    fn bitnr_covers_every_data_bit_once() {
        // The 144 codewords carry 864 data bits; each must appear exactly once.
        let mut seen = [0u32; TOTAL_CODEWORDS * 6];
        for row in BITNR.iter() {
            for &bit in row.iter() {
                if bit >= 0 {
                    seen[bit as usize] += 1;
                }
        ...
        for (bit, &count) in seen.iter().enumerate() {
            assert_eq!(count, 1, "data bit {bit} appears {count} times");
        }
    }

    #[test]
    fn place_then_read_round_trips() {
        let mut codewords = [0u8; TOTAL_CODEWORDS];
        for (i, c) in codewords.iter_mut().enumerate() {
            *c = ((i as u8).wrapping_mul(7).wrapping_add(3)) & 0x3f;
        }
        let modules = place_codewords(&codewords);
        assert_eq!(read_codewords(&modules), codewords);
    }
}

...
const packageDirectories = ['packages', 'extensions'] as const;
...
type FormatterCommand = typeof execFile;
...
function isGeneratedFenceParserFailure(error: unknown): boolean {
  const diagnostics = commandOutput(error)
    .split('\n')
    .filter((line) => /^\s*\S.*\s+error\s+/u.test(line));
  return diagnostics.length > 0 && diagnostics.every((line) => /Parsing error:/u.test(line));
}

/** Run the generated-reference ESLint autofix followed by the repository Prettier write pass. */
export async function formatGeneratedDocumentation(
  repoRoot: string,
  outputPath: string,
  ...
  try {
    await runCommand(eslintPath, ['--config', eslintConfigPath, '--fix', outputPath], { cwd: repoRoot });
  } catch (error) {
    // Generated signatures intentionally contain declaration fragments (for example, an interface without a body).
    // The Markdown processor still visits those fences, but the current Babel parser reports them as syntax errors.
    // Do not let that known renderer limitation prevent Prettier or generation from completing; all other failures
    // remain fatal.
  ...
  for (const directory of packageDirectories) {
    ...
    if (await fileExists(resolve(base, 'package.json'))) {
      ...
      for (const manifestPath of manifests) {
        ...
        // Extension server packages are generated staging artifacts, not documentation sources.
        const relativePackageRoot = relative(repoRoot, dirname(manifestPath));
        if (manifest?.name !== undefined && !relativePackageRoot.split(sep).includes('server')) {
          roots.push(dirname(manifestPath));
        }
      }
    }
  }

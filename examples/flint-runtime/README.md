# Flint runtime example

This framework-neutral Vite fixture compiles `src/runtime.flint` together with its
same-project `src/project-helper.flint` dependency and a second project at
`shared-project/shared-value.flint`. Same-project links are static by default;
...
is available to the Flint module itself; the DOM is used only by the
...
Flint models recoverable outcomes with `Option<T>` and `Result<T,

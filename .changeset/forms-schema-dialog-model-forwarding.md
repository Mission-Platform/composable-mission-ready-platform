...
Fix `BaseSchemaFormDialog` silently dropping every value update on the Vue build.
...
`BaseSchemaForm` emitted `undefined`, so field edits never reached the host. The
